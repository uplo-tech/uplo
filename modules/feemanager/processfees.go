...
func (fm *FeeManager) createAndPersistTransaction(feeUIDs []modules.FeeUID, outputs []types.UplocoinOutput) (err error) {
	// Submit the outputs and get the transactions
	txns, err := fm.staticCommon.staticWallet.SendUplocoinsMulti(outputs)
	if err != nil {
		return errors.AddContext(err, "unable to build transaction for fee")
	}

	// From SendUplocoinsMulti, the transaction that was just created is the
