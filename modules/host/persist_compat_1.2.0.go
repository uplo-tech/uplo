...
var (
	...
	v112StorageManagerDBMetadata = persist.Metadata{
		Header:  "Uplo Storage Manager DB",
		Version: "0.6.0",
	}
...
func (h *Host) loadCompatV100(p *persistence) error {
	var compatPersistence struct {
		FinancialMetrics struct {
			PotentialStorageRevenue types.Currency `json:"potentialerevenue,uplomismatch"`
		}
		Settings struct {
			MinContractPrice          types.Currency `json:"contractprice,uplomismatch"`
			MinDownloadBandwidthPrice types.Currency `json:"minimumdownloadbandwidthprice,uplomismatch"`
			MinStoragePrice           types.Currency `json:"storageprice,uplomismatch"`
			MinUploadBandwidthPrice   types.Currency `json:"minimumuploadbandwidthprice,uplomismatch"`
		}
	}
