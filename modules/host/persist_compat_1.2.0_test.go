...
func loadExistingHostWithNewDeps(modulesDir, uploMuxDir, hostDir string) (closeFn, modules.Host, error) {
	// Create the uplomux
	mux, err := modules.NewUploMux(uploMuxDir, modulesDir, "localhost:0", "localhost:0")
	if err != nil {
		return nil, nil, err
	}
	...
	if !bytes.Equal(h.publicKey.Key, pubKey[:]) {
		return nil, nil, errors.New("host and uplomux pubkeys don't match")
	}
	privKey := mux.PrivateKey()
	if !bytes.Equal(h.secretKey[:], privKey[:]) {
		return nil, nil, errors.New("host and uplomux privkeys don't match")
	}
...
func TestV112StorageManagerUpgrade(t *testing.T) {
	...
	// ensures the uplomux picks this up and triggers its compatibility flow for
	// v112 as well.
	...
	uploMuxDir := filepath.Join(modulesDir, modules.UploMuxDir)
	closefn, host, err := loadExistingHostWithNewDeps(modulesDir, uploMuxDir, legacyHost)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
