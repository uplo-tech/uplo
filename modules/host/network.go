...
func (h *Host) initNetworking(address string) (err error) {
	...
	// Create a listener for the UploMux.
	if !h.dependencies.Disrupt("DisableHostUplomux") {
		err = h.staticMux.NewListener(modules.HostUploMuxSubscriberName, h.threadedHandleStream)
		if err != nil {
			return errors.AddContext(err, "Failed to subscribe to the UploMux")
		}
		// Close the listener when h.tg.OnStop is called.
		h.tg.OnStop(func() {
			h.staticMux.CloseListener(modules.HostUploMuxSubscriberName)
		})
	}
...
// threadedHandleStream handles incoming UploMux streams.
func (h *Host) threadedHandleStream(stream uplomux.Stream) {
