...
func TestV143HostUpgrade(t *testing.T) {
	...
	// uplomux but also to verify we're using the correct testdata
	hp := persistence{}
	...
	// create a uplomux using the compat flow to ensure we save it using the
	// host's key pair, this simulates how it would be on production (without
	// having to add uplomux testdata in the host module)
	uploMuxDir := filepath.Join(persistDir, modules.UploMuxDir)
	err = os.MkdirAll(uploMuxDir, persist.DefaultDiskPermissionsTest)
	if err != nil {
		t.Fatal(err)
	}

	// convert the keypair into a mux.ED25519 key pair
	smPK := modules.UploPKToMuxPK(hp.PublicKey)
	var smSK mux.ED25519SecretKey
	copy(smSK[:], hp.SecretKey[:])

	mux, err := uplomux.CompatV1421NewWithKeyPair("localhost:0", "localhost:0", log.DiscardLogger, uploMuxDir, smSK, smPK)
	if err != nil {
		t.Fatal(err)
	}
	err = mux.Close()
	...
	// load a new host
	closefn, host, err := loadExistingHostWithNewDeps(persistDir, uploMuxDir, hostDir)
	if err != nil {
		t.Fatal(err)
	}
