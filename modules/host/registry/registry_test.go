...
func TestRegistryRace(t *testing.T) {
	...
	keys := make([]types.UploPublicKey, 0, numEntries)
	...
	worker := func(key types.UploPublicKey, sk crypto.SecretKey, rv modules.SignedRegistryValue, nextExpiry, nextRevision *uint64) {
		for {
...
func BenchmarkRegistryUpdate(b *testing.B) {
	...
	keys := make([]types.UploPublicKey, 0, nEntries)
