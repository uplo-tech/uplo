...
func (h *Host) managedAddCollateral(settings modules.HostExternalSettings, txnSet []types.Transaction) (builder modules.TransactionBuilder, newParents []types.Transaction, newInputs []types.UplocoinInput, newOutputs []types.UplocoinOutput, err error) {
	...
	err = builder.FundUplocoins(hostPortion)
	if err != nil {
		builder.Drop()
