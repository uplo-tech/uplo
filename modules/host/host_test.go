...
type renterHostPair struct {
	...
	staticRenterPK   types.UploPublicKey
	staticRenterMux  *uplomux.UploMux
	staticHT         *hostTester
...
func newRenterHostPairCustomHostTester(ht *hostTester) (*renterHostPair, error) {
	...
	// prepare a uplomux for the renter
	...
	muxLogger, err := persist.NewFileLogger(filepath.Join(renterMuxDir, "uplomux.log"))
	if err != nil {
		return nil, errors.AddContext(err, "unable to create mux logger")
	}
	renterMux, err := uplomux.New("127.0.0.1:0", "127.0.0.1:0", muxLogger.Logger, renterMuxDir)
	if err != nil {
...
func (p *renterHostPair) managedNewStream() uplomux.Stream {
	pk := modules.UploPKToMuxPK(p.staticHT.host.publicKey)
	address := p.staticHT.host.ExternalSettings().UploMuxAddress()
	subscriber := modules.HostUploMuxSubscriberName
...
// returned response.
func (p *renterHostPair) managedPayByContract(stream uplomux.Stream, amount types.Currency, refundAccount modules.AccountID) error {
	// create the revision.
...
// EA.
func (p *renterHostPair) managedPayByEphemeralAccount(stream uplomux.Stream, amount types.Currency) error {
	// Send the payment request.
...
func (p *renterHostPair) managedFinalizeWriteProgram(stream uplomux.Stream, lastOutput executeProgramResponse, bh types.BlockHeight) error {
...
// it.
func (p *renterHostPair) managedBeginSubscription(payByFC bool, fundAmt types.Currency, fundAcc modules.AccountID, subscriber types.Specifier) (_ uplomux.Stream, err error) {
	stream := p.managedNewStream()
	defer func() {
		if err != nil {
			err = errors.Compose(err, stream.Close())
...
func (p *renterHostPair) BeginSubscription(budget types.Currency, subscriber types.Specifier) (uplomux.Stream, error) {
...
func (p *renterHostPair) StopSubscription(stream uplomux.Stream) error {
...
func (p *renterHostPair) SubcribeToRV(stream uplomux.Stream, pt *modules.RPCPriceTable, pubkey types.UploPublicKey, tweak crypto.Hash) (*modules.SignedRegistryValue, error) {
...
func (p *renterHostPair) UnsubcribeFromRV(stream uplomux.Stream, pt *modules.RPCPriceTable, pubkey types.UploPublicKey, tweak crypto.Hash) error {
	// Send the type of the request.
...
func (p *renterHostPair) FundSubscription(stream uplomux.Stream, fundAmt types.Currency) error {
	// Send the type of the request.
...
func (p *renterHostPair) ExtendSubscription(stream uplomux.Stream, pt *modules.RPCPriceTable) error {
	// Send the type of the request.
...
func TestHostMultiClose(t *testing.T) {
	...
	err = ht.host.Close()
	if !errors.Contains(err, uplosync.ErrStopped) {
		t.Fatal(err)
	}
	err = ht.host.Close()
	if !errors.Contains(err, uplosync.ErrStopped) {
