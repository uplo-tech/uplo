...
// string otherwise.
func (h *Host) ProcessPayment(stream uplomux.Stream) (modules.PaymentDetails, error) {
...
func (h *Host) staticPayByEphemeralAccount(stream uplomux.Stream) (modules.PaymentDetails, error) {
...
// given stream.
func (h *Host) managedPayByContract(stream uplomux.Stream) (modules.PaymentDetails, error) {
...
func (h *Host) managedFundAccount(stream uplomux.Stream, request modules.FundAccountRequest, cost types.Currency) (types.Currency, error) {
	// read the PayByContractRequest
	var pbcr modules.PayByContractRequest
	if err := modules.RPCRead(stream, &pbcr); err != nil {
...
func revisionFromRequest(recent types.FileContractRevision, pbcr modules.PayByContractRequest) types.FileContractRevision {
	...
	rev.NewValidProofOutputs = make([]types.UplocoinOutput, len(pbcr.NewValidProofValues))
	for i, v := range pbcr.NewValidProofValues {
		if i >= len(recent.NewValidProofOutputs) {
			break
		}
		rev.NewValidProofOutputs[i] = types.UplocoinOutput{
			Value:      v,
			UnlockHash: recent.NewValidProofOutputs[i].UnlockHash,
		}
	}

	rev.NewMissedProofOutputs = make([]types.UplocoinOutput, len(pbcr.NewMissedProofValues))
	for i, v := range pbcr.NewMissedProofValues {
		if i >= len(recent.NewMissedProofOutputs) {
			break
		}
		rev.NewMissedProofOutputs[i] = types.UplocoinOutput{
			Value:      v,
