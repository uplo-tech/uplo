...
func TestHostPersistCompat100(t *testing.T) {
	...
	// Create a new uplomux to ensure it has the chance to load the appropriate
	// set of keys
	uploMuxDir := filepath.Join(ht.persistDir, modules.UploMuxDir)
	mux, err := modules.NewUploMux(uploMuxDir, ht.persistDir, "localhost:0", "localhost:0")
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
