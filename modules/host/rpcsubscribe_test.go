...
// testing.
func randomRegistryValue() (modules.SignedRegistryValue, types.UploPublicKey, crypto.SecretKey) {
...
func assertSubscriptionInfos(host *Host, spk types.UploPublicKey, tweak crypto.Hash, n int) ([]*subscriptionInfo, error) {
...
func readAndAssertRegistryValueNotification(spk types.UploPublicKey, rv modules.SignedRegistryValue, r io.Reader) error {
...
func testRPCSubscribeBasic(t *testing.T, rhp *renterHostPair) {
	...
	err := rhp.staticRenterMux.NewListener(hex.EncodeToString(sub[:]), func(stream uplomux.Stream) {
		notificationMu.Lock()
