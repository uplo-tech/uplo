...
type Host interface {
	...
	RegistryUpdate(rv modules.SignedRegistryValue, pubKey types.UploPublicKey, expiry types.BlockHeight) (modules.SignedRegistryValue, error)
	RegistryGet(pubKey types.UploPublicKey, tweak crypto.Hash) (modules.SignedRegistryValue, bool)
}

// MDM (Merklized Data Machine) is a virtual machine that executes instructions
// on the data in a Uplo file contract. The file contract tracks the size and
