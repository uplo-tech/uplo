...
// the Host.
func newHost(dependencies modules.Dependencies, smDeps modules.Dependencies, cs modules.ConsensusSet, g modules.Gateway, tpool modules.TransactionPool, wallet modules.Wallet, mux *uplomux.UploMux, listenerAddress string, persistDir string) (_ *Host, err error) {
...
func New(cs modules.ConsensusSet, g modules.Gateway, tpool modules.TransactionPool, wallet modules.Wallet, mux *uplomux.UploMux, address string, persistDir string) (*Host, error) {
...
func NewCustomHost(deps modules.Dependencies, cs modules.ConsensusSet, g modules.Gateway, tpool modules.TransactionPool, wallet modules.Wallet, mux *uplomux.UploMux, address string, persistDir string) (*Host, error) {
...
func NewCustomTestHost(deps modules.Dependencies, smDeps modules.Dependencies, cs modules.ConsensusSet, g modules.Gateway, tpool modules.TransactionPool, wallet modules.Wallet, mux *uplomux.UploMux, address string, persistDir string) (*Host, error) {
...
func (h *Host) BandwidthCounters() (uint64, uint64, time.Time, error) {
	...
	// wrap the uplomux streams since that wouldn't give us the raw data sent over
...
func (h *Host) PublicKey() types.UploPublicKey {
	h.mu.RLock()
	defer h.mu.RUnlock()
	return h.publicKey
}
...
func (h *Host) RegistryGet(pubKey types.UploPublicKey, tweak crypto.Hash) (modules.SignedRegistryValue, bool) {
	err := h.tg.Add()
	if err != nil {
...
func (h *Host) RegistryUpdate(rv modules.SignedRegistryValue, pubKey types.UploPublicKey, expiry types.BlockHeight) (modules.SignedRegistryValue, error) {
	err := h.tg.Add()
	if err != nil {
