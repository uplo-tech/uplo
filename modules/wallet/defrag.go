...
func (w *Wallet) managedCreateDefragTransaction() ([]types.Transaction, error) {
	...
	err = dbForEachUplocoinOutput(w.dbTx, func(scoid types.UplocoinOutputID, sco types.UplocoinOutput) {
	...
	addSignatures(&txn, types.FullCoveredFields, parentUnlockConditions, crypto.Hash(parentTxn.UplocoinOutputID(0)), w.keys[parentUnlockConditions.UnlockHash()], consensusHeight)
	...
	if err = dbPutSpentOutput(w.dbTx, types.OutputID(parentTxn.UplocoinOutputID(0)), consensusHeight); err != nil {
		return nil, err
	}
