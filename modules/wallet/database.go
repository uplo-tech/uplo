...
var (
	...
	// bucketUplocoinOutputs maps a UplocoinOutputID to its UplocoinOutput. Only
	// outputs that the wallet controls are stored. The wallet uses these
	// outputs to fund transactions.
	bucketUplocoinOutputs = []byte("bucketUplocoinOutputs")
	// bucketUplocoinOutputs maps a UplofundOutputID to its UplofundOutput. Only
	// outputs that the wallet controls are stored. The wallet uses these
	// outputs to fund transactions.
	bucketUplofundOutputs = []byte("bucketUplofundOutputs")
	...
	dbBuckets = [][]byte{
		...
		bucketUplocoinOutputs,
		bucketUplofundOutputs,
		bucketSpentOutputs,
	...
	keyUplofundPool            = []byte("keyUplofundPool")
...
func dbReset(tx *bolt.Tx) error {
	...
	dbPutUplofundPool(tx, types.ZeroCurrency)
...
func dbPutUplocoinOutput(tx *bolt.Tx, id types.UplocoinOutputID, output types.UplocoinOutput) error {
	return dbPut(tx.Bucket(bucketUplocoinOutputs), id, output)
}
func dbDeleteUplocoinOutput(tx *bolt.Tx, id types.UplocoinOutputID) error {
	return dbDelete(tx.Bucket(bucketUplocoinOutputs), id)
}
func dbForEachUplocoinOutput(tx *bolt.Tx, fn func(types.UplocoinOutputID, types.UplocoinOutput)) error {
	return dbForEach(tx.Bucket(bucketUplocoinOutputs), fn)
}

func dbPutUplofundOutput(tx *bolt.Tx, id types.UplofundOutputID, output types.UplofundOutput) error {
	return dbPut(tx.Bucket(bucketUplofundOutputs), id, output)
}
func dbDeleteUplofundOutput(tx *bolt.Tx, id types.UplofundOutputID) error {
	return dbDelete(tx.Bucket(bucketUplofundOutputs), id)
}
func dbForEachUplofundOutput(tx *bolt.Tx, fn func(types.UplofundOutputID, types.UplofundOutput)) error {
	return dbForEach(tx.Bucket(bucketUplofundOutputs), fn)
}
...
// dbGetUplofundPool returns the value of the uplofund pool.
func dbGetUplofundPool(tx *bolt.Tx) (pool types.Currency, err error) {
	err = encoding.Unmarshal(tx.Bucket(bucketWallet).Get(keyUplofundPool), &pool)
	return
}

// dbPutUplofundPool stores the value of the uplofund pool.
func dbPutUplofundPool(tx *bolt.Tx, pool types.Currency) error {
	return tx.Bucket(bucketWallet).Put(keyUplofundPool, encoding.Marshal(pool))
}
