...
func postEncryptionTesting(m modules.TestMiner, w *Wallet, masterKey crypto.CipherKey) {
	...
	_, err = w.SendUplocoins(types.CalculateCoinbase(0), types.UnlockHash{})
	if err != nil {
		panic(err)
	}
	_, err = m.AddBlock()
	...
	if UplocoinBal2.Cmp(UplocoinBal) >= 0 {
...
func TestLock(t *testing.T) {
	...
	// Lock the wallet.
	UplocoinBalance, _, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Error(err)
	}
	err = wt.wallet.Lock()
	...
	UplocoinBalance2, _, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Error(err)
	}
	if !UplocoinBalance2.Equals(UplocoinBalance) {
		t.Error("Uplocoin balance reporting changed upon closing the wallet")
	}
	...
	UplocoinBalance3, _, _, err := wt.wallet.ConfirmedBalance()
	if err != nil {
		t.Error(err)
	}
	if UplocoinBalance3.Cmp(UplocoinBalance2) <= 0 {
...
func TestReset(t *testing.T) {
	...
	originalKey := crypto.GenerateUploKey(crypto.TypeDefaultWallet)
	...
	newKey := crypto.GenerateUploKey(crypto.TypeDefaultWallet)
