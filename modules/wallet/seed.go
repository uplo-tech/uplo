...
// NextAddresses returns n unlock hashes that are ready to receive Uplocoins or
// uplofunds. The addresses are generated using the primary address seed.
//
...
// NextAddress returns an unlock hash that is ready to receive Uplocoins or
// uplofunds. The address is generated using the primary address seed.
...
// If only uplofunds were found, the fee is deducted from the wallet.
func (w *Wallet) SweepSeed(seed modules.Seed) (coins, funds types.Currency, err error) {
	...
	if len(s.UplocoinOutputs) == 0 && len(s.uplofundOutputs) == 0 {
	...
	var UplocoinOutputs, uplofundOutputs []scannedOutput
	for _, sco := range s.UplocoinOutputs {
		UplocoinOutputs = append(UplocoinOutputs, sco)
	}
	for _, sfo := range s.uplofundOutputs {
		uplofundOutputs = append(uplofundOutputs, sfo)
	}

	for len(UplocoinOutputs) > 0 || len(uplofundOutputs) > 0 {
		// process up to maxOutputs UplocoinOutputs
		txnUplocoinOutputs := make([]scannedOutput, maxOutputs)
		n := copy(txnUplocoinOutputs, UplocoinOutputs)
		txnUplocoinOutputs = txnUplocoinOutputs[:n]
		UplocoinOutputs = UplocoinOutputs[n:]

		// process up to (maxOutputs-n) uplofundOutputs
		txnUplofundOutputs := make([]scannedOutput, maxOutputs-n)
		n = copy(txnUplofundOutputs, uplofundOutputs)
		txnUplofundOutputs = txnUplofundOutputs[:n]
		uplofundOutputs = uplofundOutputs[n:]
		...
		for _, output := range txnUplocoinOutputs {
			// construct a Uplocoin input that spends the output
			sk := generateSpendableKey(seed, output.seedIndex)
			tb.AddUplocoinInput(types.UplocoinInput{
		...
		for _, output := range txnUplofundOutputs {
			// construct a uplofund input that spends the output
			sk := generateSpendableKey(seed, output.seedIndex)
			tb.AddUplofundInput(types.UplofundInput{
				ParentID:         types.UplofundOutputID(output.id),
				UnlockConditions: sk.UnlockConditions,
			})
			// add a signature for the input
		...
		estTxnSize := (len(txnUplocoinOutputs) + len(txnUplofundOutputs)) * outputSize
		estFee := maxFee.Mul64(uint64(estTxnSize))
		tb.AddMinerFee(estFee)

		// calculate total Uplocoin payout
		...
		case !txnCoins.IsZero() && txnFunds.IsZero():
			// if we're sweeping coins but not funds, add a Uplocoin output for
			// them
			tb.AddUplocoinOutput(types.UplocoinOutput{
				Value:      txnCoins,
				UnlockHash: uc.UnlockHash(),
			})

		case txnCoins.IsZero() && !txnFunds.IsZero():
			// if we're sweeping funds but not coins, add a uplofund output for
			...
			tb.AddUplofundOutput(types.UplofundOutput{
				Value:      txnFunds,
				UnlockHash: uc.UnlockHash(),
			})
			err = tb.FundUplocoins(estFee)
			if err != nil {
				return types.Currency{}, types.Currency{}, errors.New("couldn't pay transaction fee on swept funds: " + err.Error())
			}

		case !txnCoins.IsZero() && !txnFunds.IsZero():
			// if we're sweeping both coins and funds, add a Uplocoin output and a
			// uplofund output
			tb.AddUplocoinOutput(types.UplocoinOutput{
				Value:      txnCoins,
				UnlockHash: uc.UnlockHash(),
		...
		for _, sfo := range txnUplofundOutputs {
		...
		// this case, we iterate through the UplocoinInputs and add a signature for
		// any input that belongs to the wallet.
		w.mu.RLock()
		for _, input := range txn.UplocoinInputs {
