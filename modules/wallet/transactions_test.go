...
func TestIntegrationTransactions(t *testing.T) {
	...
	_, err = wt.wallet.SendUplocoins(sentValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationTransaction(t *testing.T) {
	...
	// test sending Uplocoins
	sentValue := types.NewCurrency64(5000)
	sendTxns, err := wt.wallet.SendUplocoins(sentValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationAddressTransactions(t *testing.T) {
	...
	_, err = wt.wallet.SendUplocoins(types.NewCurrency64(5005), addr)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransactionInputOutputIDs(t *testing.T) {
	...
	// verify the miner payouts and Uplocoin outputs/inputs have correct IDs
	...
	for _, txn := range txns {
		...
		for i, output := range txn.Outputs {
			...
			if output.FundType == types.SpecifierUplocoinOutput {
				if output.ID != types.OutputID(txn.Transaction.UplocoinOutputID(uint64(i))) {
					t.Fatal("Uplocoin output had incorrect output ID")
				}
			}
		}
