...
func TestSignTransaction(t *testing.T) {
	...
	// get a Uplocoin output and a uplofund output
	...
	for _, o := range outputs {
		if o.FundType == types.SpecifierUplocoinOutput {
			sco = o
		} else if o.FundType == types.SpecifierUplofundOutput {
			sfo = o
		}
	}
	...
	txn := types.Transaction{
		UplofundInputs: []types.UplofundInput{{
			ParentID:         types.UplofundOutputID(sfo.ID),
		...
		UplocoinInputs: []types.UplocoinInput{{
			ParentID:         types.UplocoinOutputID(sco.ID),
...
func TestSignTransactionNoWallet(t *testing.T) {
	...
	txn := types.Transaction{
		...
		UplofundInputs: []types.UplofundInput{{
			ParentID:         types.UplofundOutputID{2}, // doesn't need to actually exist
			UnlockConditions: sk.UnlockConditions,
		}},
		UplofundOutputs: []types.UplofundOutput{{
			Value:      types.NewCurrency64(1),
...
func TestUnspentOutputs(t *testing.T) {
	...
	_, err = wt.wallet.SendUplocoins(types.UplocoinPrecision.Mul64(77), addr)
	if err != nil {
		t.Fatal(err)
	}
	wt.miner.AddBlock()
