...
type (
	...
	// SendUplocoins and SendUplocoinsMulti to fail before AcceptTransactionSet
	// is called
	dependencySendUplocoinsInterrupted struct {
...
func (d *dependencySendUplocoinsInterrupted) Disrupt(s string) bool {
	if d.f && s == "SendUplocoinsInterrupted" {
		d.f = false
		return true
	}
	return false
...
// fail causes the next SendUplocoinsInterrupted disrupt to return true
func (d *dependencySendUplocoinsInterrupted) fail() {
	d.f = true
}
