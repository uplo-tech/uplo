...
func createWalletTester(name string, deps modules.Dependencies) (*walletTester, error) {
	...
	masterKey := crypto.GenerateUploKey(crypto.TypeDefaultWallet)
...
func TestAdvanceLookaheadForceRescan(t *testing.T) {
	...
	farPayout := types.UplocoinPrecision.Mul64(8888)

	builder, err := wt.wallet.StartTransaction()
	if err != nil {
		t.Fatal(err)
	}
	builder.AddUplocoinOutput(types.UplocoinOutput{
		UnlockHash: farAddr,
		Value:      farPayout,
	})
	err = builder.FundUplocoins(farPayout)
	if err != nil {
		t.Fatal(err)
	}
	...
	for uh, index := range wt.wallet.lookahead {
		...
		sco := types.UplocoinOutput{
			UnlockHash: uh,
			Value:      types.UplocoinPrecision.Mul64(1000),
		}
		builder.AddUplocoinOutput(sco)
		payout = payout.Add(sco.Value)
...
func TestDistantWallets(t *testing.T) {
	...
	for i := uint64(0); i < lookaheadBuffer/2; i++ {
		_, err = wt.wallet.SendUplocoins(types.UplocoinPrecision, types.UnlockHash{})
		if err != nil {
			t.Fatal(err)
		}
		if err := wt.addBlockNoPayout(); err != nil {
	...
	value := types.UplocoinPrecision.Mul64(1e3)
	tbuilder.AddUplocoinOutput(types.UplocoinOutput{
		UnlockHash: farAddr,
		Value:      value,
	})
	err = tbuilder.FundUplocoins(value)
	if err != nil {
		t.Fatal(err)
	}
