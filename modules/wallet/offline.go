...
func (w *Wallet) UnspentOutputs() ([]modules.UnspentOutput, error) {
	...
	dbForEachUplocoinOutput(w.dbTx, func(scoid types.UplocoinOutputID, sco types.UplocoinOutput) {
	...
	dbForEachUplofundOutput(w.dbTx, func(sfoid types.UplofundOutputID, sfo types.UplofundOutput) {
		outputs = append(outputs, modules.UnspentOutput{
...
func (w *Wallet) UnlockConditions(addr types.UnlockHash) (uc types.UnlockConditions, err error) {
	...
	uc.PublicKeys = append([]types.UploPublicKey(nil), uc.PublicKeys...)
	return uc, nil
}
...
func (w *Wallet) RemoveWatchAddresses(addrs []types.UnlockHash, unused bool) error {
	...
	err := func() error {
		...
		if !unused {
			...
			// UplocoinOutputs bucket. Iterate through the bucket and remove
			// any outputs that we are no longer watching.
			var outputIDs []types.UplocoinOutputID
			dbForEachUplocoinOutput(w.dbTx, func(scoid types.UplocoinOutputID, sco types.UplocoinOutput) {
			...
			for _, scoid := range outputIDs {
				if err := dbDeleteUplocoinOutput(w.dbTx, scoid); err != nil {
					return err
				}
			}
