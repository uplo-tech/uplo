...
type seedScanner struct {
	...
	seed             modules.Seed
	UplocoinOutputs   map[types.UplocoinOutputID]scannedOutput
	uplofundOutputs   map[types.UplofundOutputID]scannedOutput
...
func (s *seedScanner) ProcessConsensusChange(cc modules.ConsensusChange) {
	...
	for _, diff := range cc.UplocoinOutputDiffs {
		if diff.Direction == modules.DiffApply {
			if index, exists := s.keys[diff.UplocoinOutput.UnlockHash]; exists && diff.UplocoinOutput.Value.Cmp(s.dustThreshold) > 0 {
				s.UplocoinOutputs[diff.ID] = scannedOutput{
					id:        types.OutputID(diff.ID),
					value:     diff.UplocoinOutput.Value,
					seedIndex: index,
				}
			}
			...
			if _, exists := s.keys[diff.UplocoinOutput.UnlockHash]; exists {
				delete(s.UplocoinOutputs, diff.ID)
			}
		}
	}
	for _, diff := range cc.UplofundOutputDiffs {
		if diff.Direction == modules.DiffApply {
			...
			// sweep every uplofund found
			if index, exists := s.keys[diff.UplofundOutput.UnlockHash]; exists {
				s.uplofundOutputs[diff.ID] = scannedOutput{
					id:        types.OutputID(diff.ID),
					value:     diff.UplofundOutput.Value,
					seedIndex: index,
				}
			}
		} else if diff.Direction == modules.DiffRevert {
			// NOTE: DiffRevert means the output was either spent or was in a
			// block that was reverted.
			if _, exists := s.keys[diff.UplofundOutput.UnlockHash]; exists {
				delete(s.uplofundOutputs, diff.ID)
			}
		}
	}
	...
	for _, diff := range cc.UplocoinOutputDiffs {
		index, exists := s.keys[diff.UplocoinOutput.UnlockHash]
		if exists {
	...
	for _, diff := range cc.UplofundOutputDiffs {
		index, exists := s.keys[diff.UplofundOutput.UnlockHash]
		if exists {
			s.log.Debugln("Seed scanner found a key used at index", index)
			if index > s.largestIndexSeen {
				s.largestIndexSeen = index
...
func newSeedScanner(seed modules.Seed, log *persist.Logger) *seedScanner {
	return &seedScanner{
		...
		UplocoinOutputs: make(map[types.UplocoinOutputID]scannedOutput),
		uplofundOutputs: make(map[types.UplofundOutputID]scannedOutput),
