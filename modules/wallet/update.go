...
type (
	spentUplocoinOutputSet map[types.UplocoinOutputID]types.UplocoinOutput
	spentUplofundOutputSet map[types.UplofundOutputID]types.UplofundOutput
)
...
func (w *Wallet) updateLookahead(tx *bolt.Tx, cc modules.ConsensusChange) (bool, error) {
	...
	for _, diff := range cc.UplocoinOutputDiffs {
		if index, ok := w.lookahead[diff.UplocoinOutput.UnlockHash]; ok {
	...
	for _, diff := range cc.UplofundOutputDiffs {
		if index, ok := w.lookahead[diff.UplofundOutput.UnlockHash]; ok {
			if index > largestIndex {
				largestIndex = index
			}
		}
...
func (w *Wallet) updateConfirmedSet(tx *bolt.Tx, cc modules.ConsensusChange) error {
	for _, diff := range cc.UplocoinOutputDiffs {
		...
		if !w.isWalletAddress(diff.UplocoinOutput.UnlockHash) {
			continue
		}

		var err error
		if diff.Direction == modules.DiffApply {
			w.log.Println("Wallet has gained a spendable Uplocoin output:", diff.ID, "::", diff.UplocoinOutput.Value.HumanString())
			err = dbPutUplocoinOutput(tx, diff.ID, diff.UplocoinOutput)
		} else {
			w.log.Println("Wallet has lost a spendable Uplocoin output:", diff.ID, "::", diff.UplocoinOutput.Value.HumanString())
			err = dbDeleteUplocoinOutput(tx, diff.ID)
		}
		if err != nil {
			w.log.Severe("Could not update Uplocoin output:", err)
			return err
		}
	}
	for _, diff := range cc.UplofundOutputDiffs {
		// Verify that the diff is relevant to the wallet.
		if !w.isWalletAddress(diff.UplofundOutput.UnlockHash) {
			continue
		}

		var err error
		if diff.Direction == modules.DiffApply {
			w.log.Println("Wallet has gained a spendable uplofund output:", diff.ID, "::", diff.UplofundOutput.Value)
			err = dbPutUplofundOutput(tx, diff.ID, diff.UplofundOutput)
		} else {
			w.log.Println("Wallet has lost a spendable uplofund output:", diff.ID, "::", diff.UplofundOutput.Value)
			err = dbDeleteUplofundOutput(tx, diff.ID)
		}
		if err != nil {
			w.log.Severe("Could not update uplofund output:", err)
			return err
		}
	}
	for _, diff := range cc.UplofundPoolDiffs {
		var err error
		if diff.Direction == modules.DiffApply {
			err = dbPutUplofundPool(tx, diff.Adjusted)
		} else {
			err = dbPutUplofundPool(tx, diff.Previous)
		}
		if err != nil {
			w.log.Severe("Could not update uplofund pool:", err)
			return err
		}
	}
	return nil
...
// outputs and collects them in a map of UplocoinOutputID -> UplocoinOutput.
func computeSpentUplocoinOutputSet(diffs []modules.UplocoinOutputDiff) spentUplocoinOutputSet {
	outputs := make(spentUplocoinOutputSet)
	for _, diff := range diffs {
		if diff.Direction == modules.DiffRevert {
			// DiffRevert means spent.
			outputs[diff.ID] = diff.UplocoinOutput
		}
	}
	return outputs
}

// computeSpentUplofundOutputSet scans a slice of Uplofund output diffs for spent
// outputs and collects them in a map of UplofundOutputID -> UplofundOutput.
func computeSpentUplofundOutputSet(diffs []modules.UplofundOutputDiff) spentUplofundOutputSet {
	outputs := make(spentUplofundOutputSet)
	for _, diff := range diffs {
		if diff.Direction == modules.DiffRevert {
			// DiffRevert means spent.
			outputs[diff.ID] = diff.UplofundOutput
		}
	}
	return outputs
}
...
func (w *Wallet) computeProcessedTransactionsFromBlock(tx *bolt.Tx, block types.Block, spentUplocoinOutputs spentUplocoinOutputSet, spentUplofundOutputs spentUplofundOutputSet, consensusHeight types.BlockHeight) []modules.ProcessedTransaction {
	...
	for _, txn := range block.Transactions {
		...
		for _, sco := range txn.UplocoinOutputs {
		...
		for _, sfo := range txn.UplofundOutputs {
		...
		for i, sco := range txn.UplocoinOutputs {
			...
			if po.WalletAddress {
				w.log.Println("\tUplocoin Output:", po.ID, "::", po.Value.HumanString())
			}
		}
		...
		for _, sfi := range txn.UplofundInputs {
			pi := modules.ProcessedInput{
				...
				FundType:       types.SpecifierUplofundInput,
				WalletAddress:  w.isWalletAddress(sfi.UnlockConditions.UnlockHash()),
				RelatedAddress: sfi.UnlockConditions.UnlockHash(),
				Value:          spentUplofundOutputs[sfi.ParentID].Value,
			}
			pt.Inputs = append(pt.Inputs, pi)
			// Log any wallet-relevant inputs.
			if pi.WalletAddress {
				w.log.Println("\tUplofund Input:", pi.ParentID, "::", pi.Value.HumanString())
			}

			uplofundPool, err := dbGetUplofundPool(w.dbTx)
			if err != nil {
				w.log.Println("could not get uplofund pool: ", err)
				continue
			}

			sfo := spentUplofundOutputs[sfi.ParentID]
			po := modules.ProcessedOutput{
				...
				Value:          uplofundPool.Sub(sfo.ClaimStart).Mul(sfo.Value),
			}
			pt.Outputs = append(pt.Outputs, po)
			// Log any wallet-relevant outputs.
			if po.WalletAddress {
		...
		for i, sfo := range txn.UplofundOutputs {
			po := modules.ProcessedOutput{
				ID:             types.OutputID(txn.UplofundOutputID(uint64(i))),
				FundType:       types.SpecifierUplofundOutput,
				MaturityHeight: consensusHeight,
			...
			// Log any wallet-relevant outputs.
			if po.WalletAddress {
				w.log.Println("\tUplofund Output:", po.ID, "::", po.Value.HumanString())
			}
		}
...
func (w *Wallet) applyHistory(tx *bolt.Tx, cc modules.ConsensusChange) error {
	spentUplocoinOutputs := computeSpentUplocoinOutputSet(cc.UplocoinOutputDiffs)
	spentUplofundOutputs := computeSpentUplofundOutputSet(cc.UplofundOutputDiffs)
	...
	for _, block := range cc.AppliedBlocks {
		...
		pts := w.computeProcessedTransactionsFromBlock(tx, block, spentUplocoinOutputs, spentUplofundOutputs, consensusHeight)
		for _, pt := range pts {
...
func (w *Wallet) ReceiveUpdatedUnconfirmedTransactions(diff *modules.TransactionPoolDiff) {
	...
	for _, unconfirmedTxnSet := range diff.AppliedTransactions {
		...
		spentUplocoinOutputs := make(map[types.UplocoinOutputID]types.UplocoinOutput)
		for _, scod := range unconfirmedTxnSet.Change.UplocoinOutputDiffs {
			...
			if scod.Direction == modules.DiffRevert {
				spentUplocoinOutputs[scod.ID] = scod.UplocoinOutput
			}
		}
