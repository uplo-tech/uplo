...
const (
	...
	// HostUploMuxSubscriberName is the name used by the host to register a
	// listener on the UploMux.
	HostUploMuxSubscriberName = "host"
...
const (
	...
	// support 6 month contracts when Uplo leaves beta.
...
var (
	...
	// 4096-byte messages.
	DefaultBaseRPCPrice = types.UplocoinPrecision.Mul64(100).Div64(1e9) // 100 nS
	...
	// keep the data even if the price of Uplocoin fluctuates, the price of raw
	// storage fluctuates, or the host realizes that there is unexpected
	// opportunity cost in being a host.
	DefaultCollateral = types.UplocoinPrecision.Mul64(100).Div(BlockBytesPerMonthTerabyte) // 100 UPLO/ TB / Month
	...
	// relatively small file contract, but millions of Uplocoins could be locked
	...
	// bit.
	DefaultMaxCollateral = types.UplocoinPrecision.Mul64(5e3)
	...
	// transaction.
	DefaultContractPrice = types.UplocoinPrecision.Div64(100).Div64(1e3).Mul64(EstimatedFileContractRevisionAndProofTransactionSetSize)

	// DefaultDownloadBandwidthPrice defines the default price of upload
	// bandwidth. The default is set to 10 Uplocoins per gigabyte, because
	// download bandwidth is expected to be plentiful but also in-demand.
	DefaultDownloadBandwidthPrice = types.UplocoinPrecision.Mul64(25).Div(BytesPerTerabyte) // 25 UPLO/ TB
	...
	DefaultMaxEphemeralAccountBalance = types.UplocoinPrecision
	...
	DefaultSectorAccessPrice = types.UplocoinPrecision.Mul64(2).Div64(1e6) // 2 uS
	...
	DefaultStoragePrice = types.UplocoinPrecision.Mul64(50).Div(BlockBytesPerMonthTerabyte) // 50 UPLO/ TB / Month

	// DefaultUploadBandwidthPrice defines the default price of upload
	// bandwidth. The default is set to 1 Uplocoin per GB, because the host is
	...
	// data.
	DefaultUploadBandwidthPrice = types.UplocoinPrecision.Mul64(1).Div(BytesPerTerabyte) // 1 UPLO/ TB
	...
	CompatV1412DefaultMaxEphemeralAccountBalance = types.UplocoinPrecision
)
...
type (
	...
	StorageObligation struct {
		...
		ValidProofOutputs  []types.UplocoinOutput `json:"validproofoutputs"`
		MissedProofOutputs []types.UplocoinOutput `json:"missedproofoutputs"`
	}
...
// fields set. NetAddress, RemainingStorage, TotalStorage, UnlockHash, RevisionNumber and UploMuxPort are not set.
