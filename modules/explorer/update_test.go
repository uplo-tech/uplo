...
// TestIntegrationExplorerFileContractMetrics checks that the Uplocoin
...
func TestIntegrationExplorerFileContractMetrics(t *testing.T) {
	...
	builder.FundUplocoins(types.NewCurrency64(5e9))
	fcOutputs := []types.UplocoinOutput{{Value: types.NewCurrency64(4805e6)}}
	...
	builder.FundUplocoins(types.NewCurrency64(1e9))
	fcOutputs = []types.UplocoinOutput{{Value: types.NewCurrency64(961e6)}}
	fc = types.FileContract{
		FileSize:           15e3,
		WindowStart:        et.cs.Height() + 2,
		WindowEnd:          et.cs.Height() + 3,
