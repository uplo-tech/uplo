	...
	err := e.db.Update(func(tx *bolt.Tx) (err error) {
		...
		for _, block := range cc.RevertedBlocks {
			...
			for j, payout := range block.MinerPayouts {
				...
				dbRemoveUplocoinOutputID(tx, scoid, tbid)
			...
			for _, txn := range block.Transactions {
				...
				for _, sci := range txn.UplocoinInputs {
					dbRemoveUplocoinOutputID(tx, sci.ParentID, txid)
					dbRemoveUnlockHash(tx, sci.UnlockConditions.UnlockHash(), txid)
				}
				for k, sco := range txn.UplocoinOutputs {
					scoid := txn.UplocoinOutputID(uint64(k))
					dbRemoveUplocoinOutputID(tx, scoid, txid)
					dbRemoveUnlockHash(tx, sco.UnlockHash, txid)
					dbRemoveUplocoinOutput(tx, scoid)
				}
				...
				for _, sfi := range txn.UplofundInputs {
					dbRemoveUplofundOutputID(tx, sfi.ParentID, txid)
					dbRemoveUnlockHash(tx, sfi.UnlockConditions.UnlockHash(), txid)
					dbRemoveUnlockHash(tx, sfi.ClaimUnlockHash, txid)
				}
				for k, sfo := range txn.UplofundOutputs {
					sfoid := txn.UplofundOutputID(uint64(k))
					dbRemoveUplofundOutputID(tx, sfoid, txid)
		...
		for _, block := range cc.AppliedBlocks {
			...
			for j, payout := range block.MinerPayouts {
				scoid := block.MinerPayoutID(uint64(j))
				dbAddUplocoinOutputID(tx, scoid, tbid)
			...
			for _, txn := range block.Transactions {
				...
				for _, sci := range txn.UplocoinInputs {
					dbAddUplocoinOutputID(tx, sci.ParentID, txid)
					dbAddUnlockHash(tx, sci.UnlockConditions.UnlockHash(), txid)
				}
				for j, sco := range txn.UplocoinOutputs {
					scoid := txn.UplocoinOutputID(uint64(j))
					dbAddUplocoinOutputID(tx, scoid, txid)
		...
		// Update stats according to UplocoinOutputDiffs
		for _, scod := range cc.UplocoinOutputDiffs {
			if scod.Direction == modules.DiffApply {
				dbAddUplocoinOutput(tx, scod.ID, scod.UplocoinOutput)
			}
		}

		// Update stats according to UplofundOutputDiffs
		for _, sfod := range cc.UplofundOutputDiffs {
			if sfod.Direction == modules.DiffApply {
				dbAddUplofundOutput(tx, sfod.ID, sfod.UplofundOutput)
			}
		}
...
// Add/Remove Uplocoin output
func dbAddUplocoinOutput(tx *bolt.Tx, id types.UplocoinOutputID, output types.UplocoinOutput) {
	mustPut(tx.Bucket(bucketUplocoinOutputs), id, output)
}
func dbRemoveUplocoinOutput(tx *bolt.Tx, id types.UplocoinOutputID) {
	mustDelete(tx.Bucket(bucketUplocoinOutputs), id)
}

// Add/Remove txid from Uplocoin output ID bucket
func dbAddUplocoinOutputID(tx *bolt.Tx, id types.UplocoinOutputID, txid types.TransactionID) {
	b, err := tx.Bucket(bucketUplocoinOutputIDs).CreateBucketIfNotExists(encoding.Marshal(id))
	assertNil(err)
	mustPutSet(b, txid)
}
func dbRemoveUplocoinOutputID(tx *bolt.Tx, id types.UplocoinOutputID, txid types.TransactionID) {
	bucket := tx.Bucket(bucketUplocoinOutputIDs).Bucket(encoding.Marshal(id))
	mustDelete(bucket, txid)
	if bucketIsEmpty(bucket) {
		tx.Bucket(bucketUplocoinOutputIDs).DeleteBucket(encoding.Marshal(id))
	}
}

// Add/Remove uplofund output
func dbAddUplofundOutput(tx *bolt.Tx, id types.UplofundOutputID, output types.UplofundOutput) {
	mustPut(tx.Bucket(bucketUplofundOutputs), id, output)
}

// Add/Remove txid from uplofund output ID bucket
func dbAddUplofundOutputID(tx *bolt.Tx, id types.UplofundOutputID, txid types.TransactionID) {
	b, err := tx.Bucket(bucketUplofundOutputIDs).CreateBucketIfNotExists(encoding.Marshal(id))
	assertNil(err)
	mustPutSet(b, txid)
}
func dbRemoveUplofundOutputID(tx *bolt.Tx, id types.UplofundOutputID, txid types.TransactionID) {
	bucket := tx.Bucket(bucketUplofundOutputIDs).Bucket(encoding.Marshal(id))
	mustDelete(bucket, txid)
	if bucketIsEmpty(bucket) {
		tx.Bucket(bucketUplofundOutputIDs).DeleteBucket(encoding.Marshal(id))
	}
}
...
func dbCalculateBlockFacts(tx *bolt.Tx, cs modules.ConsensusSet, block types.Block) blockFacts {
	...
	bf.TotalCoins = types.CalculateNumUplocoins(bf.Height)
	...
	for _, txn := range block.Transactions {
		bf.UplocoinInputCount += uint64(len(txn.UplocoinInputs))
		bf.UplocoinOutputCount += uint64(len(txn.UplocoinOutputs))
		...
		bf.UplofundInputCount += uint64(len(txn.UplofundInputs))
		bf.UplofundOutputCount += uint64(len(txn.UplofundOutputs))
...
func dbAddGenesisBlock(tx *bolt.Tx) {
	...
	for _, transaction := range types.GenesisBlock.Transactions {
		...
		// Add Genesis Uplocoin outputs to database
		for i, sco := range transaction.UplocoinOutputs {
			scoid := transaction.UplocoinOutputID(uint64(i))
			dbAddUplocoinOutputID(tx, scoid, txid)
			dbAddUnlockHash(tx, sco.UnlockHash, txid)
			dbAddUplocoinOutput(tx, scoid, sco)
		}

		// Add Geesis Uplofund outputs to database
		for i, sfo := range transaction.UplofundOutputs {
			sfoid := transaction.UplofundOutputID(uint64(i))
			dbAddUplofundOutputID(tx, sfoid, txid)
			dbAddUnlockHash(tx, sfo.UnlockHash, txid)
			dbAddUplofundOutput(tx, sfoid, sfo)
		}
	}
	...
	dbAddBlockFacts(tx, blockFacts{
		BlockFacts: modules.BlockFacts{
			...
			UplocoinOutputCount: uint64(len(types.GenesisUplocoinAllocation)),
			UplofundOutputCount: uint64(len(types.GenesisUplofundAllocation)),
		},
