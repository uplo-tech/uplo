...
func computeConsensusChangeDiffs(pb *processedBlock, apply bool) modules.ConsensusChangeDiffs {
	if apply {
		return modules.ConsensusChangeDiffs{
			UplocoinOutputDiffs:        pb.UplocoinOutputDiffs,
			FileContractDiffs:         pb.FileContractDiffs,
			UplofundOutputDiffs:        pb.UplofundOutputDiffs,
			DelayedUplocoinOutputDiffs: pb.DelayedUplocoinOutputDiffs,
			UplofundPoolDiffs:          pb.UplofundPoolDiffs,
		}
	}
	...
	cd := modules.ConsensusChangeDiffs{
		UplocoinOutputDiffs:        make([]modules.UplocoinOutputDiff, len(pb.UplocoinOutputDiffs)),
		FileContractDiffs:         make([]modules.FileContractDiff, len(pb.FileContractDiffs)),
		UplofundOutputDiffs:        make([]modules.UplofundOutputDiff, len(pb.UplofundOutputDiffs)),
		DelayedUplocoinOutputDiffs: make([]modules.DelayedUplocoinOutputDiff, len(pb.DelayedUplocoinOutputDiffs)),
		UplofundPoolDiffs:          make([]modules.UplofundPoolDiff, len(pb.UplofundPoolDiffs)),
	}
	for i, d := range pb.UplocoinOutputDiffs {
		d.Direction = !d.Direction
		cd.UplocoinOutputDiffs[len(cd.UplocoinOutputDiffs)-i-1] = d
	}
	for i, d := range pb.FileContractDiffs {
		d.Direction = !d.Direction
		cd.FileContractDiffs[len(cd.FileContractDiffs)-i-1] = d
	}
	for i, d := range pb.UplofundOutputDiffs {
		d.Direction = !d.Direction
		cd.UplofundOutputDiffs[len(cd.UplofundOutputDiffs)-i-1] = d
	}
	for i, d := range pb.DelayedUplocoinOutputDiffs {
		d.Direction = !d.Direction
		cd.DelayedUplocoinOutputDiffs[len(cd.DelayedUplocoinOutputDiffs)-i-1] = d
	}
	for i, d := range pb.UplofundPoolDiffs {
		d.Direction = !d.Direction
		cd.UplofundPoolDiffs[len(cd.UplofundPoolDiffs)-i-1] = d
	}
	return cd
}
