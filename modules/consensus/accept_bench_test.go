...
func BenchmarkAcceptEmptyBlocks(b *testing.B) {
	...
	cs, errChan := New(g, false, filepath.Join(testdir, modules.ConsensusDir))
...
func BenchmarkAcceptSmallBlocks(b *testing.B) {
	...
	b.StopTimer()
	for j := 0; j < b.N; j++ {
		// Create a transaction with a miner fee, a normal Uplocoin output, and
		// a funded file contract.
		txnBuilder, err := cst.wallet.StartTransaction()
		if err != nil {
			b.Fatal(err)
		}
		err = txnBuilder.FundUplocoins(types.NewCurrency64(125e6))
		if err != nil {
			b.Fatal(err)
		}
		// Add a small miner fee.
		txnBuilder.AddMinerFee(types.NewCurrency64(5e6))
		// Add a Uplocoin output.
		txnBuilder.AddUplocoinOutput(types.UplocoinOutput{Value: types.NewCurrency64(20e6)})
		// Add a file contract.
