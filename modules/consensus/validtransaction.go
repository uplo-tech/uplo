...
var (
	...
	errMissingUplocoinOutput       = errors.New("transaction spends a nonexisting Uplocoin output")
	errUplocoinInputOutputMismatch = errors.New("Uplocoin inputs do not equal Uplocoin outputs for transaction")
	errUplofundInputOutputMismatch = errors.New("uplofund inputs do not equal uplofund outputs for transaction")
...
// validUplocoins checks that the Uplocoin inputs and outputs are valid in the
// context of the current consensus set.
func validUplocoins(tx *bolt.Tx, t types.Transaction) error {
	...
	for _, sci := range t.UplocoinInputs {
		...
		if scoBytes == nil {
			return errMissingUplocoinOutput
		}
	...
	if !inputSum.Equals(t.UplocoinOutputSum()) {
		return errUplocoinInputOutputMismatch
	}
	return nil
}
...
// validUplofunds checks that the uplofund portions of the transaction are valid
// in the context of the consensus set.
func validUplofunds(tx *bolt.Tx, t types.Transaction) (err error) {
	// Compare the number of input uplofunds to the output uplofunds.
	var uplofundInputSum types.Currency
	var uplofundOutputSum types.Currency
	for _, sfi := range t.UplofundInputs {
		...
		uplofundInputSum = uplofundInputSum.Add(sfo.Value)
	}
	for _, sfo := range t.UplofundOutputs {
		uplofundOutputSum = uplofundOutputSum.Add(sfo.Value)
	}
	if !uplofundOutputSum.Equals(uplofundInputSum) {
		return errUplofundInputOutputMismatch
	}
	return
}
...
// covers a UplocoinInput controlled by the primary or failsafe UnlockHash. To
...
func (cs *ConsensusSet) tryTransactionSet(txns []types.Transaction) (modules.ConsensusChange, error) {
	...
	cc := modules.ConsensusChange{
		ConsensusChangeDiffs: modules.ConsensusChangeDiffs{
			UplocoinOutputDiffs:        diffHolder.UplocoinOutputDiffs,
			FileContractDiffs:         diffHolder.FileContractDiffs,
			UplofundOutputDiffs:        diffHolder.UplofundOutputDiffs,
			DelayedUplocoinOutputDiffs: diffHolder.DelayedUplocoinOutputDiffs,
			UplofundPoolDiffs:          diffHolder.UplofundPoolDiffs,
		},
	}
	return cc, nil
}
