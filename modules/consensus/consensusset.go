...
func consensusSetBlockingStartup(gateway modules.Gateway, persistDir string, deps modules.Dependencies) (*ConsensusSet, error) {
	...
	for _, transaction := range types.GenesisBlock.Transactions {
		// Create the diffs for the genesis Uplocoin outputs.
		for i, UplocoinOutput := range transaction.UplocoinOutputs {
			scid := transaction.UplocoinOutputID(uint64(i))
			scod := modules.UplocoinOutputDiff{
				Direction:     modules.DiffApply,
				ID:            scid,
				UplocoinOutput: UplocoinOutput,
			}
			cs.blockRoot.UplocoinOutputDiffs = append(cs.blockRoot.UplocoinOutputDiffs, scod)
		}
		// Create the diffs for the genesis uplofund outputs.
		for i, uplofundOutput := range transaction.UplofundOutputs {
			sfid := transaction.UplofundOutputID(uint64(i))
			sfod := modules.UplofundOutputDiff{
				Direction:     modules.DiffApply,
				ID:            sfid,
				UplofundOutput: uplofundOutput,
			}
			cs.blockRoot.UplofundOutputDiffs = append(cs.blockRoot.UplofundOutputDiffs, sfod)
		}
	}
