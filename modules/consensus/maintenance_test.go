...
func TestApplyMinerPayouts(t *testing.T) {
	...
	pb.Block.MinerPayouts = append(pb.Block.MinerPayouts, types.UplocoinOutput{Value: types.NewCurrency64(12)})
	...
	exists := cst.cs.db.inDelayedUplocoinOutputsHeight(cst.cs.dbBlockHeight()+types.MaturityDelay, mpid0)
	if !exists {
	...
	exists = cst.cs.db.inUplocoinOutputs(mpid0)
	if exists {
		t.Error("miner payout was added to the Uplocoin output set")
	}
	if cst.cs.db.lenDelayedUplocoinOutputsHeight(cst.cs.dbBlockHeight()+types.MaturityDelay) != 2 { // 1 for consensus set creation, 1 for the output that just got added.
		t.Error("wrong number of delayed Uplocoin outputs in consensus set")
	}
	if len(pb.DelayedUplocoinOutputDiffs) != 1 {
		t.Fatal("block node did not get the delayed Uplocoin output diff")
	}
	if pb.DelayedUplocoinOutputDiffs[0].Direction != modules.DiffApply {
		t.Error("delayed Uplocoin output diff has the wrong direction")
	}
	if pb.DelayedUplocoinOutputDiffs[0].ID != mpid0 {
		t.Error("delayed Uplocoin output diff has wrong id")
	}
	...
	pb2.Block.MinerPayouts = []types.UplocoinOutput{
	...
	exists = cst.cs.db.inDelayedUplocoinOutputsHeight(cst.cs.dbBlockHeight()+types.MaturityDelay, mpid1)
	if !exists {
		t.Error("delayed Uplocoin output was not created")
	}
	exists = cst.cs.db.inDelayedUplocoinOutputsHeight(cst.cs.dbBlockHeight()+types.MaturityDelay, mpid2)
	if !exists {
		t.Error("delayed Uplocoin output was not created")
	}
	if len(pb2.DelayedUplocoinOutputDiffs) != 2 {
	...
	defer func() {
		...
		cst.cs.db.rmDelayedUplocoinOutputsHeight(pb.Height+types.MaturityDelay, mpid0)
		cst.cs.db.addUplocoinOutputs(mpid0, types.UplocoinOutput{})
		_ = cst.cs.db.Update(func(tx *bolt.Tx) error {
			applyMinerPayouts(tx, pb)
			return nil
		})
...
// TestApplyMaturedUplocoinOutputs probes the applyMaturedUplocoinOutputs method
// of the consensus set.
func TestApplyMaturedUplocoinOutputs(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	cst, err := createConsensusSetTester(t.Name())
	...
	cst.cs.db.addUplocoinOutputs(types.UplocoinOutputID{}, types.UplocoinOutput{})
	_ = cst.cs.db.Update(func(tx *bolt.Tx) error {
		createDSCOBucket(tx, pb.Height)
		return nil
	})
	cst.cs.db.addDelayedUplocoinOutputsHeight(pb.Height, types.UplocoinOutputID{}, types.UplocoinOutput{})
	_ = cst.cs.db.Update(func(tx *bolt.Tx) error {
...
func TestApplyMissedStorageProof(t *testing.T) {
	...
	expiringFC := types.FileContract{
		...
		WindowEnd:          pb.Height,
		MissedProofOutputs: []types.UplocoinOutput{{Value: types.NewCurrency64(290e3)}},
	}
	...
	exists = cst.cs.db.inDelayedUplocoinOutputsHeight(pb.Height+types.MaturityDelay, spoid)
	if !exists {
		t.Error("missed proof output was never created")
	}
	exists = cst.cs.db.inUplocoinOutputs(spoid)
	if exists {
		t.Error("storage proof output made it into the Uplocoin output set")
	}
	exists = cst.cs.db.inFileContracts(types.FileContractID{})
	if exists {
	...
	defer func() {
		...
		// Trigger errPayoutsAlreadyPaid from Uplocoin outputs.
		cst.cs.db.rmDelayedUplocoinOutputsHeight(pb.Height+types.MaturityDelay, spoid)
		cst.cs.db.addUplocoinOutputs(spoid, types.UplocoinOutput{})
		cst.cs.applyMissedStorageProof(pb, types.FileContractID{})
	}()
	// Trigger errPayoutsAlreadyPaid from delayed outputs.
	cst.cs.db.rmFileContracts(types.FileContractID{})
	cst.cs.db.addFileContracts(types.FileContractID{}, expiringFC)
	cst.cs.db.addDelayedUplocoinOutputsHeight(pb.Height+types.MaturityDelay, spoid, types.UplocoinOutput{})
	cst.cs.applyMissedStorageProof(pb, types.FileContractID{})
}
*/
...
func TestApplyFoundationSubsidy(t *testing.T) {
	...
	apply := func(height types.BlockHeight) (dscod modules.DelayedUplocoinOutputDiff, created bool) {
		err := cst.cs.db.Update(func(tx *bolt.Tx) error {
			pb := &processedBlock{
				Height: height,
			}
			...
			if len(pb.DelayedUplocoinOutputDiffs) > 0 {
				dscod = pb.DelayedUplocoinOutputDiffs[0]
				created = true
			}
			return nil
		})
	...
	} else if !dscod.UplocoinOutput.Value.Equals(types.InitialFoundationSubsidy) {
		t.Error("output for hardfork block should equal InitialFoundationSubsidy")
	} else if dscod.UplocoinOutput.UnlockHash != types.InitialFoundationUnlockHash {
	...
	} else if !dscod.UplocoinOutput.Value.Equals(types.FoundationSubsidyPerBlock.Mul64(uint64(types.FoundationSubsidyFrequency))) {
		t.Error("output for first subsidy block should equal FoundationSubsidyPerBlock*FoundationSubsidyFrequency")
	} else if dscod.UplocoinOutput.UnlockHash != newPrimary {
