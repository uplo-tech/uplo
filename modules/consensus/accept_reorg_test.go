...
// TestIntegrationUplocoinReorg tries to reorganize a Uplocoin output block out
// of, and then back into, the consensus set.
func TestIntegrationUplocoinReorg(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Give a Uplocoin block to cstMain.
	rs.cstMain.testSpendUplocoinsBlock()
...
func TestBuriedBadFork(t *testing.T) {
	...
	badBlock := types.Block{
		...
		MinerPayouts: []types.UplocoinOutput{{Value: types.CalculateCoinbase(pb.Height)}},
		Transactions: []types.Transaction{{
			UplocoinInputs: []types.UplocoinInput{{}}, // Will trigger an error on full verification but not partial verification.
		}},
	}
