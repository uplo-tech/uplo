...
func TestIntegrationDoSBlockHandling(t *testing.T) {
	...
	// transaction. The transaction has more Uplocoin inputs than outputs.
	txnBuilder, err := cst.wallet.StartTransaction()
	if err != nil {
		t.Fatal(err)
	}
	err = txnBuilder.FundUplocoins(types.NewCurrency64(50))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !errors.Contains(err, errUplocoinInputOutputMismatch) {
		t.Fatalf("expected %v, got %v", errUplocoinInputOutputMismatch, err)
	}
...
func TestMinerPayoutHandling(t *testing.T) {
	...
	block.MinerPayouts = append(block.MinerPayouts, types.UplocoinOutput{Value: types.NewCurrency64(1)})
...
func TestBuriedBadTransaction(t *testing.T) {
	...
	txnBuilder.AddUplocoinOutput(types.UplocoinOutput{Value: txnValue})
	...
	block := types.Block{
		...
		MinerPayouts: []types.UplocoinOutput{{Value: types.CalculateCoinbase(pb.Height + 1)}},
		Transactions: txns,
	}
...
func TestInconsistentCheck(t *testing.T) {
	...
	// Corrupt the consensus set by adding a new uplofund output.
	sfo := types.UplofundOutput{
		Value: types.NewCurrency64(1),
	}
	cst.cs.dbAddUplofundOutput(types.UplofundOutputID{}, sfo)
...
func TestTaxHardfork(t *testing.T) {
	...
	fc := types.FileContract{
		...
		Payout:             payout,
		ValidProofOutputs:  []types.UplocoinOutput{{Value: outputSize}},
		MissedProofOutputs: []types.UplocoinOutput{{Value: outputSize}},
	...
	// Check that the uplofund pool was increased by the faulty float amount.
	uplofundPool := cst.cs.dbGetUplofundPool()
	if !uplofundPool.Equals64(15590e3) {
		t.Fatal("uplofund pool was not increased correctly")
	}
	...
	// Check that the uplofund pool did not change after the submitted revision.
	uplofundPool = cst.cs.dbGetUplofundPool()
	if !uplofundPool.Equals64(15590e3) {
		t.Fatal("uplofund pool was not increased correctly")
	}
...
func TestChainedAcceptBlock(t *testing.T) {
	...
	badBlock.Transactions = append(badBlock.Transactions, types.Transaction{
		UplocoinInputs: []types.UplocoinInput{{
			ParentID: types.UplocoinOutputID{1},
		}},
	})
...
func TestFoundationUpdateBlocks(t *testing.T) {
	...
	foundationOutput := func(height types.BlockHeight) (id types.UplocoinOutputID, sco types.UplocoinOutput, exists bool) {
	...
	// Also send 1 UPLOto the primary and failsafe addresses for later use.
	scoid, sco, ok = foundationOutput(outputHeight)
	if !ok {
	...
	txn = types.Transaction{
		...
		UplocoinOutputs: []types.UplocoinOutput{
			{Value: sco.Value.Sub(types.UplocoinPrecision.Mul64(2)), UnlockHash: types.UnlockHash{}},
			{Value: types.UplocoinPrecision, UnlockHash: types.InitialFoundationUnlockHash},
			{Value: types.UplocoinPrecision, UnlockHash: types.InitialFoundationFailsafeUnlockHash},
		},
	...
	primarySCOID := txn.UplocoinOutputID(1)
	failsafeSCOID := txn.UplocoinOutputID(2)
	...
	txn.UplocoinInputs[0] = types.UplocoinInput{
