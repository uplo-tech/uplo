...
func (pb *ProgramBuilder) AddUpdateRegistryInstruction(spk types.UploPublicKey, rv SignedRegistryValue) error {
	// Marshal pubKey.
...
// program as done in host version 1.0.1 and below.
func (pb *ProgramBuilder) V154AddUpdateRegistryInstruction(spk types.UploPublicKey, rv SignedRegistryValue) error {
	// Marshal pubKey.
	pk := encoding.Marshal(spk)
	// Compute the argument offsets.
	tweakOff := uint64(pb.programData.Len())
...
func (pb *ProgramBuilder) V154AddReadRegistryInstruction(spk types.UploPublicKey, tweak crypto.Hash) (types.Currency, error) {
	// Marshal pubKey.
	pk := encoding.Marshal(spk)
	// Compute the argument offsets.
...
func (pb *ProgramBuilder) AddReadRegistryInstruction(spk types.UploPublicKey, tweak crypto.Hash) (types.Currency, error) {
	// Marshal pubKey.
	pk := encoding.Marshal(spk)
	// Compute the argument offsets.
	pubKeyOff := uint64(pb.programData.Len())
