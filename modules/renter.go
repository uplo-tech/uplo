...
type WorkerPool interface {
	Worker(types.UploPublicKey) (Worker, error)
}
...
var (
	...
	DefaultAllowance = Allowance{
		Funds:       types.UplocoinPrecision.Mul64(2500),
...
type RenterStats struct {
	...
	TotalUplofiles uint64 `json:"totaluplofiles"`
	Totaluplodirs  uint64 `json:"totaluplodirs"`
	TotalSize     uint64 `json:"totalsize"`
...
type (
	...
	PartialChunk struct {
		...
		InPartialsFile bool            // 'true' if the combined chunk is already in the partials uplofile.
...
// DirectoryInfo provides information about a uplodir
type DirectoryInfo struct {
	...
	DirMode             os.FileMode `json:"mode,uplomismatch"` // Field is called DirMode for fuse compatibility
	...
	DirSize             uint64      `json:"size,uplomismatch"` // Stays as 'size' in json for compatibility
...
func (d DirectoryInfo) Name() string { return d.UploPath.Name() }
...
type DownloadInfo struct {
	...
	UploPath         UploPath `json:"uplopath"`         // The uplopath of the file used for the download.
...
type FileInfo struct {
	...
	ModificationTime time.Time         `json:"modtime,uplomismatch"` // Stays as 'modtime' in json for compatibility
	FileMode         os.FileMode       `json:"mode,uplomismatch"`    // Field is called FileMode for fuse compatibility
	NumStuckChunks   uint64            `json:"numstuckchunks"`
...
// Name implements os.FileInfo.
func (f FileInfo) Name() string { return f.UploPath.Name() }
...
type HostScoreBreakdown struct {
	...
	PriceAdjustment            float64 `json:"pricesmultiplier,uplomismatch"`
...
type RecoverableContract struct {
	...
	// InputParentID is the ParentID of the first UplocoinInput of the
	// transaction that contains this contract.
	InputParentID types.UplocoinOutputID `json:"inputparentid"`
...
type RenterContract struct {
	...
	//
	// UplofundFee is the amount of money spent on uplofund fees when creating the
	// contract. The uplofund fee that the renter pays covers both the renter and
...
type ContractorSpending struct {
	...
	// includes the ContractFee, TxnFee and UplofundFee
	...
	ContractSpendingDeprecated types.Currency `json:"contractspending,uplomismatch"`
...
type UploadedBackup struct {
	...
	Size           uint64 // size of snapshot .uplo file
...
type (
	...
	WorkerStatus struct {
		...
		HostPubKey      types.UploPublicKey   `json:"hostpubkey"`
...
type Renter interface {
	...
	// existing file, a suffix of the form _[num] is appended to the uplopath.
	// [num] is incremented until a uplopath is found that is not already in
	// use.
	...
	ContractUtility(pk types.UploPublicKey) (ContractUtility, bool)
	...
	Mount(mountPoint string, sp UploPath, opts MountOptions) error
	...
	SetFileStuck(uploPath UploPath, stuck bool) error
	...
	UploadedBackups() ([]UploadedBackup, []types.UploPublicKey, error)

	// BackupsOnHost returns the backups stored on the specified host.
	BackupsOnHost(hostKey types.UploPublicKey) ([]UploadedBackup, error)

	// DeleteFile deletes a file entry from the renter.
	DeleteFile(uploPath UploPath) error
	...
	File(uploPath UploPath) (FileInfo, error)
	...
	FileList(uploPath UploPath, recursive, cached bool, flf FileListFunc) error

	// Filter returns the renter's hostdb's filterMode and filteredHosts
	Filter() (FilterMode, map[string]types.UploPublicKey, error)

	// SetFilterMode sets the renter's hostdb filter mode
	SetFilterMode(fm FilterMode, hosts []types.UploPublicKey) error

	// Host provides the DB entry and score breakdown for the requested host.
	Host(pk types.UploPublicKey) (HostDBEntry, bool, error)

	// InitialScanComplete returns a boolean indicating if the initial scan of the
	// hostdb is completed.
	InitialScanComplete() (bool, error)

	// PriceEstimation estimates the cost in Uplocoins of performing various
	...
	RenameFile(uploPath, newUploPath UploPath) error

	// RenameDir changes the path of a dir.
	RenameDir(oldPath, newPath UploPath) error
	...
	// used.
	ReadRegistry(spk types.UploPublicKey, tweak crypto.Hash, timeout time.Duration) (SignedRegistryValue, error)
	...
	SetFileTrackingPath(uploPath UploPath, newPath string) error

	// UpdateRegistry updates the registries on all workers with the given
	// registry value.
	UpdateRegistry(spk types.UploPublicKey, srv SignedRegistryValue, timeout time.Duration) error
	...
	// from the Uplo network and also returns the fileName of the streamed
	// resource.
	Streamer(uplopath UploPath, disableLocalFetch bool) (string, Streamer, error)
	...
	CreateDir(uploPath UploPath, mode os.FileMode) error

	// DeleteDir deletes a directory from the renter
	DeleteDir(uploPath UploPath) error

	// DirList lists the directories in a uplodir
	DirList(uploPath UploPath) ([]DirectoryInfo, error)
	...
	// CreateSkylinkFromUplofile will create a skylink from a uplofile. This will
	...
	// separately as well.
	CreateSkylinkFromUplofile(SkyfileUploadParameters, UploPath) (Skylink, error)
	...
	// DownloadSkylink will fetch a file from the Uplo network using the skylink.
	...
	// UploadSkyfile will upload data to the Uplo network from a reader and
...
// allows for streaming files uploaded to the Uplo network.
...
type HostDB interface {
	...
	// SetFilterMode sets the renter's hostdb filter mode
	SetFilterMode(lm FilterMode, hosts []types.UploPublicKey) error
	...
	IncrementSuccessfulInteractions(types.UploPublicKey) error

	// IncrementFailedInteractions increments the number of failed interactions with
	// a host for a given key
	IncrementFailedInteractions(types.UploPublicKey) error
	...
	RandomHosts(int, []types.UploPublicKey, []types.UploPublicKey) ([]HostDBEntry, error)
	...
	// renter.
	RandomHostsWithAllowance(int, []types.UploPublicKey, []types.UploPublicKey, Allowance) ([]HostDBEntry, error)
