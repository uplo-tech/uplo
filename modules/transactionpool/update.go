...
func findSets(ts []types.Transaction) [][]types.Transaction {
	...
	for i, t := range ts {
		...
		for _, obj := range t.UplocoinInputs {
		...
		for _, obj := range t.UplofundInputs {
			txid, exists := objMap[ObjectID(obj.ParentID)]
			if exists {
				parentSet := forward(txMap[txid])
				parentSets[parentSet] = struct{}{}
		...
		if len(parentSets) == 0 {
			...
			// proof outputs, uplofund claim outputs; these outputs are not
		...
		// transactions.
		for j := range t.UplocoinOutputs {
			scoid := t.UplocoinOutputID(uint64(j))
		...
		for j := range t.UplofundOutputs {
			sfoid := t.UplofundOutputID(uint64(j))
