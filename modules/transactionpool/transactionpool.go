...
type (
	// ObjectID is the ID of an object such as Uplocoin output and file
	...
	TransactionPool struct {
		...
		recentMedianFee types.Currency // UPLOper byte
...
func (tp *TransactionPool) Transaction(id types.TransactionID) (types.Transaction, []types.Transaction, bool) {
	...
	addOutputIDs := func(txn types.Transaction) {
		...
		for _, input := range txn.UplofundInputs {
			parentIDs[types.OutputID(input.ParentID)] = struct{}{}
		}
	...
	isParent := func(t types.Transaction) bool {
		for i := range t.UplocoinOutputs {
			if _, exists := parentIDs[types.OutputID(t.UplocoinOutputID(uint64(i)))]; exists {
				return true
			}
		}
