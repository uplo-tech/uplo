...
func TestAcceptTransactionSet(t *testing.T) {
	...
	txns, err := tpt.wallet.SendUplocoins(types.NewCurrency64(100), types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestConflictingTransactionSets(t *testing.T) {
	...
	err = txnBuilder.FundUplocoins(fund)
	if err != nil {
		t.Fatal(err)
	}
	...
	// other create a Uplocoin output.
	txnIndex := len(txnSet) - 1
	txnSet[txnIndex].MinerFees = append(txnSet[txnIndex].MinerFees, fund)
	txnSetDoubleSpend[txnIndex].UplocoinOutputs = append(txnSetDoubleSpend[txnIndex].UplocoinOutputs, types.UplocoinOutput{Value: fund})
...
func TestCheckMinerFees(t *testing.T) {
	...
	graphFund := types.UplocoinPrecision.Mul64(1000)
	...
	txns, err := tpt.wallet.SendUplocoinsMulti(outputs)
	if err != nil {
		t.Error(err)
	}
	...
	_, err = tpt.wallet.SendUplocoins(types.UplocoinPrecision.Mul64(50), types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < numGraphs; i++ {
		...
		for j := 0; j < graphLens; j++ {
			edges = append(edges, types.TransactionGraphEdge{
				Dest:   j + 1,
				Fee:    types.UplocoinPrecision,
				Source: j,
				Value:  graphFund.Sub(types.UplocoinPrecision.Mul64(uint64(j + 1))),
			})
		}
		graph, err := types.TransactionGraph(finalTxn.UplocoinOutputID(uint64(i)), edges)
		if err != nil {
			t.Fatal(err)
		}
	...
	source := finalTxn.UplocoinOutputID(uint64(numGraphs))
	lowFee := types.UplocoinPrecision.Div64(3)
	remaining := types.UplocoinPrecision.Mul64(1000).Sub(lowFee)
...
func TestTransactionGraph(t *testing.T) {
	...
	txns, err := tpt.wallet.SendUplocoins(types.UplocoinPrecision.Mul64(100), types.UnlockConditions{}.UnlockHash())
	if err != nil {
		t.Fatal(err)
	}

	// Get the output of that transaction.
	graphSourceOutputID := txns[len(txns)-1].UplocoinOutputID(0)
	edge := types.TransactionGraphEdge{
		Dest:   1,
		Fee:    types.UplocoinPrecision.Mul64(10),
		Source: 0,
		Value:  types.UplocoinPrecision.Mul64(90),
	}
...
func TestTransactionGraphDiamond(t *testing.T) {
	...
	for i := range sources {
		edges = append(edges, types.TransactionGraphEdge{
			Dest:   dests[i],
			Fee:    types.UplocoinPrecision.Mul64(fees[i]),
			Source: sources[i],
			Value:  types.UplocoinPrecision.Mul64(values[i]),
		})
	}
...
func TestAcceptFCAndConflictingRevision(t *testing.T) {
	...
	builder.AddFileContract(types.FileContract{
		...
		Payout:             payout,
		ValidProofOutputs:  []types.UplocoinOutput{{Value: types.PostTax(tpt.cs.Height(), payout)}},
		MissedProofOutputs: []types.UplocoinOutput{{Value: types.PostTax(tpt.cs.Height(), payout)}},
		UnlockHash:         types.UnlockConditions{}.UnlockHash(),
	})
	...
	rSet := []types.Transaction{{
		FileContractRevisions: []types.FileContractRevision{{
			...
			NewValidProofOutputs:  []types.UplocoinOutput{{Value: types.PostTax(tpt.cs.Height(), payout)}},
			NewMissedProofOutputs: []types.UplocoinOutput{{Value: types.PostTax(tpt.cs.Height(), payout)}},
		}},
	}}
...
func TestPartialConfirmationWeave(t *testing.T) {
	...
	err = builder1.FundUplocoins(funding1)
	if err != nil {
		t.Fatal(err)
	}
	scOutput1 := types.UplocoinOutput{
		Value:      funding1,
		UnlockHash: emptyUH,
	}
	i1 := builder1.AddUplocoinOutput(scOutput1)
	...
	err = builder2.FundUplocoins(funding2)
	if err != nil {
		t.Fatal(err)
	}
	scOutput2 := types.UplocoinOutput{
		Value:      funding2,
		UnlockHash: emptyUH,
	}
	i2 := builder2.AddUplocoinOutput(scOutput2)
	...
	txn1 := types.Transaction{
		UplocoinInputs: []types.UplocoinInput{{
			ParentID: tSet1[len(tSet1)-1].UplocoinOutputID(i1),
		}},
	...
	txn2 := types.Transaction{
		UplocoinInputs: []types.UplocoinInput{{
			ParentID: tSet2[len(tSet2)-1].UplocoinOutputID(i2),
		}},
		UplocoinOutputs: []types.UplocoinOutput{{
			Value:      funding2,
			UnlockHash: emptyUH,
	...
	child := types.Transaction{
		UplocoinInputs: []types.UplocoinInput{
			{
				ParentID: txn1.UplocoinOutputID(0),
			},
			{
				ParentID: txn2.UplocoinOutputID(0),
			},
		},
		UplocoinOutputs: []types.UplocoinOutput{{
