...
// TestuplodConfigPersistCompat confirms that the compat code for the writebps
// json tag update performs as expected
func TestuplodConfigPersistCompat(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Create uplodconfig
	testDir := build.TempDir("uplodconfig", t.Name())
...
// saveLoadCheck is a helper to check saving and loading the uplod config file
// and verifying the correct values for the WriteBPS fields
func saveLoadCheck(sc *uplodConfig, writeBPS, writeBPSDeprepacted int64) error {
	// Set both fields
...
func checkWriteBPS(sc *uplodConfig, writeBPS int64) error {
