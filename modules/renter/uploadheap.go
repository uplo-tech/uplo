...
func (uch uploadChunkHeap) Less(i, j int) bool {
	...
	//
	//  4) Remote Chunks
	//    - These are chunks of a uplofile that do not have a local file to repair
	//    from
	//
...
// heap. The index of the chunks in the heap refers to the uplofile index.
...
func (r *Renter) managedBuildUnfinishedChunk(entry *filesystem.FileNode, chunkIndex uint64, hosts map[string]struct{}, hostPublicKeys map[string]types.UploPublicKey, priority bool, offline, goodForRenew map[string]bool, mm *memoryManager) (*unfinishedUploadChunk, error) {
	// Copy entry
	...
	uuc := &unfinishedUploadChunk{
		...
		staticUploPath: entryCopy.UploFilePath(),
...
// file.
//
// NOTE: each unfinishedUploadChunk needs its own UploFileSetEntry. This is due
// to the UploFiles being removed from memory. Since the renter does not keep the
// UploFiles in memory the unfinishedUploadChunks need to close the UploFile when
// they are done and so cannot share a UploFileSetEntry as the first chunk to
...
func (r *Renter) managedBuildUnfinishedChunks(entry *filesystem.FileNode, hosts map[string]struct{}, target repairTarget, offline, goodForRenew map[string]bool, mm *memoryManager) []*unfinishedUploadChunk {
	...
	if workerPoolLen < minPieces {
		...
		if allowance.Hosts < uint64(minPieces) && target == targetUnstuckChunks {
			...
			r.log.Printf("WARN: allownace had insufficient hosts for chunk to reach minimum redundancy, have %v need %v for file %v", allowance.Hosts, minPieces, entry.UploFilePath())
...
func (r *Renter) managedAddChunksToHeap(hosts map[string]struct{}) (*uniqueRefreshPaths, error) {
	uploPaths := r.newUniqueRefreshPaths()
	...
	for r.uploadHeap.managedLen() < maxUploadHeapChunks && r.directoryHeap.managedLen() > 0 {
		select {
		case <-r.tg.StopChan():
			return uploPaths, errors.New("renter shutdown before we could finish adding chunks to heap")
		default:
		}
		...
		if errors.Contains(err, threadgroup.ErrStopped) {
			...
			if consecutiveDirHeapFailures > maxConsecutiveDirHeapFailures {
				...
				return uploPaths, errors.AddContext(err, "too many consecutive dir heap failures")
			}
			continue
		}
		consecutiveDirHeapFailures = 0
		...
		r.managedBuildChunkHeap(dir.staticUploPath, hosts, targetUnstuckChunks, offline, goodForRenew)
		...
		// Since we added chunks from this directory, track the uploPath
		err = uploPaths.callAdd(dir.staticUploPath)
		if err != nil {
			r.repairLog.Println("WARN: error adding uplopath to tracked paths to bubble:", err)
		}
		r.repairLog.Printf("Added %v chunks from %s to the repair heap", chunksAdded, dir.staticUploPath)
	}
...
func (r *Renter) managedBuildAndPushRandomChunk(uploPath modules.UploPath, hosts map[string]struct{}, target repairTarget, mm *memoryManager) error {
	// Open file
	file, err := r.staticFileSystem.OpenUploFile(uploPath)
	if err != nil {
		return err
	}
	...
	if len(unfinishedUploadChunks) == 0 {
		return fmt.Errorf("No stuck chunks built from %v", uploPath)
	}
...
func (r *Renter) callBuildAndPushChunks(files []*filesystem.FileNode, hosts map[string]struct{}, target repairTarget, offline, goodForRenew map[string]bool) {
	...
	// get the directory uplopath
	dirUploPath, err := r.staticFileSystem.FileUploPath(files[0]).Dir()
	if err != nil {
		r.log.Println("WARN: unable to get directory UploPath and add directory back to directory heap:", err)
		return
	}
...
func (r *Renter) managedBuildChunkHeap(dirUploPath modules.UploPath, hosts map[string]struct{}, target repairTarget, offline, goodForRenew map[string]bool) {
	// Get Directory files
	...
	for _, fi := range fileinfos {
		// skip sub directories and non uplofiles
		ext := filepath.Ext(fi.Name())
		if fi.IsDir() || ext != modules.UploFileExtension {
			continue
		}

		// Open UploFile
		uploPath, err := dirUploPath.Join(strings.TrimSuffix(fi.Name(), ext))
		if err != nil {
			r.log.Println("WARN: could not create uploPath:", err)
			continue
		}
		file, err := r.staticFileSystem.OpenUploFile(uploPath)
		if err != nil {
			r.log.Println("WARN: could not open uplofile:", err)
			continue
		}
		...
		if target == targetStuckChunks && file.NumStuckChunks() == 0 {
			...
			err = file.Close()
			if err != nil {
				r.log.Println("WARN: Could not close file:", file.UploFilePath(), err)
			}
			continue
		}
	...
	if len(files) == 0 {
		r.log.Debugln("No files pulled from `", dirUploPath, "` to build the repair heap")
		return
	}
	...
	if len(files) > maxUploadHeapChunks && target == targetUnstuckChunks {
		...
		for i := maxUploadHeapChunks; i < len(files); i++ {
			err = files[i].Close()
			if err != nil {
				r.log.Println("WARN: Could not close file:", files[i].UploFilePath(), err)
			}
		}
...
func (r *Renter) managedRefreshHostsAndWorkers() map[string]struct{} {
	...
	// key is the String() representation of the host's UploPublicKey.
	//
...
func (r *Renter) managedRepairLoop() error {
	...
	for r.uploadHeap.managedLen() >= minUploadHeapSize || smallRepair || time.Now().After(repairBreakTime) {
		...
		chunkPath := nextChunk.staticUploPath
...
func (r *Renter) threadedUploadAndRepair() {
	...
	for {
		...
		// backups is different from the uplofileset that stores non-system files
		// and chunks.
		heapLen := r.uploadHeap.managedLen()
		offline, goodForRenew, _ := r.managedContractUtilityMaps()
		...
		// Add chunks to heap.
		dirUploPaths, err := r.managedAddChunksToHeap(hosts)
		if err != nil {
