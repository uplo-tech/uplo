...
func TestHostTree(t *testing.T) {
	...
	var removed []types.UploPublicKey
...
func TestRandomHosts(t *testing.T) {
	...
	// returned.
	uniqueHosts := tree.SelectRandom(3, []types.UploPublicKey{
	...
	randHosts = tree.SelectRandom(3, []types.UploPublicKey{{}, {}, {}}, nil)
	if len(randHosts) != 3 {
		t.Error("didn't get 3 hosts")
	}
...
func TestHostTreeFilter(t *testing.T) {
	...
	// host.
	if numHosts := len(tree.SelectRandom(2, nil, []types.UploPublicKey{entry2.PublicKey})); numHosts != 1 {
		t.Error("Expected only one host but was", numHosts)
	}

	// Call SelectRandom again but ignore host 3. This should give us no host.
	if numHosts := len(tree.SelectRandom(2, nil, []types.UploPublicKey{entry3.PublicKey})); numHosts != 0 {
