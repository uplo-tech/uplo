...
func (hdb *HostDB) managedScanHost(entry modules.HostDBEntry) {
	...
	err = func() error {
		...
		// Need to apply the custom resolver to the uplomux address.
		uplomuxAddr := settings.UploMuxAddress()
		if hdb.staticDeps.Disrupt("customResolver") {
			port := modules.NetAddress(uplomuxAddr).Port()
			uplomuxAddr = fmt.Sprintf("127.0.0.1:%s", port)
		}

		// Try opening a connection to the uplomux, this is a very lightweight
		// way of checking that RHP3 is supported.
		_, err = fetchPriceTable(hdb.staticMux, uplomuxAddr, timeout, modules.UploPKToMuxPK(entry.PublicKey))
		if err != nil {
			hdb.staticLog.Debugf("%v uplomux ping not successful: %v\n", entry.PublicKey, err)
			return err
		}
		return nil
	}()
...
func fetchPriceTable(uplomux *uplomux.UploMux, hostAddr string, timeout time.Duration, hpk mux.ED25519PublicKey) (_ *modules.RPCPriceTable, err error) {
	stream, err := uplomux.NewEphemeralStream(modules.HostUploMuxSubscriberName, hostAddr, timeout, hpk)
	if err != nil {
