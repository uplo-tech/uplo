...
type (
	...
	download struct {
		...
		staticUploPath         modules.UploPath    // The path of the uplofile at the time the download started.
	...
	downloadParams struct {
		...
		file              *uplofile.Snapshot   // The file to download.
...
func (r *Renter) managedDownload(p modules.RenterDownloadParameters) (_ *download, err error) {
	...
	entry, err := r.staticFileSystem.OpenUploFile(p.UploPath)
	if err != nil {
		return nil, err
	}
	defer func() {
	...
	// Prepare snapshot.
	snap, err := entry.SnapshotRange(p.UploPath, p.Offset, p.Length)
	if err != nil {
		return nil, err
	}
...
func (r *Renter) managedNewDownload(params downloadParams) (*download, error) {
	...
	d := &download{
		...
		staticUploPath:         params.file.UploPath(),
...
func (d *download) Start() error {
	...
	for chunkIndex := minChunk; chunkIndex <= maxChunk; chunkIndex++ {
		...
		for pieceIndex, pieceSet := range pieces {
			for _, piece := range pieceSet {
				...
				if exists {
					d.r.log.Println("ERROR: Worker has multiple pieces uploaded for the same chunk.", params.file.UploPath(), chunkIndex, pieceIndex, piece.HostPubKey.String())
				}
	...
	for i := minChunk; i <= maxChunk; i++ {
		udc := &unfinishedDownloadChunk{
			...
			staticChunkIndex: i,
			staticCacheID:    fmt.Sprintf("%v:%v", d.staticUploPath, i),
