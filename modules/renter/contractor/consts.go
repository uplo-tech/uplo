...
var (
	...
	// allowance is 100 UPLOper contract (5,000 UPLOtotal for 50 contracts, or
	// 2,000 UPLOtotal for 20 contracts, etc.), then the minimum amount of funds
...
// contracts.
var (
	maxCollateral    = types.UplocoinPrecision.Mul64(1e3) // 1k UPLO
	maxDownloadPrice = maxStoragePrice.Mul64(3 * uint64(types.BlocksPerMonth))
	maxStoragePrice  = build.Select(build.Var{
		Dev:      types.UplocoinPrecision.Mul64(300e3).Div(modules.BlockBytesPerMonthTerabyte), // 1 order of magnitude greater
		Standard: types.UplocoinPrecision.Mul64(30e3).Div(modules.BlockBytesPerMonthTerabyte),  // 30k UPLO/ TB / Month
		Testing:  types.UplocoinPrecision.Mul64(3e6).Div(modules.BlockBytesPerMonthTerabyte),   // 2 orders of magnitude greater
	}).(types.Currency)
