...
//                       +
//
func createTestTransactionTree(numRoots int, chainLength int) (txnSet []types.Transaction, roots []types.Transaction, subRootTx types.Transaction, fcTxn types.Transaction, rootParentOutputs map[types.UplocoinOutputID]bool) {
	roots = make([]types.Transaction, 0, numRoots)
	rootParents := make(map[types.UplocoinOutputID]bool)

	// All the root txs create outputs spent in subRootTx.
	subRootTx = types.Transaction{
		UplocoinInputs: make([]types.UplocoinInput, 0, numRoots),
	...
	for i := 0; i < numRoots; i++ {
		nextRootTx := types.Transaction{
			UplocoinInputs: []types.UplocoinInput{
				{
					ParentID: types.UplocoinOutputID(crypto.HashObject(fastrand.Bytes(32))),
				},
			},
			UplocoinOutputs: []types.UplocoinOutput{
				{UnlockHash: types.UnlockHash(crypto.HashObject(fastrand.Bytes(32)))},
		...
		rootParents[nextRootTx.UplocoinInputs[0].ParentID] = true

		subRootTx.UplocoinInputs = append(subRootTx.UplocoinInputs, types.UplocoinInput{
			ParentID: nextRootTx.UplocoinOutputID(0),
		})
	...
	for i := 0; i < chainLength; i++ {
		...
		for i := 0; i < len(txnSet); i++ {
			if len(txnSet[i].UplocoinInputs) > 1 {
	...
	fcTxn = types.Transaction{
		UplocoinInputs: []types.UplocoinInput{{ParentID: txnSet[0].UplocoinOutputID(0)}},
...
func addChildren(txnSet []types.Transaction, numDependencies int, hasOutputs bool) []types.Transaction {
	...
	if !hasOutputs {
		for i := 0; i < numDependencies; i++ {
			txnSet[0].UplocoinOutputs = append(txnSet[0].UplocoinOutputs, types.UplocoinOutput{
				UnlockHash: types.UnlockHash(crypto.HashObject(fastrand.Bytes(32))),
			})
		}
	}
	...
	for i := 0; i < numDependencies; i++ {
		newChild := types.Transaction{
			UplocoinInputs:  make([]types.UplocoinInput, 1),
			UplocoinOutputs: make([]types.UplocoinOutput, 0),
		}

		newChild.UplocoinInputs[0] = types.UplocoinInput{
			ParentID: txnSet[0].UplocoinOutputID(uint64(i)),
		}

		newChild.UplocoinOutputs = append(newChild.UplocoinOutputs, types.UplocoinOutput{
			UnlockHash: types.UnlockHash(crypto.HashObject(fastrand.Bytes(32))),
		})
...
func TestWatchdogRevisionCheck(t *testing.T) {
	...
	a := modules.Allowance{
		Funds:              types.UplocoinPrecision.Mul64(100), // 100 UPLO
		Hosts:              1,
		Period:             50,
		RenewWindow:        10,
...
func TestWatchdogGetParents(t *testing.T) {
	...
	for i := 0; i < 10-1; i++ {
		parentID := txnSet[i].UplocoinInputs[0].ParentID
		expectedID := txnSet[i+1].UplocoinOutputID(0)
	...
	if parentOids[0] != rootTx.UplocoinInputs[0].ParentID {
	...
	for i := 0; i < len(rootTx2.UplocoinInputs); i++ {
		foundParent := false
		for j := 0; j < len(parentOids2); j++ {
			if parentOids2[i] == rootTx2.UplocoinInputs[j].ParentID {
				foundParent = true
			}
		}
		if !foundParent {
	...
	bigRootIDs := make(map[types.UplocoinOutputID]bool)

	// All the root txs create outputs spent in subRootTx.
	subRootTx := types.Transaction{
		UplocoinInputs: make([]types.UplocoinInput, numBigRoots),
	}
	...
	for i := 0; i < numBigRoots; i++ {
		...
		subRootTx.UplocoinInputs[i] = types.UplocoinInput{
			ParentID: nextRootTx.UplocoinOutputID(0),
		}
		bigRoots = append(bigRoots, nextRootTx)
		bigRootIDs[nextRootTx.UplocoinInputs[0].ParentID] = true
	}
