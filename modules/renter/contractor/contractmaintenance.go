...
type (
	...
	fileContractRenewal struct {
		...
		hostPubKey types.UploPublicKey
	}
)
...
func (c *Contractor) managedEstimateRenewFundingRequirements(contract modules.RenterContract, blockHeight types.BlockHeight, allowance modules.Allowance) (types.Currency, error) {
	...
	// Aggregate all estimates so far to compute the estimated uplofunds fees.
	// The transaction fees are not included in the uplofunds estimate because
	// users are not charged uplofund fees on money that doesn't go into the file
	// contract (and the transaction fee goes to the miners, not the file
	// contract).
	beforeUplofundFeesEstimate := maintenanceCost.Add(newUploadsCost).Add(newDownloadsCost).Add(contractPrice)
	afterUplofundFeesEstimate := types.Tax(blockHeight, beforeUplofundFeesEstimate).Add(beforeUplofundFeesEstimate)
	...
	estimatedCost := afterUplofundFeesEstimate.Add(txnFees)
...
func (c *Contractor) managedPrunedRedundantAddressRange() {
	...
	pks := make([]types.UploPublicKey, 0, len(allContracts))
...
// I/O.
func (c *Contractor) managedRenew(id types.FileContractID, hpk types.UploPublicKey, contractFunding types.Currency, newEndHeight types.BlockHeight, hostSettings modules.HostExternalSettings) (modules.RenterContract, error) {
	...
	if c.staticDeps.Disrupt("DefaultRenewSettings") {
		...
		host.HostExternalSettings.UploMuxPort = hostSettings.UploMuxPort
	}
	...
	err = txnBuilder.FundUplocoins(params.Funding)
	if err != nil {
	...
	if c.staticDeps.Disrupt("LegacyRenew") || build.VersionCmp(host.Version, "1.0.1") < 0 {
...
func (c *Contractor) threadedContractMaintenance() {
	...
	allContracts := c.staticContracts.ViewAll()
	c.mu.RLock()
	var blacklist []types.UploPublicKey
	var addressBlacklist []types.UploPublicKey
	for _, contract := range allContracts {
