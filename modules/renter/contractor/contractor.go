...
func (emptyWorkerPool) Worker(_ types.UploPublicKey) (modules.Worker, error) {
...
type Contractor struct {
	...
	maintenanceLock      uplosync.TryMutex
...
func (c *Contractor) ContractPublicKey(pk types.UploPublicKey) (crypto.PublicKey, bool) {
	c.mu.RLock()
...
func (c *Contractor) PeriodSpending() (modules.ContractorSpending, error) {
	...
	for _, contract := range allContracts {
		...
		spending.ContractFees = spending.ContractFees.Add(contract.UplofundFee)
	...
	for _, contract := range c.oldContracts {
		...
		if contract.StartHeight >= c.currentPeriod {
			...
			spending.PreviousSpending = spending.PreviousSpending.Add(contract.ContractFee).Add(contract.TxnFee).
				Add(contract.UplofundFee).Add(contract.DownloadSpending).Add(contract.UploadSpending).Add(contract.StorageSpending)
		} else {
			// Calculate Previous spending
			spending.PreviousSpending = spending.PreviousSpending.Add(contract.ContractFee).Add(contract.TxnFee).
				Add(contract.UplofundFee).Add(contract.DownloadSpending).Add(contract.UploadSpending).Add(contract.StorageSpending)
...
func (c *Contractor) ProvidePayment(stream io.ReadWriter, host types.UploPublicKey, rpc types.Specifier, amount types.Currency, refundAccount modules.AccountID, blockHeight types.BlockHeight) error {
