...
import (
	...
	modWallet "github.com/uplo-tech/uplo/modules/wallet"
...
func newCustomTestingHost(testdir string, cs modules.ConsensusSet, tp modules.TransactionPool, mux *uplomux.UploMux, deps modules.Dependencies) (modules.Host, closeFn, error) {
...
func newTestingHost(testdir string, cs modules.ConsensusSet, tp modules.TransactionPool, mux *uplomux.UploMux) (modules.Host, closeFn, error) {
...
// used for testing host/renter interactions. It allows to pass a custom uplomux.
func newCustomTestingTrio(name string, mux *uplomux.UploMux, hdeps, cdeps modules.Dependencies) (modules.Host, *Contractor, modules.TestMiner, closeFn, error) {
	testdir := build.TempDir("contractor", name)
...
func TestIntegrationFormContract(t *testing.T) {
	...
	_, _, err = c.managedNewContract(hostEntry, types.UplocoinPrecision.Mul64(50), c.blockHeight+100)
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestFormContractSmallAllowance(t *testing.T) {
	...
	// set an allowance but don't use SetAllowance to avoid automatic contract
	// formation. Setting funds to 1UPLOto mimic bug report found in production.
	// Using production number of hosts as well
	c.mu.Lock()
	c.allowance = modules.DefaultAllowance
	c.allowance.Funds = types.UplocoinPrecision.Mul64(1)
...
func TestIntegrationReviseContract(t *testing.T) {
	...
	// form a contract with the host
	_, contract, err := c.managedNewContract(hostEntry, types.UplocoinPrecision.Mul64(50), c.blockHeight+100)
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationRenew(t *testing.T) {
	...
	contract, err = c.managedRenew(contract.ID, contract.HostPublicKey, types.UplocoinPrecision.Mul64(50), c.blockHeight+200, hostSettings)
	if err != nil {
		t.Fatal(err)
	}
	...
	contract, err = c.managedRenew(contract.ID, contract.HostPublicKey, types.UplocoinPrecision.Mul64(50), c.blockHeight+100, hostSettings)
	if err != nil {
		t.Fatal(err)
	}
...
// own this contract. See https://github.com/uplo-tech/uplo/issues/2327.
func TestContractPresenceLeak(t *testing.T) {
	...
	// form a contract with the host
	_, contract, err := c.managedNewContract(hostEntry, types.UplocoinPrecision.Mul64(10), c.blockHeight+100)
	if err != nil {
		t.Fatal(err)
	}
