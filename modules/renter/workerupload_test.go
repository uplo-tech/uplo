...
func TestCheckUploadGouging(t *testing.T) {
	...
	minAllowance := modules.Allowance{
		...
		// One Uplocoin is multiplied by the number of elements that are checked
		// for gouging, and then divided by the gounging denominator.
		Funds:  types.UplocoinPrecision.Mul64(4).Div64(uploadGougingFractionDenom).Sub(oneCurrency),
		Period: 1, // 1 block.
	...
	minHostSettings := modules.HostExternalSettings{
		BaseRPCPrice:         types.UplocoinPrecision,
		SectorAccessPrice:    types.UplocoinPrecision,
		UploadBandwidthPrice: types.UplocoinPrecision.Div64(modules.StreamUploadSize),
		StoragePrice:         types.UplocoinPrecision.Div64(modules.StreamUploadSize),
	}
	...
	maxAllowance.MaxRPCPrice = types.UplocoinPrecision.Add(oneCurrency)
	maxAllowance.MaxContractPrice = oneCurrency
	maxAllowance.MaxDownloadBandwidthPrice = oneCurrency
	maxAllowance.MaxSectorAccessPrice = types.UplocoinPrecision.Add(oneCurrency)
	maxAllowance.MaxStoragePrice = types.UplocoinPrecision.Div64(modules.StreamUploadSize).Add(oneCurrency)
	maxAllowance.MaxUploadBandwidthPrice = types.UplocoinPrecision.Div64(modules.StreamUploadSize).Add(oneCurrency)
	...
	failAllowance.MaxRPCPrice = types.UplocoinPrecision.Sub(oneCurrency)
	...
	failAllowance = maxAllowance
	failAllowance.MaxSectorAccessPrice = types.UplocoinPrecision.Sub(oneCurrency)
	err = checkUploadGouging(failAllowance, minHostSettings)
	if err == nil {
		t.Error("expecting price gouging check to fail")
	}
	...
	failAllowance = maxAllowance
	failAllowance.MaxStoragePrice = types.UplocoinPrecision.Div64(modules.StreamUploadSize).Sub(oneCurrency)
	err = checkUploadGouging(failAllowance, minHostSettings)
	if err == nil {
		t.Error("expecting price gouging check to fail")
	}
	...
	failAllowance = maxAllowance
	failAllowance.MaxUploadBandwidthPrice = types.UplocoinPrecision.Div64(modules.StreamUploadSize).Sub(oneCurrency)
	err = checkUploadGouging(failAllowance, minHostSettings)
	if err == nil {
		t.Error("expecting price gouging check to fail")
	}
