...
// fuseDirnode is a fuse node for the fs package that covers a uplodir.
//
...
// fuseFilenode is a fuse node for the fs package that covers a uplofile.
//
...
func (ffn *fuseFilenode) Flush(ctx context.Context, fh fs.FileHandle) syscall.Errno {
	...
	if err != nil {
		uploPath := ffn.staticFilesystem.renter.staticFileSystem.FileUploPath(ffn.staticFileNode)
		ffn.staticFilesystem.renter.log.Printf("error when flushing fuse file %v: %v", uploPath, err)
		return errToStatus(err)
	}
	return errToStatus(nil)
}
...
func (fdn *fuseDirnode) Lookup(ctx context.Context, name string, out *fuse.EntryOut) (*fs.Inode, syscall.Errno) {
	...
	if fileErr == nil {
		...
		if err != nil {
			uploPath := fdn.staticFilesystem.renter.staticFileSystem.DirUploPath(fdn.staticDirNode)
			fdn.staticFilesystem.renter.log.Printf("Unable to fetch fileinfo on file %v from dir %v: %v", name, uploPath, err)
	...
	if dirErr != nil {
		uploPath := fdn.staticFilesystem.renter.staticFileSystem.DirUploPath(fdn.staticDirNode)
		fdn.staticFilesystem.renter.log.Printf("Unable to perform lookup on %v in dir %v; file err %v :: dir err %v", name, uploPath, fileErr, dirErr)
...
func (ffn *fuseFilenode) Open(ctx context.Context, flags uint32) (fs.FileHandle, uint32, syscall.Errno) {
	...
	if err != nil {
		uploPath := ffn.staticFilesystem.renter.staticFileSystem.FileUploPath(ffn.staticFileNode)
		ffn.staticFilesystem.renter.log.Printf("Unable to get stream for file %v: %v", uploPath, err)
...
func (ffn *fuseFilenode) Read(ctx context.Context, f fs.FileHandle, dest []byte, offset int64) (fuse.ReadResult, syscall.Errno) {
	...
	if err != nil {
		uploPath := ffn.staticFilesystem.renter.staticFileSystem.FileUploPath(ffn.staticFileNode)
		ffn.staticFilesystem.renter.log.Printf("Error seeking to offset %v during call to Read in file %s: %v", offset, uploPath.String(), err)
		return nil, errToStatus(err)
	}
	...
	if err != nil && !errors.Contains(err, io.EOF) && err != io.ErrUnexpectedEOF {
		uploPath := ffn.staticFilesystem.renter.staticFileSystem.FileUploPath(ffn.staticFileNode)
		ffn.staticFilesystem.renter.log.Printf("Error reading from offset %v during call to Read in file %s: %v", offset, uploPath.String(), err)
		return nil, errToStatus(err)
	}
...
func (fdn *fuseDirnode) Readdir(ctx context.Context) (fs.DirStream, syscall.Errno) {
	...
	if err != nil {
		uploPath := fdn.staticFilesystem.renter.staticFileSystem.DirUploPath(fdn.staticDirNode)
		fdn.staticFilesystem.renter.log.Printf("Unable to get file and directory list for fuse directory %v: %v", uploPath, err)
		return nil, errToStatus(err)
	}
...
func (ffs *fuseFS) setStatfsOut(out *fuse.StatfsOut) error {
	...
	dirs, err := ffs.renter.DirList(modules.RootUploPath())
	if err != nil {
...
func (fdn *fuseDirnode) Statfs(ctx context.Context, out *fuse.StatfsOut) syscall.Errno {
	...
	if err != nil {
		uploPath := fdn.staticFilesystem.renter.staticFileSystem.DirUploPath(fdn.staticDirNode)
		fdn.staticFilesystem.renter.log.Printf("Error fetching statfs for fuse dir %v: %v", uploPath, err)
		return errToStatus(err)
	}
	return errToStatus(nil)
}
...
func (ffn *fuseFilenode) Statfs(ctx context.Context, out *fuse.StatfsOut) syscall.Errno {
	...
	if err != nil {
		uploPath := ffn.staticFilesystem.renter.staticFileSystem.FileUploPath(ffn.staticFileNode)
		ffn.staticFilesystem.renter.log.Printf("Error fetching statfs for fuse file %v: %v", uploPath, err)
		return errToStatus(err)
	}
	return errToStatus(nil)
}
