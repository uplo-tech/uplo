...
// updateuplodirHealth is a helper method to update the health and the aggregate
// health of a uplodir
func (r *Renter) updateuplodirHealth(uploPath modules.UploPath, health, aggregateHealth float64) (err error) {
...
func TestDirectoryHeap(t *testing.T) {
	...
	if !d.staticUploPath.Equals(modules.RootUploPath()) {
		t.Fatal("Directory should be root directory but is", d.staticUploPath)
	}

	// Make sure pushing an unexplored dir that doesn't exist works.
	randomSP, err := modules.RootUploPath().Join(modules.RandomUploPath().String())
	if err != nil {
		t.Fatal(err)
	}
	...
	// Try again but this time just remove the .uplodir file.
	err = os.Remove(randomSP.uplodirMetadataSysPath(rt.renter.staticFileSystem.Root()))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.managedPushUnexploredDirectory(randomSP)
...
func TestPushSubDirectories(t *testing.T) {
	...
	uploPath1, err := modules.NewUploPath("SubDir1")
	if err != nil {
		t.Fatal(err)
	}
	uploPath2, err := modules.NewUploPath("SubDir2")
	if err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(uploPath1, modules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(uploPath2, modules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}

	// Update metadata
	err = rt.renter.updateuplodirHealth(uploPath1, float64(2), float64(2))
	if err != nil {
		t.Fatal(err)
	}

	err = rt.renter.updateuplodirHealth(uploPath2, float64(3), float64(3))
	if err != nil {
		t.Fatal(err)
	}
	...
	// Add uplofiles sub directories
	d := &directory{
		staticUploPath: modules.RootUploPath(),
	}
	...
	d = rt.renter.directoryHeap.managedPop()
	if !d.staticUploPath.Equals(uploPath2) {
		t.Fatalf("Expected directory %v but found %v", uploPath2.String(), d.staticUploPath.String())
	}
	...
	d = rt.renter.directoryHeap.managedPop()
	if !d.staticUploPath.Equals(uploPath1) {
		t.Fatalf("Expected directory %v but found %v", uploPath1.String(), d.staticUploPath.String())
	}
...
func TestNextExploredDirectory(t *testing.T) {
	...
	uploPath1_1, err := uploPath1.Join("SubDir1")
	if err != nil {
		t.Fatal(err)
	}
	uploPath1_2, err := uploPath1.Join("SubDir2")
	if err != nil {
		t.Fatal(err)
	}
	uploPath2_1, err := uploPath2.Join("SubDir1")
	if err != nil {
		t.Fatal(err)
	}
	uploPath2_2, err := uploPath2.Join("SubDir2")
	if err != nil {
		t.Fatal(err)
	}
	// Create Directories
	if err := rt.renter.CreateDir(uploPath1_1, modules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(uploPath1_2, modules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(uploPath2_1, modules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}
	if err := rt.renter.CreateDir(uploPath2_2, modules.DefaultDirPerm); err != nil {
		t.Fatal(err)
	}

	// Update metadata
	err = rt.renter.updateuplodirHealth(modules.RootUploPath(), float64(0), float64(3))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateuplodirHealth(uploPath1, float64(1), float64(2))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateuplodirHealth(uploPath1_1, float64(1), float64(1))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateuplodirHealth(uploPath1_2, float64(2), float64(2))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateuplodirHealth(uploPath2, float64(1), float64(3))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateuplodirHealth(uploPath2_1, float64(1), float64(1))
	if err != nil {
		t.Fatal(err)
	}
	err = rt.renter.updateuplodirHealth(uploPath2_2, float64(3), float64(3))
	if err != nil {
		t.Fatal(err)
	}
	...
	// Directory should be root/home/uplofiles/SubDir2/SubDir2
	if !d.staticUploPath.Equals(uploPath2_2) {
		t.Fatalf("Expected directory %v but found %v", uploPath2_2.String(), d.staticUploPath.String())
	}
	if d.aggregateHealth != float64(3) {
		t.Fatal("Expected AggregateHealth to be 3 but was", d.aggregateHealth)
	}
	...
	// Directory should be root/homes/uplofiles/SubDir1/SubDir2
	if !d.staticUploPath.Equals(uploPath1_2) {
		t.Fatalf("Expected directory %v but found %v", uploPath1_2.String(), d.staticUploPath.String())
	}
	if d.aggregateHealth != float64(2) {
		t.Fatal("Expected AggregateHealth to be 2 but was", d.aggregateHealth)
	}
