...
const (
	// persistVersion defines the Uplo version that the persistence was
	// last updated
	persistVersion = "1.4.2"
)

const (
	// AlertMSGUplofileLowRedundancy indicates that a file is below 75% redundancy.
	AlertMSGUplofileLowRedundancy = "The UploFile mentioned in the 'Cause' is below 75% redundancy"
	// AlertUplofileLowRedundancyThreshold is the health threshold at which we start
	// registering the LowRedundancy alert for a Uplofile.
	AlertUplofileLowRedundancyThreshold = 0.75
)

// AlertCauseUplofileLowRedundancy creates a customized "cause" for a uplofile
// with a certain path and health.
func AlertCauseUplofileLowRedundancy(uploPath modules.UploPath, health, redundancy float64) string {
	return fmt.Sprintf("Uplofile '%v' has a health of %v and redundancy of %v", uploPath.String(), health, redundancy)
}
...
var (
	...
	// in the cache size on Uplo's end growing to match the size of the
