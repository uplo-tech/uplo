...
func (w *worker) managedDropUploadChunks() {
	...
	for chunkToDrop := chunksToDrop.Pop(); chunkToDrop != nil; chunkToDrop = chunksToDrop.Pop() {
		...
		w.renter.repairLog.Debugf("dropping chunk %v of %s because the worker is dropping all chunks", chunkToDrop.staticIndex, chunkToDrop.staticUploPath)
	}
}
...
func (w *worker) managedPerformUploadChunkJob() {
	...
	// the upload attempt.
	//
	// Ignore the error if it's a ErrMaxVirtualSectors coming from a pre-1.0.1
	// host.
	root, err := e.Upload(uc.physicalChunkData[pieceIndex])
	ignoreErr := build.VersionCmp(hostSettings.Version, "1.0.1") < 0 && err != nil && strings.Contains(err.Error(), modules.ErrMaxVirtualSectors.Error())
	...
	if err != nil {
		failureErr := fmt.Errorf("Worker failed to add new piece to UploFile: %v", err)
		w.managedUploadFailed(uc, pieceIndex, failureErr)
		return
	}
...
func (w *worker) managedUploadFailed(uc *unfinishedUploadChunk, pieceIndex uint64, failureErr error) {
	w.renter.repairLog.Printf("Worker upload failed. Worker: %v, Chunk: %v of %s, Error: %v", w.staticHostPubKey, uc.staticIndex, uc.staticUploPath, failureErr)
	// Mark the failure in the worker if the gateway says we are online. It's
	// not the worker's fault if we are offline.
	if w.renter.g.Online() && !(strings.Contains(failureErr.Error(), uplofile.ErrDeleted.Error()) || errors.Contains(failureErr, uplofile.ErrDeleted)) {
		w.mu.Lock()
