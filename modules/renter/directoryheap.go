...
// directory is a helper struct that represents a uplodir in the
// repairDirectoryHeap
...
type directoryHeap struct {
	...
	// heap
	heapDirectories map[modules.UploPath]*directory
...
func (dh *directoryHeap) managedPop() (d *directory) {
	...
	if dh.heap.Len() > 0 {
		d = heap.Pop(&dh.heap).(*directory)
		delete(dh.heapDirectories, d.staticUploPath)
	}
	return d
}
...
func (dh *directoryHeap) managedPush(d *directory) {
	...
	_, exists := dh.heapDirectories[d.staticUploPath]
	if exists {
		if !dh.update(d) {
	...
	heap.Push(&dh.heap, d)
	dh.heapDirectories[d.staticUploPath] = d
}
...
func (dh *directoryHeap) managedReset() {
	dh.mu.Lock()
	defer dh.mu.Unlock()
	dh.heapDirectories = make(map[modules.UploPath]*directory)
...
func (dh *directoryHeap) update(d *directory) bool {
	heapDir, exists := dh.heapDirectories[d.staticUploPath]
	if !exists {
		return false
	}
	...
	heapDir.mu.Unlock()
	dh.heapDirectories[d.staticUploPath] = heapDir
	heap.Fix(&dh.heap, heapDir.index)
	return true
}

// managedPushDirectory adds a directory to the directory heap
func (dh *directoryHeap) managedPushDirectory(uploPath modules.UploPath, metadata uplodir.Metadata, explored bool) {
	d := &directory{
...
func (r *Renter) managedNextExploredDirectory() (*directory, error) {
	...
	for {
		...
		if err != nil {
			contextStr := fmt.Sprintf("unable to push subdirectories for `%v`", d.staticUploPath)
...
func (r *Renter) managedPushSubDirectories(d *directory) error {
	subDirs, err := r.managedSubDirectories(d.staticUploPath)
	if err != nil {
		contextStr := fmt.Sprintf("unable to get subdirectories for `%v`", d.staticUploPath)
...
// managedPushUnexploredDirectory reads the health from the uplodir metadata and
// pushes an unexplored directory element onto the heap
func (r *Renter) managedPushUnexploredDirectory(uploPath modules.UploPath) (err error) {
	// Grab the uplodir metadata. If it doesn't exist, create it.
	...
	r.directoryHeap.managedPushDirectory(uploPath, metadata, false)
	return nil
}
