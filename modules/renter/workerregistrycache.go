...
func (rc *registryRevisionCache) Get(pubKey types.UploPublicKey, tweak crypto.Hash) (uint64, bool) {
	rc.mu.Lock()
	defer rc.mu.Unlock()
...
func (rc *registryRevisionCache) Set(pubKey types.UploPublicKey, rv modules.SignedRegistryValue, force bool) {
	rc.mu.Lock()
	defer rc.mu.Unlock()
