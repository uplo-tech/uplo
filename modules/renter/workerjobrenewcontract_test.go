...
func TestRenewContract(t *testing.T) {
	...
	funding := types.UplocoinPrecision
	...
	expectedValidRenterOutput := types.UplocoinOutput{
	...
	expectedValidHostOutput := types.UplocoinOutput{
		Value:      hostPayout,
		UnlockHash: params.Host.UnlockHash,
	}
	expectedMissedHostOutput := types.UplocoinOutput{
		Value:      hostCollateral.Sub(baseCollateral).Add(params.Host.ContractPrice),
		UnlockHash: params.Host.UnlockHash,
	}
	expectedVoidOutput := types.UplocoinOutput{
