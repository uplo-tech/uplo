...
func (fm *fuseManager) Mount(mountPoint string, sp modules.UploPath, opts modules.MountOptions) (err error) {
	...
	rootDirNode, err := fm.renter.staticFileSystem.Openuplodir(sp)
	if err != nil {
		return errors.AddContext(err, "unable to open the mounted uplopath in the filesystem")
	}
	...
	if ok {
		return errors.New("there is already a uplo fuse system mounted at " + mountPoint)
	}
...
func (fm *fuseManager) MountInfo() []modules.MountInfo {
	...
	for mountPoint, filesystem := range fm.mountPoints {
		uploPath := filesystem.renter.staticFileSystem.DirUploPath(filesystem.root.staticDirNode)
