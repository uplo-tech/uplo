...
var (
	...
	// will allocate to complete an upload of a snapshot .uplo file to all hosts.
	// This is done with each host in parallel, and the .uplo file is not
...
type snapshotEntry struct {
	...
	DataSectors  [4]crypto.Hash // pointers to sectors containing snapshot .uplo file
}

// calcSnapshotUploadProgress calculates the upload progress of a snapshot.
func calcSnapshotUploadProgress(fileUploadProgress float64, dotUploUploadProgress float64) float64 {
	return 0.8*fileUploadProgress + 0.2*dotUploUploadProgress
}
...
func (r *Renter) UploadedBackups() ([]modules.UploadedBackup, []types.UploPublicKey, error) {
	...
	hosts := make([]types.UploPublicKey, 0, len(r.persist.SyncedContracts))
...
// of operations.
func (r *Renter) BackupsOnHost(hostKey types.UploPublicKey) ([]modules.UploadedBackup, error) {
	if err := r.tg.Add(); err != nil {
		return nil, err
	}
	defer r.tg.Done()
...
// UploadBackup creates a backup of the renter which is uploaded to the uplo
...
// uplo network as a snapshot and can be retrieved using only the seed.
func (r *Renter) managedUploadBackup(src, name string) error {
	...
	// Begin uploading the backup. When the upload finishes, the backup .uplo
...
func (r *Renter) DownloadBackup(dst string, name string) (err error) {
	...
	// Download snapshot's .uplo file.
	_, dotUplo, err := r.managedDownloadSnapshot(uid)
	if err != nil {
		return err
	}
	// Store it in the backup file set.
	backupUploPath, err := modules.BackupFolder.Join(name)
	if err != nil {
		return err
	}
	if err := r.staticFileSystem.WriteFile(backupUploPath, dotUplo, 0666); err != nil {
		return err
	}
	// Load the .uplo file.
	uploPath, err := modules.BackupFolder.Join(name)
	if err != nil {
		return err
	}
	...
	// Use .uplo file to download snapshot.
	snap, err := entry.Snapshot(uploPath)
	if err != nil {
		return err
	}
...
// managedUploadSnapshot uploads a snapshot .uplo file to all hosts.
func (r *Renter) managedUploadSnapshot(meta modules.UploadedBackup, dotUplo []byte) error {
...
func (r *Renter) managedDownloadSnapshot(uid [16]byte) (ub modules.UploadedBackup, dotUplo []byte, err error) {
	if err := r.tg.Add(); err != nil {
	...
	for i := range contracts {
		err := func() (err error) {
			...
			for _, root := range entry.DataSectors {
				...
				dotUplo = append(dotUplo, data...)
				if uint64(len(dotUplo)) >= entry.Size {
					dotUplo = dotUplo[:entry.Size]
					break
				}
			}
...
func (r *Renter) threadedSynchronizeSnapshots() {
	...
	for {
		...
		// First, process any snapshot uplofiles that may have finished uploading.
		...
		flf := func(info modules.FileInfo) {
			...
			for _, meta = range r.persist.UploadedBackups {
				sp, _ := info.UploPath.Rebase(modules.BackupFolder, modules.RootUploPath())
			...
			r.log.Println("Uploading snapshot", info.UploPath)
			err := func() error {
				// Grab the entry for the uploaded backup's uplofile.
				entry, err := r.staticFileSystem.OpenUploFile(info.UploPath)
				if err != nil {
					return errors.AddContext(err, "failed to get entry for snapshot")
				}
				// Read the uplofile from disk.
				...
				// holds a lock on the underlying dotuplo, which is also actively
				// a part of the repair system. The repair system locks the
				// renter then tries to grab a lock on the uplofile, while
				...
				meta.Size = uint64(len(dotUplo))
				if err := r.managedUploadSnapshot(meta, dotUplo); err != nil {
					return errors.Compose(err, entry.Close())
				}
				...
				// Delete the local uplofile.
				if err := r.staticFileSystem.DeleteFile(info.UploPath); err != nil {
					return err
				}
				return nil
			}()
			if err != nil {
				r.log.Println("Failed to upload snapshot .uplo:", err)
			}
		}
		...
		err = func() (err error) {
			...
			for _, uid := range missing {
				ub, dotUplo, err := r.managedDownloadSnapshot(uid)
				if err != nil {
					// TODO: if snapshot can't be found on any host, delete it
					return err
				}
				if err := w.UploadSnapshot(r.tg.StopCtx(), ub, dotUplo); err != nil {
					return err
				}
