...
// the uplo network.
func (r *Renter) Streamer(uploPath modules.UploPath, disableLocalFetch bool) (_ string, _ modules.Streamer, err error) {
	...
	node, err := r.staticFileSystem.OpenUploFile(uploPath)
	if err != nil {
		return "", nil, err
	}
	defer func() {
	...
	// Create the streamer
	snap, err := node.Snapshot(uploPath)
	if err != nil {
		return "", nil, err
	}
	s := r.managedStreamer(snap, disableLocalFetch)
	return uploPath.String(), s, nil
}

// StreamerByNode will open a streamer for the renter, taking a FileNode as
// input instead of a uplopath. This is important for fuse, which has filenodes
...
func (r *Renter) StreamerByNode(node *filesystem.FileNode, disableLocalFetch bool) (modules.Streamer, error) {
	...
	// Grab the current UploPath of the FileNode and then create a snapshot.
	sp := r.staticFileSystem.FileUploPath(node)
...
// managedStreamer creates a streamer from a uplofile snapshot and starts filling
// its cache.
func (r *Renter) managedStreamer(snapshot *uplofile.Snapshot, disableLocalFetch bool) modules.Streamer {
	s := &streamer{
		staticFile: snapshot,
		r:          r,
