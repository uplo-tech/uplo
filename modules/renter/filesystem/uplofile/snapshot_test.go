...
// TestSnapshot tests if a snapshot is created correctly from a UploFile.
func TestSnapshot(t *testing.T) {
	...
	// Make sure the snapshot has the same fields as the UploFile.
	...
	if sp != snap.staticUploPath {
		t.Error("uplopaths don't match")
	}
	// Compare the pieces.
...
// snapshots of Uplofiles with different sizes.
func benchmarkSnapshot(b *testing.B, fileSize uint64) {
	...
	for i := uint64(0); i < sf.NumChunks(); i++ {
		for j := uint64(0); j < uint64(rc.NumPieces()); j++ {
			if err := sf.AddPiece(types.UploPublicKey{}, i, j, crypto.Hash{}); err != nil {
				b.Fatal(err)
			}
		}
	}
	...
	for i := 0; i < b.N; i++ {
		_, err := sf.Snapshot(modules.RootUploPath())
		if err != nil {
			b.Fatal(err)
		}
	}
