...
// marshalMetadata marshals the metadata of the UploFile using json encoding.
func marshalMetadata(md Metadata) ([]byte, error) {
	return json.Marshal(md)
}

// marshalPubKeyTable marshals the public key table of the UploFile using Uplo
// encoding.
func marshalPubKeyTable(pubKeyTable []HostPublicKey) ([]byte, error) {
	...
	for _, pk := range pubKeyTable {
		if err := pk.MarshalUplo(&buf); err != nil {
			return nil, err
		}
	}
...
// find out by taking a look at the erasure coder within the uplofile header.
...
// unmarshalMetadata unmarshals the json encoded metadata of the UploFile.
...
// unmarshalPubKeyTable unmarshals a uplo encoded public key table.
func unmarshalPubKeyTable(raw []byte) (keys []HostPublicKey, err error) {
	...
	for {
		...
		if err = key.UnmarshalUplo(r); errors.Contains(err, io.EOF) {
			break
		} else if err != nil {
			return nil, err
		}
