...
type (
	...
	RPCRenewContractCollateralResponse struct {
		...
		NewInputs   []types.UplocoinInput
		NewOutputs  []types.UplocoinOutput
		FinalRevSig crypto.Signature
	}
...
func (epr RPCExecuteProgramResponse) MarshalUplo(w io.Writer) error {
	var errStr string
	if epr.Error != nil {
...
func (epr *RPCExecuteProgramResponse) UnmarshalUplo(r io.Reader) error {
	var errStr string
...
func (resp *rpcResponse) MarshalUplo(w io.Writer) error {
	if resp.data == nil {
		resp.data = struct{}{}
	}
...
func (resp *rpcResponse) UnmarshalUplo(r io.Reader) error {
