...
func TestNewAccountID(t *testing.T) {
	...
	// verify the account ID is valid by converting it to a UploPublicKey
	_ = aid.SPK()
...
func TestAccountID_FromSPK(t *testing.T) {
	...
	aid.FromSPK(types.UploPublicKey{})
	if !aid.IsZeroAccount() {
...
// TestAccountID_MarshalUplo tests the UploMarshaler implementation.
func TestAccountID_MarshalUplo(t *testing.T) {
	t.Parallel()
	spk := types.UploPublicKey{
		Algorithm: types.SignatureEd25519,
		Key:       fastrand.Bytes(32),
...
// TestAccountIDCompatUploMarshal makes sure that the persistence data of a
// UploPublicKey matches the data of a AccountID.
func TestAccountIDCompatUploMarhsal(t *testing.T) {
	t.Parallel()
	spk := types.UploPublicKey{
		Algorithm: types.SignatureEd25519,
		Key:       fastrand.Bytes(32),
