...
const (
	// MinimumSupportedRenterHostProtocolVersion is the minimum version of Uplo
...
const (
	...
	// NegotiateMaxUploPubkeySize defines the maximum size that a UploPubkey is
	// allowed to be when being sent over the wire during negotiation.
	NegotiateMaxUploPubkeySize = 1e3
...
type (
	...
	HostExternalSettings struct {
		...
		// is only to cover the Uplocoin fees that the host will suffer when
		...
		UploMuxPort string `json:"uplomuxport"`
	}
...
// UploMuxAddress returns the address of the host's uplomux.
func (hes HostExternalSettings) UploMuxAddress() string {
	return fmt.Sprintf("%s:%s", hes.NetAddress.Host(), hes.UploMuxPort)
}
...
// protocol.
func NewRenterSession(conn net.Conn, hostPublicKey types.UploPublicKey) (*RenterHostSession, LoopChallengeRequest, error) {
...
// transaction.
func CreateAnnouncement(addr NetAddress, pk types.UploPublicKey, sk crypto.SecretKey) (signedAnnouncement []byte, err error) {
...
func DecodeAnnouncement(fullAnnouncement []byte) (na NetAddress, spk types.UploPublicKey, err error) {
