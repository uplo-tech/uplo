...
func testParseUploadRequestParameters(t *testing.T) {
	...
	buildRequest := func(query url.Values, headers http.Header) *http.Request {
		req, err := http.NewRequest("POST", fmt.Sprintf("/skynet/skyfile/%s?%s", uplopath.String(), query.Encode()), nil)
		if err != nil {
	...
	param := httprouter.Param{Key: "uplopath", Value: uplopath.String()}
	...
	// verify 'uplopath' (no root)
	req = buildRequest(url.Values{}, http.Header{})
	_, params = parseRequest(req, defaultParams)
	expected, err := modules.SkynetFolder.Join(uplopath.String())
	if err != nil || params.uploPath != expected {
		t.Fatal("Unexpected", err)
	}

	// verify 'uplopath' (at root)
	req = buildRequest(url.Values{"root": trueStr}, http.Header{})
	_, params = parseRequest(req, defaultParams)
	if params.uploPath != uplopath {
		t.Fatal("Unexpected")
	}
