...
type (
	...
	RenterContract struct {
		...
		HostPublicKey types.UploPublicKey `json:"hostpublickey"`
		// HostVersion is the version of Uplo that the host is running
		...
		StorageSpendingDeprecated types.Currency `json:"StorageSpending,uplomismatch"`
	...
	// RenterShareASCII contains an ASCII-encoded .uplo file.
	RenterShareASCII struct {
		ASCIIuplo string `json:"asciiuplo"`
	}
	...
	RenterBackupsGET struct {
		...
		SyncedHosts   []types.UploPublicKey   `json:"syncedhosts"`
		UnsyncedHosts []types.UploPublicKey   `json:"unsyncedhosts"`
	}
	...
	DownloadInfo struct {
		...
		Offset          uint64          `json:"offset"`          // The offset within the uplofile requested for the download.
		UploPath         modules.UploPath `json:"uplopath"`         // The uplopath of the file used for the download.
...
// rebaseInputUploPath rebases the UploPath provided by the user to one that is
// prefixed by the user's home directory.
func rebaseInputUploPath(uploPath modules.UploPath) (modules.UploPath, error) {
	// Prepend the provided uplopath with the /home/uplofiles dir.
	if uploPath.IsRoot() {
		return modules.UserFolder, nil
	}
	return modules.UserFolder.Join(uploPath.String())
}

// trimuplodirFolder is a helper method to trim /home/uplofiles off of the
// uplopaths of the dirinfos since the user expects a path relative to
// /home/uplofiles and not relative to root.
func trimuplodirFolder(dis ...modules.DirectoryInfo) (_ []modules.DirectoryInfo, err error) {
	for i := range dis {
		dis[i].UploPath, err = dis[i].UploPath.Rebase(modules.UserFolder, modules.RootUploPath())
		if err != nil {
			return nil, err
		}
	}
...
// trimuplodirFolderOnFiles is a helper method to trim /home/uplofiles off of the
// uplopaths of the fileinfos since the user expects a path relative to
// /home/uplofiles and not relative to root.
func trimuplodirFolderOnFiles(fis ...modules.FileInfo) (_ []modules.FileInfo, err error) {
	for i := range fis {
		fis[i].UploPath, err = fis[i].UploPath.Rebase(modules.UserFolder, modules.RootUploPath())
		if err != nil {
			return nil, errors.AddContext(err, "unable to trim the user uplo path from a provided fileinfo")
		}
	}
	return fis, nil
}

// trimuplodirInfo is a helper method to trim /home/uplofiles off of the
// uplopaths of the fileinfos since the user expects a path relative to
// /home/uplofiles and not relative to root.
...
func (api *API) renterBackupsHandlerGET(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	var unsyncedHosts []types.UploPublicKey
outer:
	...
	if req.FormValue("host") != "" {
		var hostKey types.UploPublicKey
...
func (api *API) renterBackupsCreateHandlerPOST(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	tmpDir, err := ioutil.TempDir("", "uplo-backup")
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
...
func (api *API) renterCleanHandlerPOST(w http.ResponseWriter, _ *http.Request, _ httprouter.Params) {
	...
	cleanFunc := func(fi modules.FileInfo) {
		...
		deleteErrs = errors.Compose(deleteErrs, api.renter.DeleteFile(fi.UploPath))
	}
	err := api.renter.FileList(modules.RootUploPath(), true, false, cleanFunc)
...
func (api *API) parseRenterContracts(disabled, inactive, expired bool) RenterContracts {
	...
	for _, c := range api.renter.Contracts() {
		...
		contract := RenterContract{
			...
			Fees:                      c.TxnFee.Add(c.UplofundFee).Add(c.ContractFee),
...
func (api *API) renterFuseHandlerGET(w http.ResponseWriter, _ *http.Request, _ httprouter.Params) {
	...
	for i := 0; i < len(rfi.MountPoints); i++ {
		rebased, err := rfi.MountPoints[i].UploPath.Rebase(modules.UserFolder, modules.RootUploPath())
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
		rfi.MountPoints[i].UploPath = rebased
	}
...
func (api *API) renterFuseMountHandlerPOST(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	var err error
	spfv := req.FormValue("uplopath")
	if spfv == "" {
		uploPath = modules.RootUploPath()
	} else {
		uploPath, err = modules.NewUploPath(spfv)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
	}
	uploPath, err = rebaseInputUploPath(uploPath)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
	...
	if err := api.renter.Mount(mount, uploPath, opts); err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
	WriteSuccess(w)
...
func (api *API) renterRenameHandler(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	// Parse the uploPath and the newUploPath
	uploPath, err := modules.NewUploPath(ps.ByName("uplopath"))
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
	newUploPath, err := modules.NewUploPath(req.FormValue("newuplopath"))
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}

	// Determine whether the user is requesting a user uplopath, or a root uplopath.
	root, err := isCalledWithRootFlag(req)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
	// Rebase the user's input to the user folder if the user is requesting a user uplopath.
	if !root {
		uploPath, err = rebaseInputUploPath(uploPath)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		...
		newUploPath, err = rebaseInputUploPath(newUploPath)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
	}
	err = api.renter.RenameFile(uploPath, newUploPath)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
...
// renterFileHandler handles GET requests to the /renter/file/:uplopath API endpoint.
func (api *API) renterFileHandlerGET(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	// Determine the uplopath that the user wants to get the file from.
	uploPath, err := modules.NewUploPath(ps.ByName("uplopath"))
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	...
	// Fetch the file.
	file, err := api.renter.File(uploPath)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}

	// If the user requested the user uplopath, trim the dir folder so that the
	// output is all centered around the user's folder.
	if !root {
		files, err := trimuplodirFolderOnFiles(file)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
...
// renterFileHandler handles POST requests to the /renter/file/:uplopath API endpoint.
func (api *API) renterFileHandlerPOST(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	uploPath, err := modules.NewUploPath(ps.ByName("uplopath"))
	if err != nil {
		WriteError(w, Error{"unable to parse uplopath: " + err.Error()}, http.StatusBadRequest)
		return
	}
	if !root {
		uploPath, err = rebaseInputUploPath(uploPath)
	...
	if newTrackingPath != "" {
		if err := api.renter.SetFileTrackingPath(uploPath, newTrackingPath); err != nil {
	...
	if stuck != "" {
		...
		if err := api.renter.SetFileStuck(uploPath, s); err != nil {
...
func (api *API) renterFilesHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	files, err = trimuplodirFolderOnFiles(files...)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusInternalServerError)
		return
	}
...
func (api *API) renterDeleteHandler(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	err = api.renter.DeleteFile(uploPath)
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
...
func parseDownloadParameters(w http.ResponseWriter, req *http.Request, ps httprouter.Params) (modules.RenterDownloadParameters, error) {
	...
	// Determines whether to interpret the uplopath as a root path or originating
	// from /home/user.
	...
	// Parse the root parameter. If it is set we rebase the uplopath.
	...
	uploPath, err := modules.NewUploPath(ps.ByName("uplopath"))
	if err != nil {
		return modules.RenterDownloadParameters{}, errors.AddContext(err, "error parsing the uplopath")
	}

	// If root is not set we need to rebase the uplopath.
	if !root {
		uploPath, err = rebaseInputUploPath(uploPath)
		if err != nil {
...
func (api *API) renterStreamHandler(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	fileName, streamer, err := api.renter.Streamer(uploPath, disableLocalFetch)
	if err != nil {
...
// renterValidateUploPathHandler handles the API call that validates a uplopath
func (api *API) renterValidateUploPathHandler(w http.ResponseWriter, _ *http.Request, ps httprouter.Params) {
	// Try and create a new uplopath, this will validate the potential uplopath
	_, err := modules.NewUploPath(ps.ByName("uplopath"))
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	}
...
func (api *API) renterDirHandlerGET(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	str := ps.ByName("uplopath")
	if str == "" || str == "/" {
		uploPath = modules.RootUploPath()
	} else {
		uploPath, err = modules.NewUploPath(str)
	}
	if err != nil {
		WriteError(w, Error{err.Error()}, http.StatusBadRequest)
		return
	...
	directories, err := api.renter.DirList(uploPath)
	if err != nil {
	...
	if !root {
		directories, err = trimuplodirFolder(directories...)
		if err != nil {
			WriteError(w, Error{err.Error()}, http.StatusBadRequest)
			return
		}
	...
	var files []modules.FileInfo
	var mu sync.Mutex
	err = api.renter.FileList(uploPath, false, true, func(fi modules.FileInfo) {
		mu.Lock()
		files = append(files, fi)
		mu.Unlock()
	})
...
// renterDirHandlerPOST handles POST requests to /renter/dir/:uplopath?action=<>
...
func (api *API) renterDirHandlerPOST(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	if action == "create" {
		...
		err := api.renter.CreateDir(uploPath, mode)
		if err != nil {
	...
	if action == "delete" {
		err := api.renter.DeleteDir(uploPath)
		if err != nil {
	...
	if action == "rename" {
		newUploPath, err := modules.NewUploPath(req.FormValue("newuplopath"))
		if err != nil {
			WriteError(w, Error{"failed to parse newuplopath: " + err.Error()}, http.StatusBadRequest)
			return
		}
		newUploPath, err = rebaseInputUploPath(newUploPath)
		if err != nil {
		...
		err = api.renter.RenameDir(uploPath, newUploPath)
		if err != nil {
