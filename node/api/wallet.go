...
type (
	...
	WalletGET struct {
		...
		ConfirmedUplocoinBalance     types.Currency `json:"confirmedUplocoinbalance"`
		UnconfirmedOutgoingUplocoins types.Currency `json:"unconfirmedoutgoingUplocoins"`
		UnconfirmedIncomingUplocoins types.Currency `json:"unconfirmedincomingUplocoins"`

		UplocoinClaimBalance types.Currency `json:"Uplocoinclaimbalance"`
		UplofundBalance      types.Currency `json:"uplofundbalance"`
	...
	// WalletUplocoinsPOST contains the transaction sent in the POST call to
	// /wallet/Uplocoins.
	WalletUplocoinsPOST struct {
		Transactions   []types.Transaction   `json:"transactions"`
		TransactionIDs []types.TransactionID `json:"transactionids"`
	}

	// WalletUplofundsPOST contains the transaction sent in the POST call to
	// /wallet/uplofunds.
	WalletUplofundsPOST struct {
		Transactions   []types.Transaction   `json:"transactions"`
		TransactionIDs []types.TransactionID `json:"transactionids"`
	}
...
func (api *API) walletHandler(w http.ResponseWriter, _ *http.Request, _ httprouter.Params) {
	UplocoinBal, uplofundBal, uploclaimBal, err := api.wallet.ConfirmedBalance()
	if err != nil {
		WriteError(w, Error{fmt.Sprintf("Error when calling /wallet: %v", err)}, http.StatusBadRequest)
		return
	}
	UplocoinsOut, UplocoinsIn, err := api.wallet.UnconfirmedBalance()
	if err != nil {
		WriteError(w, Error{fmt.Sprintf("Error when calling /wallet: %v", err)}, http.StatusBadRequest)
		return
	}
	...
	WriteJSON(w, WalletGET{
		...
		ConfirmedUplocoinBalance:     UplocoinBal,
		UnconfirmedOutgoingUplocoins: UplocoinsOut,
		UnconfirmedIncomingUplocoins: UplocoinsIn,

		UplofundBalance:      uplofundBal,
		UplocoinClaimBalance: uploclaimBal,
...
// walletUplogkeyHandler handles API calls to /wallet/uplogkey.
func (api *API) walletUplogkeyHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	for _, keypath := range keyfiles {
		...
		if !filepath.IsAbs(keypath) {
			WriteError(w, Error{"error when calling /wallet/uplogkey: keyfiles contains a non-absolute path"}, http.StatusBadRequest)
			return
		}
	}

	for _, key := range potentialKeys {
		err := api.wallet.LoadUplogKeys(key, keyfiles)
		if err == nil {
			WriteSuccess(w)
			return
		}
		if !errors.Contains(err, modules.ErrBadEncryptionKey) {
			WriteError(w, Error{"error when calling /wallet/uplogkey: " + err.Error()}, http.StatusBadRequest)
			return
		}
	}
	WriteError(w, Error{"error when calling /wallet/uplogkey: " + modules.ErrBadEncryptionKey.Error()}, http.StatusBadRequest)
}
...
// walletUplocoinsHandler handles API calls to /wallet/Uplocoins.
func (api *API) walletUplocoinsHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	if req.FormValue("outputs") != "" {
		...
		txns, err = api.wallet.SendUplocoinsMulti(outputs)
		if err != nil {
			WriteError(w, Error{"error when calling /wallet/Uplocoins: " + err.Error()}, http.StatusInternalServerError)
			return
		}
	} else {
		...
		if !ok {
			WriteError(w, Error{"could not read amount from POST call to /wallet/Uplocoins"}, http.StatusBadRequest)
			return
		}
		dest, err := scanAddress(req.FormValue("destination"))
		if err != nil {
			WriteError(w, Error{"could not read address from POST call to /wallet/Uplocoins"}, http.StatusBadRequest)
			return
		}
		feeIncluded, err := scanBool(req.FormValue("feeIncluded"))
		if err != nil {
			WriteError(w, Error{"could not read feeIncluded from POST call to /wallet/Uplocoins"}, http.StatusBadRequest)
			return
		}

		if feeIncluded {
			txns, err = api.wallet.SendUplocoinsFeeIncluded(amount, dest)
		} else {
			txns, err = api.wallet.SendUplocoins(amount, dest)
		}
		if err != nil {
			WriteError(w, Error{"error when calling /wallet/Uplocoins: " + err.Error()}, http.StatusInternalServerError)
			return
	...
	WriteJSON(w, WalletUplocoinsPOST{
		Transactions:   txns,
		TransactionIDs: txids,
	})
}

// walletUplofundsHandler handles API calls to /wallet/uplofunds.
func (api *API) walletUplofundsHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	amount, ok := scanAmount(req.FormValue("amount"))
	if !ok {
		WriteError(w, Error{"could not read 'amount' from POST call to /wallet/uplofunds"}, http.StatusBadRequest)
		return
	}
	dest, err := scanAddress(req.FormValue("destination"))
	if err != nil {
		WriteError(w, Error{"error when calling /wallet/uplofunds: " + err.Error()}, http.StatusBadRequest)
		return
	}

	txns, err := api.wallet.SendUplofunds(amount, dest)
	if err != nil {
		WriteError(w, Error{"error when calling /wallet/uplofunds: " + err.Error()}, http.StatusInternalServerError)
		return
	}
	var txids []types.TransactionID
	for _, txn := range txns {
	...
	WriteJSON(w, WalletUplofundsPOST{
		Transactions:   txns,
		TransactionIDs: txids,
	})
}
