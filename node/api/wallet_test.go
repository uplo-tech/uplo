...
func TestWalletGETEncrypted(t *testing.T) {
	...
	srv, err := NewServer(testdir, "localhost:0", "Uplo-Agent", "", cs, nil, nil, g, nil, nil, nil, tp, w)
	if err != nil {
		t.Fatal(err)
	}
...
// TestWalletGETUplocoins probes the GET call to /wallet when the
// Uplocoin balance is being manipulated.
func TestWalletGETUplocoins(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Check the initial wallet is encrypted, unlocked, and has the Uplocoins
	// that got mined.
	var wg WalletGET
	err = st.getAPI("/wallet", &wg)
	if err != nil {
	...
	if wg.ConfirmedUplocoinBalance.Cmp(calculateCoinbase(1)) != 0 {
		t.Error("reported wallet balance does not reflect the single block that has been mined")
	}
	if wg.UnconfirmedOutgoingUplocoins.Cmp64(0) != 0 {
		t.Error("there should not be unconfirmed outgoing Uplocoins")
	}
	if wg.UnconfirmedIncomingUplocoins.Cmp64(0) != 0 {
		t.Error("there should not be unconfirmed incoming Uplocoins")
	}
	...
	sendUplocoinsValues := url.Values{}
	sendUplocoinsValues.Set("amount", "1234")
	sendUplocoinsValues.Set("destination", wag.Address.String())
	err = st.stdPostAPI("/wallet/Uplocoins", sendUplocoinsValues)
	if err != nil {
		t.Fatal(err)
	}

	// Check that the wallet is reporting unconfirmed Uplocoins.
	err = st.getAPI("/wallet", &wg)
	if err != nil {
		t.Fatal(err)
	}
	...
	if wg.UnconfirmedOutgoingUplocoins.Cmp64(0) <= 0 {
		t.Error("there should be unconfirmed outgoing Uplocoins")
	}
	if wg.UnconfirmedIncomingUplocoins.Cmp64(0) <= 0 {
		t.Error("there should be unconfirmed incoming Uplocoins")
	}
	if wg.UnconfirmedOutgoingUplocoins.Cmp(wg.UnconfirmedIncomingUplocoins) <= 0 {
		t.Error("net movement of Uplocoins should be outgoing (miner fees)")
	}
	...
	if wg.ConfirmedUplocoinBalance.Cmp(calculateCoinbase(2)) >= 0 {
...
func TestIntegrationWalletSweepSeedPOST(t *testing.T) {
	...
	st.wallet.SendUplocoins(types.UplocoinPrecision.Mul64(100), addr.UnlockHash())
	_, err = st.miner.AddBlock()
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationWalletLoadSeedPOST(t *testing.T) {
	...
	key2 := crypto.GenerateUploKey(crypto.TypeDefaultWallet)
...
func TestWalletTransactionGETid(t *testing.T) {
	...
	// Query the details of a transaction where Uplocoins were sent.
	//
	// NOTE: We call the SendUplocoins method directly to get convenient access
	// to the txid.
	sentValue := types.UplocoinPrecision.Mul64(3)
	txns, err := st.wallet.SendUplocoins(sentValue, types.UnlockHash{})
	if err != nil {
		t.Fatal(err)
	}
	_, err = st.miner.AddBlock()
	...
	sendUplocoinsValues := url.Values{}
	sendUplocoinsValues.Set("amount", sentValue.String())
	sendUplocoinsValues.Set("destination", wag.Address.String())
	err = st.stdPostAPI("/wallet/Uplocoins", sendUplocoinsValues)
	if err != nil {
		t.Fatal(err)
	...
	// Check that undocumented API behavior used in Uplo-UI still works with
	// current API.
...
// Tests that the /wallet/uplogkey call checks for relative paths.
func TestWalletRelativePathErrorUplog(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// Wallet loading from uplog should error if its source is a relative path
	loadUplogAbsoluteError := "error when calling /wallet/uplogkey: keyfiles contains a non-absolute path"

	// This should fail.
	loadUplogValues := url.Values{}
	loadUplogValues.Set("keyfiles", "test.dat")
	err = st.stdPostAPI("/wallet/uplogkey", loadUplogValues)
	if err == nil || !strings.Contains(err.Error(), loadUplogAbsoluteError) {
		t.Fatal(err)
	}
	...
	// This should fail.
	loadUplogValues = url.Values{}
	loadUplogValues.Set("keyfiles", "/test.dat,test.dat,../test.dat")
	err = st.stdPostAPI("/wallet/uplogkey", loadUplogValues)
	if err == nil || !strings.Contains(err.Error(), loadUplogAbsoluteError) {
		t.Fatal(err)
	}
	...
	// As should this.
	loadUplogValues = url.Values{}
	loadUplogValues.Set("keyfiles", "../test.dat,/test.dat")
	err = st.stdPostAPI("/wallet/uplogkey", loadUplogValues)
	if err == nil || !strings.Contains(err.Error(), loadUplogAbsoluteError) {
		t.Fatal(err)
	}
	...
	loadUplogValues.Set("keyfiles", filepath.Join(walletTestDir, "test.dat"))
	err = st.stdPostAPI("/wallet/uplogkey", loadUplogValues)
	if err == nil || err.Error() == loadUplogAbsoluteError {
		t.Fatal(err)
	}
	...
	loadUplogValues.Set("keyfiles", filepath.Join(walletTestDir, "test.dat")+","+filepath.Join(walletTestDir, "test1.dat"))
	err = st.stdPostAPI("/wallet/uplogkey", loadUplogValues)
	if err == nil || err.Error() == loadUplogAbsoluteError {
		t.Fatal(err)
	}
...
func TestWalletUplofunds(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// load uplofunds into the wallet
	uplogPath, _ := filepath.Abs("../../types/uplog0of1of1.uplokey")
	loadUplogValues := url.Values{}
	loadUplogValues.Set("keyfiles", uplogPath)
	loadUplogValues.Set("encryptionpassword", walletPassword)
	err = st.stdPostAPI("/wallet/uplogkey", loadUplogValues)
	if err != nil {
		t.Fatal(err)
	}
	...
	if wg.UplofundBalance.Cmp64(2000) != 0 {
		t.Fatalf("bad uplofund balance: expected %v, got %v", 2000, wg.UplofundBalance)
	}

	// spend the uplofunds into the wallet seed
	var wag WalletAddressGET
	err = st.getAPI("/wallet/address", &wag)
	if err != nil {
		t.Fatal(err)
	}
	sendUplofundsValues := url.Values{}
	sendUplofundsValues.Set("amount", "2000")
	sendUplofundsValues.Set("destination", wag.Address.String())
	err = st.stdPostAPI("/wallet/uplofunds", sendUplofundsValues)
	if err != nil {
		t.Fatal(err)
	}
	...
	if wg.UplocoinClaimBalance.IsZero() {
...
// TestWalletUplocoins tests the /wallet/Uplocoins endpoint, including sending
// to multiple addresses.
func TestWalletUplocoins(t *testing.T) {
	...
	sendAmount := types.UplocoinPrecision.Mul64(10000)
	var wag WalletAddressGET
	err = st2.getAPI("/wallet/address", &wag)
	if err != nil {
		t.Fatal(err)
	...
	sendUplocoinsValues := url.Values{}
	outputsJSON, _ := json.Marshal([]types.UplocoinOutput{{
		UnlockHash: wag.Address,
		Value:      sendAmount,
	}})
	sendUplocoinsValues.Set("outputs", string(outputsJSON))
	if err = st.stdPostAPI("/wallet/Uplocoins", sendUplocoinsValues); err != nil {
		t.Fatal(err)
	}
	...
	for i, w := range wallets[1:] {
		...
		if !wg.ConfirmedUplocoinBalance.Equals(sendAmount) {
			t.Errorf("wallet %d should have %v coins, has %v", i+2, sendAmount, wg.ConfirmedUplocoinBalance)
		}
	}
}
...
func TestWalletTransactionsGetAddr(t *testing.T) {
	...
	sentValue := types.UplocoinPrecision.Mul64(3)
	_, err = st.wallet.SendUplocoins(sentValue, addr)
	if err != nil {
		t.Fatal(err)
	}
