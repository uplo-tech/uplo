...
// A Server is a collection of uplod modules that can be communicated with over
// an http api.
type Server struct {
	api               *api.API
...
// host.
func (srv *Server) HostPublicKey() (types.UploPublicKey, error) {
	if srv.node.Host == nil {
		return types.UploPublicKey{}, errors.New("can't get public host key of a non-host node")
	}
...
func NewAsync(APIaddr string, requiredUserAgent string, requiredPassword string, nodeParams node.NodeParams, loadStartTime time.Time) (*Server, <-chan error) {
	...
	s, err := func() (*Server, error) {
		...
		if err != nil {
			return nil, errors.AddContext(err, "failed to load uplod config")
		}
		...
		srv := &Server{
			...
			apiServer: &http.Server{
				...
				// set reasonable timeout windows for requests, to prevent the Uplo API
		...
		// Create the Uplo node for the server after the server was started.
		n, errChan = node.New(nodeParams, loadStartTime)
		if err := modules.PeekErr(errChan); err != nil {
			if isAddrInUseErr(err) {
				return nil, fmt.Errorf("%v; are you running another instance of uplod?", err.Error())
			}
			return nil, errors.AddContext(err, "server is unable to create the Uplo node")
		}
