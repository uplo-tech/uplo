...
func (uc *UnsafeClient) SkynetSkyfilePostRawResponse(params modules.SkyfileUploadParameters) (http.Header, []byte, error) {
	...
	query := fmt.Sprintf("/skynet/skyfile/%s?%s", params.UploPath.String(), values.Encode())
	return uc.postRawResponse(query, params.Reader)
}

// RenterSkyfileGet wraps RenterFileRootGet to query a skyfile.
func (c *Client) RenterSkyfileGet(uploPath modules.UploPath, root bool) (rf api.RenterFile, err error) {
	if !root {
	...
	return c.RenterFileRootGet(uploPath)
}
...
func (c *Client) SkynetSkylinkPinPostWithTimeout(skylink string, params modules.SkyfilePinParameters, timeout int) error {
	...
	rootStr := fmt.Sprintf("%t", params.Root)
	values.Set("root", rootStr)
	values.Set("uplopath", params.UploPath.String())
	values.Set("timeout", fmt.Sprintf("%d", timeout))
...
// SkynetConvertUplofileToSkyfilePost uses the /skynet/skyfile endpoint to
// convert an existing uplofile to a skyfile. The input UploPath 'convert' is the
// uplopath of the uplofile that should be converted. The uplopath provided inside
// of the upload params is the name that will be used for the base sector of the
// skyfile.
func (c *Client) SkynetConvertUplofileToSkyfilePost(lup modules.SkyfileUploadParameters, convert modules.UploPath) (api.SkynetSkyfileHandlerPOST, error) {
	// Set the url values.
	values := url.Values{}
	...
	// Make the call to upload the file.
	query := fmt.Sprintf("/skynet/skyfile/%s?%s", lup.UploPath.String(), values.Encode())
...
func (c *Client) RegistryRead(spk types.UploPublicKey, dataKey crypto.Hash) (modules.SignedRegistryValue, error) {
	return c.RegistryReadWithTimeout(spk, dataKey, 0)
}

// RegistryReadWithTimeout queries the /skynet/registry [GET] endpoint with the
// specified timeout.
func (c *Client) RegistryReadWithTimeout(spk types.UploPublicKey, dataKey crypto.Hash, timeout time.Duration) (modules.SignedRegistryValue, error) {
	// Set the values.
	values := url.Values{}
...
func (c *Client) RegistryUpdate(spk types.UploPublicKey, dataKey crypto.Hash, revision uint64, sig crypto.Signature, skylink modules.Skylink) error {
