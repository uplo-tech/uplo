...
type (
	// A Client makes requests to the uplod HTTP API.
	Client struct {
		Options
	}
	...
	// client.
	Options struct {
		// Address is the API address of the uplod server.
		Address string

		// Password must match the password of the uplod server.
		Password string

		// UserAgent must match the User-Agent required by the uplod server. If not
		// set, it defaults to "Uplo-Agent".
		UserAgent string
...
// using build.APIPassword and the user agent will be set to "Uplo-Agent". Both
...
// setting the default uplod user agent to "Uplo-Agent" and setting the password
...
func DefaultOptions() (Options, error) {
	...
	return Options{
		...
		Password:  pw,
		UserAgent: "Uplo-Agent",
	}, nil
}

// NewRequest constructs a request to the uplod HTTP API, setting the correct
