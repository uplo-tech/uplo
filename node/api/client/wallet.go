...
// WalletUplocoinsMultiPost uses the /wallet/Uplocoin api endpoint to send money
// to multiple addresses at once
func (c *Client) WalletUplocoinsMultiPost(outputs []types.UplocoinOutput) (wsp api.WalletUplocoinsPOST, err error) {
	values := url.Values{}
	marshaledOutputs, err := json.Marshal(outputs)
	if err != nil {
		return api.WalletUplocoinsPOST{}, err
	}
	values.Set("outputs", string(marshaledOutputs))
	err = c.post("/wallet/Uplocoins", values.Encode(), &wsp)
	return
}

// WalletUplocoinsPost uses the /wallet/Uplocoins api endpoint to send money to a
// single address
func (c *Client) WalletUplocoinsPost(amount types.Currency, destination types.UnlockHash, feeIncluded bool) (wsp api.WalletUplocoinsPOST, err error) {
	values := url.Values{}
...
// WalletUplofundsPost uses the /wallet/uplofunds api endpoint to send uplofunds
// to a single address.
func (c *Client) WalletUplofundsPost(amount types.Currency, destination types.UnlockHash) (wsp api.WalletUplofundsPOST, err error) {
	values := url.Values{}
	values.Set("amount", amount.String())
	values.Set("destination", destination.String())
	err = c.post("/wallet/uplofunds", values.Encode(), &wsp)
	return
}

// WalletUplogKeyPost uses the /wallet/uplogkey endpoint to load a uplog key into
// the wallet.
func (c *Client) WalletUplogKeyPost(keyfiles, password string) (err error) {
	values := url.Values{}
	values.Set("keyfiles", keyfiles)
	values.Set("encryptionpassword", password)
	err = c.post("/wallet/uplogkey", values.Encode(), nil)
	return
}
