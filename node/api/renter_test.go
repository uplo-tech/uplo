...
func setupTestDownload(t *testing.T, size int, name string, waitOnRedundancy bool) (*serverTester, string) {
	...
	// Create a file.
	path := filepath.Join(build.UploTestingDir, "api", t.Name(), name)
...
func runDownloadTest(t *testing.T, filesize, offset, length int64, useHttpResp bool, testName string) error {
	ulUploPath, err := modules.NewUploPath(testName + ".dat")
	if err != nil {
		t.Fatal(err)
	}
	st, path := setupTestDownload(t, int(filesize), ulUploPath.String(), true)
	defer func() {
	...
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&offset=%d&length=%d", ulUploPath, offset, length)
	...
	if useHttpResp {
		...
		err = build.Retry(30, time.Second, func() error {
			...
			for _, download := range rdq.Downloads {
				if download.Received == download.Filesize && download.UploPath.Equals(ulUploPath) {
					return nil
				}
			}
...
func TestRenterDownloadError(t *testing.T) {
	...
	for _, download := range rdq.Downloads {
		if download.UploPath.String() == "test.dat" && download.Received == download.Filesize && download.Error == expectedErr.Error() {
...
func runDownloadParamTest(t *testing.T, length, offset, filesize int) error {
	...
	st, _ := setupTestDownload(t, filesize, ulUploPath, true)
	defer func() {
		if err := st.server.Close(); err != nil {
			t.Fatal(err)
		}
	...
	downpath := filepath.Join(st.dir, fname)
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&destination=%s", ulUploPath, downpath)
...
func TestRenterDownloadAsyncAndHttpRespError(t *testing.T) {
	...
	st, _ := setupTestDownload(t, int(filesize), ulUploPath, true)
	defer func() {
		if err := st.server.Close(); err != nil {
			t.Fatal(err)
		}
	...
	// Download the original file from offset 40 and length 10.
	fname := "offsetsinglechunk.dat"
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&destination=%s&async=true&httpresp=true", ulUploPath, fname)
	err := st.getAPI(dlURL, nil)
	if err == nil {
...
func TestRenterDownloadAsyncAndNotDestinationError(t *testing.T) {
	...
	// Download the original file from offset 40 and length 10.
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&async=true", ulUploPath)
	err := st.getAPI(dlURL, nil)
	if err == nil {
...
func TestRenterDownloadHttpRespAndDestinationError(t *testing.T) {
	...
	// Download the original file from offset 40 and length 10.
	fname := "test.dat"
	dlURL := fmt.Sprintf("/renter/download/%s?disablelocalfetch=true&destination=%shttpresp=true", ulUploPath, fname)
	err := st.getAPI(dlURL, nil)
	if err == nil {
...
func TestRenterAsyncDownloadError(t *testing.T) {
	...
	for _, download := range rdq.Downloads {
		if download.UploPath.String() == "test.dat" && download.Received == download.Filesize && download.Error == "" {
...
func TestRenterAsyncDownload(t *testing.T) {
	...
	for start := time.Now(); time.Since(start) < 30*time.Second; time.Sleep(time.Millisecond * 10) {
		...
		for _, download := range rdq.Downloads {
			if download.Received == download.Filesize && download.UploPath.String() == "test.dat" {
				success = true
			}
		}
		if success {
...
func TestRenterPaths(t *testing.T) {
	...
	// Create a file.
	path := filepath.Join(build.UploTestingDir, "api", t.Name(), "test.dat")
	...
	if len(rf.Files) != 1 || rf.Files[0].UploPath.String() != "foo/bar/test" {
...
func TestRenterConflicts(t *testing.T) {
	...
	// should automatically create a folder called foo/bar.uplo. Later, we'll
	// exploit this by uploading a file called foo/bar.
	uploadValues := url.Values{}
	uploadValues.Set("source", path)
	uploadValues.Set("renew", "true")
	err = st.stdPostAPI("/renter/upload/foo/bar.uplo/test", uploadValues)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(rf.Files) != 1 || rf.Files[0].UploPath.String() != "foo/bar.uplo/test" {
		t.Fatal("/renter/files did not return correct file:", rf)
	}
...
func TestRenterHandlerRename(t *testing.T) {
	...
	renameValues.Set("newuplopath", "newdne")
	...
	// Rename the file.
	renameValues.Set("newuplopath", "newtest1")
	...
	if err == nil || err.Error() != filesystem.ErrExists.Error() {
		t.Errorf("expected error to be %v; got %v", uplofile.ErrPathOverload, err)
	}
}
...
func TestRenterPricesHandlerPricey(t *testing.T) {
	...
	// you have to pay the uplofund fee on
...
func TestAdversarialPriceRenewal(t *testing.T) {
	...
	allowanceValues := url.Values{}
	testPeriod := "10000"
	allowanceValues.Set("funds", types.UplocoinPrecision.Mul64(10000).String())
	allowanceValues.Set("period", testPeriod)
	err = st.stdPostAPI("/renter", allowanceValues)
	if err != nil {
		t.Fatal(err)
	...
	err = build.Retry(100, time.Millisecond*500, func() error {
		...
		available := true
		err := st.renter.FileList(modules.RootUploPath(), true, false, func(fi modules.FileInfo) {
			if !fi.Available {
				available = false
			}
		})
...
// verify the health information stored in the uplodir metadata is getting
