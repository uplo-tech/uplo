...
// skynetSkylinkPinHandlerPOST will pin a skylink to this Uplo node, ensuring
...
func (api *API) skynetSkylinkPinHandlerPOST(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	// Parse whether the uplopath should be from root or from the skynet folder.
	var root bool
	rootStr := queryForm.Get("root")
	if rootStr != "" {
	...
	uploPathStr := queryForm.Get("uplopath")
	if root {
	...
	if err != nil {
		WriteError(w, Error{"invalid uplopath provided: " + err.Error()}, http.StatusBadRequest)
		return
	}
...
// field is set, this endpoint will create a skyfile using an existing uplofile.
// The original uplofile and the skyfile will both need to be kept in order for
...
func (api *API) skynetSkyfileHandlerPOST(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	// Check whether this is a streaming upload or a uplofile conversion. If no
	...
	if params.convertPath == "" {
		...
		// upload. The first is the uplopath, and then the second is the uplopath
		...
		// measure.
		file, err := api.renter.File(sup.UploPath)
		extendedPath := sup.UploPath
	...
	convertPath, err := modules.NewUploPath(params.convertPath)
	if err != nil {
		WriteError(w, Error{"invalid convertpath provided: " + err.Error()}, http.StatusBadRequest)
		return
	}
	convertPath, err = rebaseInputUploPath(convertPath)
	if err != nil {
		WriteError(w, Error{"invalid convertpath provided - can't rebase: " + err.Error()}, http.StatusBadRequest)
		return
	}
	skylink, err := api.renter.CreateSkylinkFromUplofile(sup, convertPath)
	if errors.Contains(err, renter.ErrSkylinkBlocked) {
		WriteError(w, Error{err.Error()}, http.StatusUnavailableForLegalReasons)
		return
	}
	if err != nil {
		WriteError(w, Error{fmt.Sprintf("failed to convert uplofile to skyfile: %v", err)}, http.StatusBadRequest)
		return
	}
...
func (api *API) skynetStatsHandlerGET(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	dis, err := api.renter.DirList(modules.RootUploPath())
	if err == nil {
