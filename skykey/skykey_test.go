...
func TestSkykeyManager(t *testing.T) {
	...
	err = skykey.marshalUplo(&buf)
	if err != nil {
		t.Fatal(err)
	}

	var decodedSkykey Skykey
	err = decodedSkykey.unmarshalUplo(&buf)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSkykeyFormatCompat(t *testing.T) {
	...
	_, err = crypto.NewUploKey(oldSkykey.ciphertype, oldSkykey.entropy)
	if err != nil {
	...
	var buf bytes.Buffer
	err = oldSkykey2.marshalUplo(&buf)
	if err != nil {
		t.Fatal(err)
	}

	var decodedOK2 compatSkykeyV148
	err = decodedOK2.unmarshalUplo(&buf)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSkykeyMarshalling(t *testing.T) {
	...
	cipherKey := crypto.GenerateUploKey(skykeyType.CipherType())
	skykey := Skykey{
		Type:    skykeyType,
