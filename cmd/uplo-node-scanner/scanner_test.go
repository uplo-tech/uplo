...
func TestLoad(t *testing.T) {
	...
	err := uploPersist.LoadJSON(persistMetadata, &data, testPersistFile)
	if err != nil {
...
func TestSendShareNodesRequests(t *testing.T) {
	...
	mainGateway, err := gateway.New("localhost:0", true, build.TempDir("UploNodeScannerTestGateway"))
	if err != nil {
	...
	for i := 0; i < numTestingGateways; i++ {
		g, err := gateway.New("localhost:0", true, build.TempDir(fmt.Sprintf("UploNodeScannerTestGateway-%d", i)))
		if err != nil {
			t.Fatal("Error making new gateway: ", err)
		}
...
func TestRestartScanner(t *testing.T) {
	...
	testDir := build.TempDir("UploNodeScanner-TestRestartScanner")
	...
	for i := 0; i < numTestingGateways; i++ {
		g, err := gateway.New(fmt.Sprintf("localhost:4444%d", i), true, build.TempDir(fmt.Sprintf("UploNodeScannerTestGateway-%d", i)))
		if err != nil {
			t.Fatal("Error making new gateway: ", err)
		}
		gateways = append(gateways, g)
	...
	err = uploPersist.LoadJSON(persistMetadata, &testData2, ns.persistFile)
	if err != nil {
