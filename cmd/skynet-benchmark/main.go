...
func main() {
	...
	// Determine which port to use when talking to uplod.
	args := os.Args
	addr := "localhost:8480"
	var password string
	var cmd string
	if len(args) == 1 {
		...
		fmt.Println("Usage: ./skynet-benchmark [optional: which test to run] [optional: endpoint for uplod api, defaults to \"localhost:8480\"] [optional: api password]\n\tTest options: 'dl' and 'basic'")
		return
	}

	// Create the client that will be used to talk to uplod.
	opts, err := client.DefaultOptions()
	if err != nil {
		fmt.Println("Unable to get Uplo client options:", err)
		return
	}
	opts.Address = addr
	if password != "" {
