...
// TODO: If you run uploc from a non-existent directory, the abs() function does
...
var (
	...
	renterBackupCreateCmd = &cobra.Command{
		...
		Short: "Create a backup of the renter's uplofiles",
		Long:  "Create a backup of the renter's uplofiles, using the specified name.",
		Run:   wrap(renterbackupcreatecmd),
	}

	renterBackupLoadCmd = &cobra.Command{
		Use:   "restorebackup [name]",
		Short: "Restore a backup of the renter's uplofiles",
		Long:  "Restore the backup of the renter's uplofiles with the given name.",
...
the command 'uploc renter lost' to see the renter's lost files.`,
	...
	renterFilesListCmd = &cobra.Command{
		...
		Long:  "List the status of a specific file or all files known to the renter within the specified folder on the Uplo network. To query the root dir either '\"\"', '/' or '.' can be supplied",
	...
	renterFuseMountCmd = &cobra.Command{
		Use:   "mount [path] [uplopath]",
		Short: "Mount a Uplo folder to your disk",
		Long: `Mount a Uplo folder to your disk. Applications will be able to see this folder
as though it is a normal part of your filesystem.  Currently experimental, and
read-only. When Uplo is ready to support read-write fuse mounting, uploc will be
...
read-only mode is used, you must use the API.`,
	...
	renterFuseUnmountCmd = &cobra.Command{
		Use:   "unmount [path]",
		Short: "Unmount a Uplo folder",
		Long: `Unmount a Uplo folder that has previously been mounted. Unmount by specifying the
local path where the Uplo folder is mounted.`,
		Run: wrap(renterfuseunmountcmd),
	}

	renterSetLocalPathCmd = &cobra.Command{
		Use:   "setlocalpath [uplopath] [newlocalpath]",
	...
	renterFilesUploadCmd = &cobra.Command{
		...
		Long: `Upload a file or folder to [path] on the Uplo network. The --data-pieces and --parity-pieces
...
For Example: 'uploc renter upload pause 3h' would pause uploads for 3 hours.`,
...
Note that setting the allowance will cause uplod to immediately begin forming
...
func renterlostcmd() {
	...
	dirs := getDir(modules.RootUploPath(), true, true)
...
// renteruploadscmd is the handler for the command `uploc renter uploads`.
...
func renteruploadscmd() {
	...
	for _, file := range filteredFiles {
		fmt.Printf("%13s  %s (uploading, %0.2f%%)\n", modules.FilesizeUnits(file.Filesize), file.UploPath, file.UploadProgress)
	}
}

// renterdownloadscmd is the handler for the command `uploc renter downloads`.
...
func renterdownloadscmd() {
	...
	if len(downloading) == 0 {
		...
		for _, file := range downloading {
			fmt.Printf("%s: %5.1f%% %s -> %s\n", file.StartTime.Format("Jan 02 03:04 PM"), 100*float64(file.Received)/float64(file.Filesize), file.UploPath, file.Destination)
		}
	}
	...
	if len(downloaded) == 0 {
		...
		for _, file := range downloaded {
			fmt.Printf("%s: %s -> %s\n", file.StartTime.Format("Jan 02 03:04 PM"), file.UploPath, file.Destination)
		}
	}
}

// renterallowancecmd is the handler for the command `uploc renter allowance`.
...
// renterallowancecancelcmd is the handler for `uploc renter allowance cancel`.
...
// rentersetallowancecmd is the handler for `uploc renter setallowance`.
...
// rentersetallowancecmdInteractive is the interactive handler for `uploc renter
// setallowance`.
...
The command 'uploc renter allowance' can be used to see a breakdown of spending.
...
Expected upload tells uplod how much uploading the user expects to do each
period. If this value is high, uplod will more strongly prefer hosts that have a
low upload bandwidth price. If this value is low, uplod will focus on other
...
Expected download tells uplod how much downloading the user expects to do each
period. If this value is high, uplod will more strongly prefer hosts that have a
low download bandwidth price. If this value is low, uplod will focus on other
...
// renterbackcreatecmd is the handler for the command `uploc renter
// createbackup`.
...
// renterbackuprestorecmd is the handler for the command `uploc renter
// restorebackup`.
...
// renterbackuplistcmd is the handler for the command `uploc renter listbackups`.
...
// rentercontractscmd is the handler for the command `uploc renter contracts`.
...
// rentercontractsviewcmd is the handler for the command `uploc renter contracts <id>`.
...
// renterfilesdownload downloads the dir at the given path from the Uplo network
...
func renterdirdownload(path, destination string) {
	...
	// Download dir.
	start := time.Now()
	tfs, skipped, totalSize, downloadErr := downloadDir(uploPath, destination)
	...
	for _, fd := range failedDownloads {
		fmt.Printf("Download of file '%v' to destination '%v' failed: %v\n", fd.UploPath, fd.Destination, fd.Error)
	}
	os.Exit(1)
}

// renterdownloadcancelcmd is the handler for the command `uploc renter download cancel [cancelID]`
...
// renterfilesdeletecmd is the handler for the command `uploc renter delete [path]`.
// Removes the specified path from the Uplo network.
func renterfilesdeletecmd(cmd *cobra.Command, paths []string) {
	for _, path := range paths {
		...
		var errFile error
		if renterDeleteRoot {
			errFile = httpClient.RenterFileDeleteRootPost(uploPath)
		} else {
			errFile = httpClient.RenterFileDeletePost(uploPath)
		}
		if errFile == nil {
		...
		var errDir error
		if renterDeleteRoot {
			errDir = httpClient.RenterDirDeleteRootPost(uploPath)
		} else {
			errDir = httpClient.RenterDirDeletePost(uploPath)
		}
		if errDir == nil {
...
// renterfilesdownload is the handler for the command `uploc renter download
...
func renterfilesdownloadcmd(path, destination string) {
	...
	_, err = httpClient.RenterFileRootGet(uploPath)
	if err == nil {
	...
	_, err = httpClient.RenterDirRootGet(uploPath)
	if err == nil {
...
// renterfileslistcmd is the handler for the command `uploc renter ls`. Lists
...
func renterfileslistcmd(cmd *cobra.Command, args []string) {
	...
	for _, dir := range dirs {
		fmt.Println(dir.dir.UploPath.String() + "/")
		w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
...
// renterfilesrenamecmd is the handler for the command `uploc renter rename [path] [newpath]`.
// Renames a file on the Uplo network.
func renterfilesrenamecmd(path, newpath string) {
	// Parse UploPath.
	uploPath, err1 := modules.NewUploPath(path)
	newUploPath, err2 := modules.NewUploPath(newpath)
	if err := errors.Compose(err1, err2); err != nil {
		die("Couldn't parse UploPath:", err)
	}
	err := httpClient.RenterRenamePost(uploPath, newUploPath, renterRenameRoot)
	if err != nil {
...
func renterfusecmd() {
	...
	w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
	fmt.Fprintf(w, "\t%s\t%s\n", "Mount Point", "UploPath")
	for _, mp := range mountPoints {
		uploPathStr := mp.UploPath.String()
		if uploPathStr == "" {
			uploPathStr = "{root}"
		}

		fmt.Fprintf(w, "\t%s\t%s\n", mp.MountPoint, uploPathStr)
	}
	if err := w.Flush(); err != nil {
		die("failed to flush writer:", err)
	}
...
// renterfusemountcmd is the handler for the command `uploc renter fuse mount [path] [uplopath]`.
func renterfusemountcmd(path, uploPathStr string) {
	// TODO: Once read-write is supported on the backend, the 'true' flag can be
	// set to 'false' - uploc will support mounting read-write by default. Need
	...
	var err error
	if uploPathStr == "" || uploPathStr == "/" {
		uploPath = modules.RootUploPath()
	} else {
		uploPath, err = modules.NewUploPath(uploPathStr)
		if err != nil {
			die("Unable to parse the uplopath that should be mounted:", err)
		}
	}
	...
	err = httpClient.RenterFuseMount(path, uploPath, opts)
	if err != nil {
...
// renterfuseunmountcmd is the handler for the command `uploc renter fuse unmount [path]`.
...
//rentersetlocalpathcmd is the handler for the command `uploc renter setlocalpath [uplopath] [newlocalpath]`
//Changes the trackingpath of the file
//through API Endpoint
func rentersetlocalpathcmd(uplopath, newlocalpath string) {
	//Parse Uplopath
	uploPath, err := modules.NewUploPath(uplopath)
	if err != nil {
		die("Couldn't parse Uplopath:", err)
	}
	err = httpClient.RenterSetRepairPathPost(uploPath, newlocalpath)
	if err != nil {
		die("Could not Change the path of the file:", err)
	}
	fmt.Printf("Updated %s localpath to %s\n", uplopath, newlocalpath)
}

// renterfilesunstuckcmd is the handler for the command `uploc renter
...
func renterfilesunstuckcmd() {
	...
	toUnstuck := make(chan modules.UploPath)
	worker := func() {
		for uploPath := range toUnstuck {
			err := httpClient.RenterSetFileStuckPost(uploPath, true, false)
			if err != nil {
				die(fmt.Sprintf("Couldn't set %v to unstuck: %v", uploPath, err))
			}
...
// renterfilesuploadcmd is the handler for the command `uploc renter upload
// [source] [path]`. Uploads the [source] file to [path] on the Uplo network.
...
func renterfilesuploadcmd(source, path string) {
	...
	if stat.IsDir() {
		...
		for _, file := range files {
			...
			// Parse UploPath.
			fUploPath, err := modules.NewUploPath(fpath)
			if err != nil {
				die("Couldn't parse UploPath:", err)
			}
			err = httpClient.RenterUploadPost(abs(file), fUploPath, uint64(numDataPieces), uint64(numParityPieces))
			if err != nil {
				failed++
		...
		err = httpClient.RenterUploadPost(abs(source), uploPath, uint64(numDataPieces), uint64(numParityPieces))
		if err != nil {
...
// renterfilesuploadpausecmd is the handler for the command `uploc renter upload
...
// renterfilesuploadresumecmd is the handler for the command `uploc renter upload
...
// renterpricescmd is the handler for the command `uploc renter prices`, which
...
// renterratelimitcmd is the handler for the command `uploc renter ratelimit`
...
// renterworkerscmd is the handler for the command `uploc renter workers`.
...
// renterworkerseacmd is the handler for the command `uploc renter workers ea`.
...
// renterworkersdownloadscmd is the handler for the command `uploc renter workers
...
// renterworkersptcmd is the handler for the command `uploc renter workers pt`.
...
// renterworkersrjcmd is the handler for the command `uploc renter workers rj`.
...
// renterworkershsjcmd is the handler for the command `uploc renter workers hs`.
...
// renterworkersuploadscmd is the handler for the command `uploc renter workers
...
// renterworkerreadregistrycmd is the handler for the command `uploc renter workers
...
// renterworkerupdateregistrycmd is the handler for the command `uploc renter
