...
var (
	// General Flags
	alertSuppress bool
	uplodir        string // Path to uplo data dir
...
// statuscmd is the handler for the command `uploc`
// prints basic information about Uplo.
func statuscmd() {
	// For UX formating
	defer fmt.Println()
...
`, currencyUnits(walletStatus.ConfirmedUplocoinBalance))
	} else {
		fmt.Printf(`Wallet:
  Status: Locked
...
func main() {
	...
	// initialize client
	initClient(rootCmd, &verbose, &httpClient, &uplodir, &alertSuppress)
...
func initCmds() *cobra.Command {
	root := &cobra.Command{
		Use:   os.Args[0],
		Short: "Uplo Client v" + build.Version,
		Long:  "Uplo Client v" + build.Version,
		Run:   wrap(statuscmd),
	}
	...
	walletLoadCmd.AddCommand(walletLoad033xCmd, walletLoadSeedCmd, walletLoadUplogCmd)
	walletSendCmd.AddCommand(walletSendUplocoinsCmd, walletSendUplofundsCmd)
	walletSendUplocoinsCmd.Flags().BoolVarP(&walletTxnFeeIncluded, "fee-included", "", false, "Take the transaction fee out of the balance being submitted instead of the fee being additional")
	walletUnlockCmd.Flags().BoolVarP(&initPassword, "password", "p", false, "Display interactive password prompt even if UPLO_WALLET_PASSWORD is set")
...
func initClient(root *cobra.Command, verbose *bool, client *client.Client, uplodir *string, alertSuppress *bool) {
	root.PersistentFlags().BoolVarP(verbose, "verbose", "v", false, "Display additional information")
	root.PersistentFlags().StringVarP(&client.Address, "addr", "a", "localhost:8480", "which host/port to communicate with (i.e. the host/port uplod is listening on)")
	root.PersistentFlags().StringVarP(&client.Password, "apipassword", "", "", "the password for the API's http authentication")
	root.PersistentFlags().StringVarP(uplodir, "uplo-directory", "d", "", "location of the uplo directory")
	root.PersistentFlags().StringVarP(&client.UserAgent, "useragent", "", "Uplo-Agent", "the useragent used by uploc to connect to the daemon's API")
	root.PersistentFlags().BoolVarP(alertSuppress, "alert-suppress", "s", false, "suppress uploc alerts")
}
