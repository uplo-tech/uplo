...
var (
	...
	walletBalanceCmd = &cobra.Command{
		...
		Long:  "View wallet balance, including confirmed and unconfirmed Uplocoins and uplofunds.",
...
By default the wallet encryption / unlock password is the same as the generated seed.`,
	...
	walletLoadCmd = &cobra.Command{
		Use:   "load",
		Short: "Load a wallet seed, v0.3.3.x wallet, or uplog keyset",
	...
	walletLoadUplogCmd = &cobra.Command{
		Use:     `uplog [filepath,...]`,
		Short:   "Load uplog key(s) into the wallet",
		Long:    "Load uplog key(s) into the wallet - typically used for uplofunds.",
		Example: "uploc wallet load uplog key1.uplokey,key2.uplokey",
		Run:     wrap(walletloaduplogcmd),
	}
	...
	walletSendCmd = &cobra.Command{
		Use:   "send",
		Short: "Send either Uplocoins or uplofunds to an address",
		Long:  "Send either Uplocoins or uplofunds to an address",
		// Run field is not set, as the send command itself is not a valid command.
		// A subcommand must be provided.
	}

	walletSendUplocoinsCmd = &cobra.Command{
		Use:   "Uplocoins [amount] [dest]",
		Short: "Send Uplocoins to an address",
		Long: `Send Uplocoins to an address. 'dest' must be a 76-byte hexadecimal address.
...
A dynamic transaction fee is applied depending on the size of the transaction and how busy the network is.`,
		Run: wrap(walletsendUplocoinscmd),
	}

	walletSendUplofundsCmd = &cobra.Command{
		Use:   "uplofunds [amount] [dest]",
		Short: "Send uplofunds",
		Long: `Send uplofunds to an address, and transfer the claim Uplocoins to your wallet.
Run 'wallet send --help' to see a list of available units.`,
		Run: wrap(walletsenduplofundscmd),
	}

	walletSignCmd = &cobra.Command{
		Use:   "sign [txn] [tosign]",
		Short: "Sign a transaction",
		Long: `Sign a transaction. If uplod is running with an unlocked wallet, the
...
provided, the wallet will fill in every TransactionSignature it has keys for.`,
	...
	walletSweepCmd = &cobra.Command{
		Use:   "sweep",
		Short: "Sweep Uplocoins and uplofunds from a seed.",
		Long: `Sweep Uplocoins and uplofunds from a seed. The outputs belonging to the seed
will be sent to your wallet.`,
	...
	walletTransactionsCmd = &cobra.Command{
		...
		Long:  "View transactions related to addresses spendable by the wallet, providing a net flow of Uplocoins and uplofunds for each transaction",
...
UPLO_WALLET_PASSWORD environment variable is set, the unlock command will
...
// walletloaduplogcmd loads a uplog key set into the wallet.
func walletloaduplogcmd(keyfiles string) {
	password, err := passwordPrompt(askPasswordText)
	if err != nil {
		die("Reading password failed:", err)
	}
	err = httpClient.WalletUplogKeyPost(keyfiles, password)
	if err != nil {
		die("Loading uplog key failed:", err)
	}
	fmt.Println("Wallet loading successful.")
}
...
// walletsendUplocoinscmd sends Uplocoins to a destination address.
func walletsendUplocoinscmd(amount, dest string) {
	...
	_, err = httpClient.WalletUplocoinsPost(value, hash, walletTxnFeeIncluded)
	if err != nil {
		die("Could not send Uplocoins:", err)
	}
	fmt.Printf("Sent %s hastings to %s\n", hastings, dest)
}

// walletsenduplofundscmd sends uplofunds to a destination address.
func walletsenduplofundscmd(amount, dest string) {
	var value types.Currency
	...
	_, err := httpClient.WalletUplofundsPost(value, hash)
	if err != nil {
		die("Could not send uplofunds:", err)
	}
	fmt.Printf("Sent %s uplofunds to %s\n", amount, dest)
}
...
`, encStatus)
	...
	unconfirmedBalance := status.ConfirmedUplocoinBalance.Add(status.UnconfirmedIncomingUplocoins).Sub(status.UnconfirmedOutgoingUplocoins)
	var delta string
	if unconfirmedBalance.Cmp(status.ConfirmedUplocoinBalance) >= 0 {
		delta = "+" + currencyUnits(unconfirmedBalance.Sub(status.ConfirmedUplocoinBalance))
	} else {
		delta = "-" + currencyUnits(status.ConfirmedUplocoinBalance.Sub(unconfirmedBalance))
	}
...
Estimated Fee:       %v / KB
`, encStatus, status.Height, currencyUnits(status.ConfirmedUplocoinBalance), delta,
		status.ConfirmedUplocoinBalance, status.UplofundBalance, status.UplocoinClaimBalance,
...
func walletsigncmd(cmd *cobra.Command, args []string) {
	...
	if err == nil {
		...
		// if uplod is running, but the wallet is locked, assume the user
		// wanted to sign with uplod
		if strings.Contains(err.Error(), modules.ErrLockedWallet.Error()) {
			die("Signing via API failed: uplod is running, but the wallet is locked.")
		}

		// uplod is not running; fallback to offline keygen
...
// transactions without uplod.
func walletsigncmdoffline(txn *types.Transaction, toSign []crypto.Hash) {
	fmt.Println("Enter your wallet seed to generate the signing key(s) now and sign without uplod.")
...
// providing a net flow of Uplocoins and uplofunds for each.
func wallettransactionscmd() {
	...
	fmt.Println("             [timestamp]    [height]                                                   [transaction id]    [net Uplocoins]   [net uplofunds]")
	...
	for _, txn := range sts {
		// Determine the number of outgoing Uplocoins and uplofunds.
		var outgoingUplofunds types.Currency
		for _, input := range txn.Inputs {
			if input.FundType == types.SpecifierUplofundInput && input.WalletAddress {
				outgoingUplofunds = outgoingUplofunds.Add(input.Value)
			}
		}

		// Determine the number of incoming Uplocoins and uplofunds.
		var incomingUplofunds types.Currency
		for _, output := range txn.Outputs {
			if output.FundType == types.SpecifierUplofundOutput && output.WalletAddress {
				incomingUplofunds = incomingUplofunds.Add(output.Value)
			}
		}

		// Convert the Uplocoins to a float.
		incomingUplocoinsFloat, _ := new(big.Rat).SetFrac(txn.ConfirmedIncomingValue.Big(), types.UplocoinPrecision.Big()).Float64()
		outgoingUplocoinsFloat, _ := new(big.Rat).SetFrac(txn.ConfirmedOutgoingValue.Big(), types.UplocoinPrecision.Big()).Float64()
...
func walletunlockcmd() {
	...
	if password != "" && !initPassword {
		fmt.Println("Using UPLO_WALLET_PASSWORD environment variable")
