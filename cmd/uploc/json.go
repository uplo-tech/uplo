...
var (
	jsonCmd = &cobra.Command{
		Use:   "json",
		Short: "provide a json dump of uplod's current status",
		Long:  "queries a large number of endpoints in the uplod api and produces a json dump with all of the information",
		Run:   wrap(jsoncmd),
	}
)

// jsoncmd queries a large number of endpoints in the uplod api and aggregates
...
func jsoncmd() {
	...
	rs.TotalUplofiles = rf.Directories[0].AggregateNumFiles
	rs.Totaluplodirs = rf.Directories[0].AggregateNumSubDirs
	...
	rs.WalletFunds = wg.ConfirmedUplocoinBalance.Add(wg.UnconfirmedIncomingUplocoins).Sub(wg.UnconfirmedOutgoingUplocoins)
