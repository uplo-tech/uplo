...
type (
	...
	// 'ValidProofOutputs' must equal 'Payout' minus the uplofund fee. This fee
	// is sent to the uplofund pool, which is a set of Uplocoins only spendable
	// by uplofund owners.
	//
	...
	FileContract struct {
		...
		ValidProofOutputs  []UplocoinOutput `json:"validproofoutputs"`
		MissedProofOutputs []UplocoinOutput `json:"missedproofoutputs"`
	...
	FileContractRevision struct {
		...
		NewValidProofOutputs  []UplocoinOutput `json:"newvalidproofoutputs"`
		NewMissedProofOutputs []UplocoinOutput `json:"newmissedproofoutputs"`
	...
	//
	// A transaction with a StorageProof cannot have any UplocoinOutputs,
	// UplofundOutputs, or FileContracts. This is because a mundane reorg can
...
func (fcr FileContractRevision) HostPublicKey() UploPublicKey {
...
func (fcr FileContractRevision) PaymentRevision(amount Currency) (FileContractRevision, error) {
	...
	rev.NewValidProofOutputs = append([]UplocoinOutput{}, fcr.NewValidProofOutputs...)
	rev.NewMissedProofOutputs = append([]UplocoinOutput{}, fcr.NewMissedProofOutputs...)
...
func (fcr FileContractRevision) ExecuteProgramRevision(revisionNumber uint64, transfer Currency, newRoot crypto.Hash, newSize uint64) (FileContractRevision, error) {
	...
	// need to manually copy slice memory
	newRevision.NewValidProofOutputs = append([]UplocoinOutput{}, fcr.NewValidProofOutputs...)
	newRevision.NewMissedProofOutputs = append([]UplocoinOutput{}, fcr.NewMissedProofOutputs...)
...
func (fc FileContract) ValidRenterOutput() UplocoinOutput {
...
func (fc FileContract) ValidHostOutput() UplocoinOutput {
...
func (fc FileContract) MissedRenterOutput() UplocoinOutput {
...
func (fc FileContract) MissedHostOutput() UplocoinOutput {
	return fc.MissedProofOutputs[1]
}

// MissedVoidOutput gets the void's missed proof output.
func (fc FileContract) MissedVoidOutput() (UplocoinOutput, error) {
	if len(fc.MissedProofOutputs) <= 2 {
		return UplocoinOutput{}, ErrMissingVoidOutput
	}
...
// ValidRenterOutput gets the renter's valid proof output.
func (fcr FileContractRevision) ValidRenterOutput() UplocoinOutput {
...
// ValidHostOutput sets gets host's missed proof output.
func (fcr FileContractRevision) ValidHostOutput() UplocoinOutput {
...
// MissedRenterOutput gets the renter's missed proof output.
func (fcr FileContractRevision) MissedRenterOutput() UplocoinOutput {
...
// MissedHostOutput gets the host's missed proof output.
func (fcr FileContractRevision) MissedHostOutput() UplocoinOutput {
...
// MissedVoidOutput gets the void's missed proof output.
func (fcr FileContractRevision) MissedVoidOutput() (UplocoinOutput, error) {
	if len(fcr.NewMissedProofOutputs) <= 2 {
		return UplocoinOutput{}, ErrMissingVoidOutput
	}
...
func (fcid FileContractID) StorageProofOutputID(proofStatus ProofStatus, i uint64) UplocoinOutputID {
...
func Tax(height BlockHeight, payout Currency) Currency {
	...
	if height < TaxHardforkHeight {
		return payout.MulFloat(0.039).RoundDown(UplofundCount)
	}
	return payout.MulTax().RoundDown(UplofundCount)
}
