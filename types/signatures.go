...
type (
	...
	CoveredFields struct {
		...
		UplocoinInputs         []uint64 `json:"Uplocoininputs"`
		UplocoinOutputs        []uint64 `json:"Uplocoinoutputs"`
		...
		UplofundInputs         []uint64 `json:"uplofundinputs"`
		UplofundOutputs        []uint64 `json:"uplofundoutputs"`
	...
	// A UploPublicKey is a public key prefixed by a Specifier. The Specifier
	...
	// are three types that use UnlockConditions: UplocoinInputs, UplofundInputs,
	...
	// certain actions, such as spending a UplocoinOutput or terminating a
	// FileContract.
	//
	...
	UnlockConditions struct {
		Timelock           BlockHeight    `json:"timelock"`
		PublicKeys         []UploPublicKey `json:"publickeys"`
	...
	inputSignatures struct {
		...
		possibleKeys        []UploPublicKey
		usedKeys            map[uint64]struct{}
		index               int
	}
)

// Ed25519PublicKey returns pk as a UploPublicKey, denoting its algorithm as
// Ed25519.
func Ed25519PublicKey(pk crypto.PublicKey) UploPublicKey {
...
// Equals compares two UploPublicKey types for equality
func (x UploPublicKey) Equals(y UploPublicKey) bool {
	return x.Algorithm == y.Algorithm && bytes.Equal(x.Key, y.Key)
}

// ToPublicKey converts the UploPublicKey into a crypto.PublicKey
func (x UploPublicKey) ToPublicKey() (pk crypto.PublicKey) {
	// sanity check key length
	if len(pk) != len(x.Key) {
		build.Critical(fmt.Sprintf("Could not convert UploPublicKey to crypto.PublicKey, incorrect key length %v != %v", len(x.Key), len(pk)))
	}
	copy(pk[:], x.Key)
	return
}
...
func (t *Transaction) wholeSigHash(sig TransactionSignature, height BlockHeight) (hash crypto.Hash) {
	...
	for _, i := range sig.CoveredFields.TransactionSignatures {
		t.TransactionSignatures[i].MarshalUplo(h)
	}
...
func (t *Transaction) partialSigHash(cf CoveredFields, height BlockHeight) (hash crypto.Hash) {
	...
	for _, input := range cf.UplocoinInputs {
		h.Write(replayPrefix(height))
		t.UplocoinInputs[input].MarshalUplo(h)
	}
	for _, output := range cf.UplocoinOutputs {
		t.UplocoinOutputs[output].MarshalUplo(h)
	}
	for _, contract := range cf.FileContracts {
		t.FileContracts[contract].MarshalUplo(h)
	}
	for _, revision := range cf.FileContractRevisions {
		t.FileContractRevisions[revision].MarshalUplo(h)
	}
	for _, storageProof := range cf.StorageProofs {
		t.StorageProofs[storageProof].MarshalUplo(h)
	}
	for _, uplofundInput := range cf.UplofundInputs {
		h.Write(replayPrefix(height))
		t.UplofundInputs[uplofundInput].MarshalUplo(h)
	}
	for _, uplofundOutput := range cf.UplofundOutputs {
		t.UplofundOutputs[uplofundOutput].MarshalUplo(h)
	}
	for _, minerFee := range cf.MinerFees {
		t.MinerFees[minerFee].MarshalUplo(h)
	}
	...
	for _, sig := range cf.TransactionSignatures {
		t.TransactionSignatures[sig].MarshalUplo(h)
	}
...
func (t Transaction) validCoveredFields() error {
	for _, sig := range t.TransactionSignatures {
		...
		fieldMaxs := []struct {
			...
			{cf.UplocoinInputs, len(t.UplocoinInputs)},
			{cf.UplocoinOutputs, len(t.UplocoinOutputs)},
			...
			{cf.UplofundInputs, len(t.UplofundInputs)},
			{cf.UplofundOutputs, len(t.UplofundOutputs)},
