...
var (
	...
	// ErrNonZeroClaimStart is an error when a transaction has a uplofund
	// output with a non-zero uplofund claim
	ErrNonZeroClaimStart = errors.New("transaction has a uplofund output with a non-zero uplofund claim")
...
func (t Transaction) followsMinimumValues() error {
	...
	for _, sfo := range t.UplofundOutputs {
		// UplofundOutputs are special in that they have a reserved field, the
...
func (t Transaction) followsStorageProofRules() error {
	...
	// If there are storage proofs, there can be no Uplocoin outputs, uplofund
...
func (t Transaction) noRepeats() error {
	// Check that there are no repeat instances of Uplocoin outputs, storage
	// proofs, contract terminations, or uplofund outputs.
	UplocoinInputs := make(map[UplocoinOutputID]struct{})
	for _, sci := range t.UplocoinInputs {
		_, exists := UplocoinInputs[sci.ParentID]
		if exists {
			return ErrDoubleSpend
		}
		UplocoinInputs[sci.ParentID] = struct{}{}
	}
	...
	uplofundInputs := make(map[UplofundOutputID]struct{})
	for _, sfi := range t.UplofundInputs {
		_, exists := uplofundInputs[sfi.ParentID]
		if exists {
			return ErrDoubleSpend
		}
		uplofundInputs[sfi.ParentID] = struct{}{}
	}
	return nil
}
