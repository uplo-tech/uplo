...
func TransactionGraph(sourceOutput UplocoinOutputID, edges []TransactionGraphEdge) ([]Transaction, error) {
	...
	txnInputs := make([][]UplocoinOutputID, biggest+1)
	txnInputs[0] = []UplocoinOutputID{sourceOutput}
	...
	for i < len(edges) {
		...
		for _, outputID := range txnInputs[j] {
			t.UplocoinInputs = append(t.UplocoinInputs, UplocoinInput{
				ParentID: outputID,
			})
		}
		...
		for i < len(edges) && edges[i].Source == current {
			t.UplocoinOutputs = append(t.UplocoinOutputs, UplocoinOutput{
				Value:      edges[i].Value,
		...
		for k := startingPoint; k < i; k++ {
			txnInputs[edges[k].Dest] = append(txnInputs[edges[k].Dest], t.UplocoinOutputID(uint64(k-startingPoint)))
		}
		ts[j] = t
		j++
	}
