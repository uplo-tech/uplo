...
func TestMinimizeTransactionSet(t *testing.T) {
	...
	source2Index, err := graph1.AddUplocoinSource(sources[1], sourceSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	txn2Outputs, err := graph1.AddTransaction(typesutil.SimpleTransaction{
		UplocoinInputs: []int{source2Index},
		UplocoinOutputs: []types.Currency{
			types.UplocoinPrecision.Mul64(499),
			types.UplocoinPrecision.Mul64(500),
		},
	...
	_, err = graph1.AddTransaction(typesutil.SimpleTransaction{
		...
		UplocoinOutputs: []types.Currency{
			types.UplocoinPrecision.Mul64(998),
			types.UplocoinPrecision.Mul64(499),
		},
	...
	graph2Source, err := graph2.AddUplocoinSource(sources[0], sourceSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = graph2.AddTransaction(typesutil.SimpleTransaction{
		UplocoinInputs:  []int{graph2Source},
		UplocoinOutputs: []types.Currency{types.UplocoinPrecision.Mul64(998)},

		MinerFees: []types.Currency{types.UplocoinPrecision.Mul64(2)},
	})
	if err != nil {
		t.Fatal(err)
	}
