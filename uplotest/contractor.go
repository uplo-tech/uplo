...
// CheckBalanceVsSpending checks the renters confirmed Uplocoin balance in their
...
func CheckBalanceVsSpending(r *TestNode, initialBalance types.Currency) error {
	...
	if expectedBalance.Cmp(wg.ConfirmedUplocoinBalance) != 0 {
		details := fmt.Sprintf(`Initial balance minus Renter Reported Spending does not equal wallet Confirmed Uplocoin Balance
		Expected Balance:   %v
		Wallet Balance:     %v
		Actual difference:  %v
		`, expectedBalance.HumanString(), wg.ConfirmedUplocoinBalance.HumanString(), initialBalance.Sub(wg.ConfirmedUplocoinBalance).HumanString())
		var diff string
		if expectedBalance.Cmp(wg.ConfirmedUplocoinBalance) > 0 {
			diff = fmt.Sprintf("Under reported by:  %v\n", expectedBalance.Sub(wg.ConfirmedUplocoinBalance).HumanString())
		} else {
			diff = fmt.Sprintf("Over reported by:   %v\n", wg.ConfirmedUplocoinBalance.Sub(expectedBalance).HumanString())
		}
		err := details + diff
...
func DrainContractsByUploading(renter *TestNode, tg *TestGroup) (startingUploadSpend types.Currency, err error) {
	...
	maxStoragePrice := types.UplocoinPrecision.Mul64(3e6).Div(modules.BlockBytesPerMonthTerabyte)
