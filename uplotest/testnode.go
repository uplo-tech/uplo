...
func newCleanNode(nodeParams node.NodeParams, asyncSync bool) (*TestNode, error) {
	...
	// Check if the UploMuxAddress is set, if not we want to set it to use a
	// random port in testing
	if nodeParams.UploMuxTCPAddress == "" {
		nodeParams.UploMuxTCPAddress = "localhost:0"
	}
	if nodeParams.UploMuxWSAddress == "" {
		nodeParams.UploMuxWSAddress = "localhost:0"
	}
...
// UploPath returns the uplopath of a local file or directory to be used for
// uploading
func (tn *TestNode) UploPath(path string) modules.UploPath {
	s := strings.TrimPrefix(path, tn.filesDir.path+string(filepath.Separator))
	sp, err := modules.NewUploPath(s)
	if err != nil {
