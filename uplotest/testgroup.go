...
var (
	...
	DefaultPaymentContractInitialFunding = types.UplocoinPrecision.Mul64(10)
...
func FundNodes(miner *TestNode, nodes map[*TestNode]struct{}) error {
	...
	scos := make([]types.UplocoinOutput, 0, uint64(len(nodes))*txnsPerNode)
	funding := wg.ConfirmedUplocoinBalance.Div64(uint64(len(nodes))).Div64(txnsPerNode + 1)
	for node := range nodes {
		...
		for i := uint64(0); i < txnsPerNode; i++ {
			scos = append(scos, types.UplocoinOutput{
				Value:      funding,
	...
	_, err = miner.WalletUplocoinsMultiPost(scos)
	if err != nil {
...
func (tg *TestGroup) setupNodes(setHosts, setNodes, setRenters map[*TestNode]struct{}) error {
	...
	for m := range tg.miners {
		...
		if wg.ConfirmedUplocoinBalance.Cmp(balance) > 0 {
			miner = m
			balance = wg.ConfirmedUplocoinBalance
		}
	}
	// Get all the nodes.
