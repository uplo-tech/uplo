...
// uploPathToFusePath will return the location that a file should exist on disk
// for a mounted fuse point. The full calculation requires knowing the uplopath,
// the fuse root, and the mountpoint on distk.
func uploPathToFusePath(sp modules.UploPath, fuseRoot modules.UploPath, mountpoint string) (string, error) {
	rebased, err := sp.Rebase(modules.RootUploPath(), fuseRoot)
	if err != nil {
		return "", errors.AddContext(err, "unable to rebase the uplopath")
	}
...
func TestFuse(t *testing.T) {
	...
	err = r.RenterFuseMount(mountpoint1, modules.RootUploPath(), defaultOpts)
	if err != nil {
		t.Fatal(err)
	}
	...
	path := remoteFile.UploPath()
	fusePath, err := uploPathToFusePath(path, modules.RootUploPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	...
	localfd1f2, err := localfd1.NewFile((int(modules.SectorSize*4) + uplotest.Fuzz()))
	if err != nil {
		t.Fatal(err)
	}
	...
	localfd2f1Path, err := uploPathToFusePath(remotefd2f1.UploPath(), modules.RootUploPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	...
	customDirUploPath, err := modules.RootUploPath().Join("custom-dir-1")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.RenterDirCreateWithModePost(customDirUploPath, customDirPerm)
	if err != nil {
		t.Fatal(err)
	}
	...
	customFileUploPath, err := customDirUploPath.Join("custom-file-1")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.RenterUploadPost(customFilePath, customFileUploPath, 1, 1)
	if err != nil {
		t.Fatal(err)
	}
	// Open the new custom dir and file in fuse and ensure that the modes are
	// set correctly.
	customDirFusePath, err := uploPathToFusePath(customDirUploPath, modules.RootUploPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	customFileFusePath, err := uploPathToFusePath(customFileUploPath, modules.RootUploPath(), mountpoint1)
	if err != nil {
		t.Fatal(err)
	}
	...
	customFileRenamedUploPath, err := customDirUploPath.Join("custom-file-1-renamed")
	if err != nil {
		t.Fatal(err)
	}
	err = r.RenterRenamePost(customFileUploPath, customFileRenamedUploPath, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Inode check. Mount the root uplofile to a special inode mountpoint then
	...
	err = r.RenterFuseMount(inodeMount, modules.RootUploPath(), defaultOpts)
	if err != nil {
		t.Fatal(err)
	}
	inodeFile1Path, err := uploPathToFusePath(remoteFile.UploPath(), modules.RootUploPath(), inodeMount)
	if err != nil {
		t.Fatal(err)
	}
	...
	inodeDir1Path, err := uploPathToFusePath(remotefd1.UploPath(), modules.RootUploPath(), inodeMount)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < threads; i++ {
		go func(id int) {
			...
			// otherwise use fd1.
			var home *uplotest.LocalDir
			var homeUploPath modules.UploPath
			if uplotest.Fuzz() < 1 {
				home = localfd1
				homeUploPath = remotefd1.UploPath()
			} else {
				home = localfd2
				homeUploPath = remotefd2.UploPath()
			}
			...
			for i := 0; i < mountIters; i++ {
				var err error
				var uploPathToMount modules.UploPath
				if uplotest.Fuzz() < 1 {
					uploPathToMount = homeUploPath
				} else {
					uploPathToMount = threadRemoteDir.UploPath()
				}
				if err != nil {
					err = errors.AddContext(err, "unable to create mountpoint")
					errMu.Lock()
				...
				err = r.RenterFuseMount(threadMount, uploPathToMount, defaultOpts)
				if err != nil {
			...
			err = r.RenterFuseMount(threadMount, modules.RootUploPath(), defaultOpts)
			if err != nil {
				err = errors.AddContext(err, "unable to mount thread mount")
				errMu.Lock()
				groupErr = errors.Compose(groupErr, err)
			...
			for i := 0; i < readIters; i++ {
				path := remoteFile.UploPath()
				fusePath, err := uploPathToFusePath(path, modules.RootUploPath(), threadMount)
				if err != nil {
			...
			if uplotest.Fuzz() == -1 {
				uploPathToMount = homeUploPath
			} else if uplotest.Fuzz() == 0 {
				uploPathToMount = threadRemoteDir.UploPath()
			} else {
				uploPathToMount = modules.RootUploPath()
			}
			err = r.RenterFuseMount(threadMount, uploPathToMount, defaultOpts)
			if err != nil {
				err = errors.AddContext(err, "unable to mount thread mount")
