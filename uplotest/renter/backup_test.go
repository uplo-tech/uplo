...
func TestCreateLoadBackup(t *testing.T) {
	...
	dirMDPath := filepath.Join(r.Dir, modules.RenterDir, modules.FileSystemRoot, modules.UserFolder.String(), "subDir", modules.uplodirExtension)
	if _, err := os.Stat(dirMDPath); os.IsNotExist(err) {
		t.Fatalf(".uplodir file at %v doesn't exist", dirMDPath)
	}
	...
	if _, err := os.Stat(dirMDPath); os.IsExist(err) {
		t.Fatal(".uplodir file does exist:", dirMDPath)
	}
	// Create a backup.
	...
	// loops to avoid updating the .uplodir file.
	...
	// The .uplodir file should also be recovered.
	dirMDPath = filepath.Join(r.Dir, modules.RenterDir, modules.FileSystemRoot, modules.UserFolder.String(), "subDir", modules.uplodirExtension)
	if _, err := os.Stat(dirMDPath); os.IsNotExist(err) {
		t.Fatalf(".uplodir file at %v doesn't exist", dirMDPath)
	}
	// There shouldn't be a .uplodir_1 file as we don't replace existing .uplodir
	// files.
	if _, err := os.Stat(dirMDPath + "_1"); os.IsExist(err) {
		t.Fatal(".uplodir_1 file does exist:", err)
	}
	// The file should be available and ready for download again.
	if _, _, err := r.DownloadByStream(rf); err != nil {
		t.Fatal(err)
	}
	// Delete the file and upload another file to the same uplopath. This one should
	// have the same uplopath but not the same UID.
	if err := r.RenterFileDeletePost(rf.UploPath()); err != nil {
		t.Fatal(err)
	}
	subDir, err = r.FilesDir().CreateDir("subDir")
	if err != nil {
	...
	sp, err := modules.NewUploPath(rf.UploPath().String() + "_1")
	if err != nil {
		t.Fatal(err)
	}
	...
	// The .uplodir file should still exist.
	if _, err := os.Stat(dirMDPath); os.IsNotExist(err) {
		t.Fatal(".uplodir file doesn't exist")
	}
	// There shouldn't be a .uplodir_1 file as we don't replace existing .uplodir
	// files.
	if _, err := os.Stat(dirMDPath + "_1"); !os.IsNotExist(err) {
		t.Fatal(".uplodir_1 file does exist")
	}
}
...
func TestRemoteBackup(t *testing.T) {
	...
	if err := r.RenterFileDeletePost(rf2.UploPath()); err != nil {
		t.Fatal(err)
	}
	...
	// Confirm uplodir exists by querying directory
	rd, err := r.RenterDirGet(modules.RootUploPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	rd, err = r.RenterDirGet(rd.Directories[1].UploPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	// pubkey.
	_, err = r.RenterBackupsOnHost(types.UploPublicKey{})
	if err == nil {
		t.Error(err)
	}
}
