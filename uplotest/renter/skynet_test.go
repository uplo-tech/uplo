...
func TestSkynet(t *testing.T) {
	...
	subTests := []uplotest.SubTest{
		...
		{Name: "ConvertUploFile", Test: testConvertUploFile},
		...
		{Name: "RenameUploPath", Test: testRenameUploPath},
...
func testSkynetBasic(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	...
	filename := "testSmall"
	uploadUploPath, err := modules.NewUploPath("testSmallPath")
	if err != nil {
		t.Fatal(err)
	}
	...
	skynetUploadPath, err := modules.SkynetFolder.Join(uploadUploPath.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	// Create some data to upload as a skyfile.
	rootData := fastrand.Bytes(100 + uplotest.Fuzz())
	// Need it to be a reader.
	rootReader := bytes.NewReader(rootData)
	// Call the upload skyfile client call.
	rootFilename := "rootTestSmall"
	rootUploadUploPath, err := modules.NewUploPath("rootTestSmallPath")
	if err != nil {
		t.Fatal(err)
	}
	// Quick fuzz on the force value so that sometimes it is set, sometimes it
	...
	// Get the list of files in the skynet directory and see if the file is
	// present.
	rootRdg, err := r.RenterDirRootGet(modules.RootUploPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	err = build.Retry(250, 250*time.Millisecond, func() error {
		uploadedFile, err := r.RenterFileRootGet(rootUploadUploPath)
		if err != nil {
			return err
		}
	...
	// one sector.
	largeData := fastrand.Bytes(int(modules.SectorSize*2) + uplotest.Fuzz())
	largeReader := bytes.NewReader(largeData)
	largeFilename := "testLarge"
	largeUploPath, err := modules.NewUploPath("testLargePath")
	if err != nil {
		t.Fatal(err)
	}
	var force2 bool
	...
	if !bytes.Equal(largeFetchedData, largeData) {
		t.Error("upload and download data does not match for large uplofiles", len(largeFetchedData), len(largeData))
	}

	// Check the metadata of the uplofile, see that the metadata of the uplofile
	// has the skylink referenced.
	largeUploadPath, err := modules.NewUploPath("testLargePath")
	if err != nil {
		t.Fatal(err)
	}
	...
	// the same as the uplofile values, because the uplofile was created
	// separately.
	//
	...
	//
	// Try to download the file behind the skylink.
	pinUploPath, err := modules.NewUploPath("testSmallPinPath")
	if err != nil {
		t.Fatal(err)
	}
	...
	err = r.RenterFileDeleteRootPost(fullPinUploPath)
	if err != nil {
		t.Fatal(err)
	}
	// Make sure the file is no longer present.
	_, err = r.RenterFileRootGet(fullPinUploPath)
	...
	largePinUploPath, err := modules.NewUploPath("testLargePinPath")
	if err != nil {
		t.Fatal(err)
	}
	largePinLUP := modules.SkyfilePinParameters{
		UploPath:             largePinUploPath,
		Force:               force,
		Root:                false,
		BaseChunkRedundancy: 2,
	}
	...
	// Use a different Uplopath to avoid path conflict.
	largePinUploPath, err = modules.NewUploPath("testLargePinPath2")
	if err != nil {
		t.Fatal(err)
	}
	largePinLUP = modules.SkyfilePinParameters{
	...
	// See if the file is present.
	fullLargePinUploPath, err := modules.SkynetFolder.Join(largePinUploPath.String())
	if err != nil {
		t.Fatal(err)
	}
	pinnedFile, err = r.RenterFileRootGet(fullLargePinUploPath)
	if err != nil {
		t.Fatal(err)
	}
	if len(pinnedFile.File.Skylinks) != 1 {
	...
	err = r.RenterFileDeleteRootPost(fullLargePinUploPath)
	if err != nil {
		t.Fatal(err)
	}
	// Make sure the file is no longer present.
	_, err = r.RenterFileRootGet(fullLargePinUploPath)
	if err == nil || !strings.Contains(err.Error(), filesystem.ErrNotExist.Error()) {
		t.Fatal("skyfile still present after deletion")
	}
	...
	// TODO: Fetch both the skyfile and the uplofile that was uploaded, make sure
...
// testConvertUploFile tests converting a uplofile to a skyfile. This test checks
// for 1-of-N redundancies and N-of-M redundancies.
func testConvertUploFile(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]

	// Upload a uplofile that will then be converted to a skyfile.
	//
	...
	// uplofile being a N-of-M redundancy
	_, err = r.SkynetConvertUplofileToSkyfilePost(sup, remoteFile.UploPath())
	if err == nil {
		t.Fatal("Expected conversion from Uplofile to Skyfile Post to fail.")
	}
	...
	sshp, err := r.SkynetConvertUplofileToSkyfilePost(sup, remoteFile.UploPath())
	if err != nil {
		t.Fatal(err)
	}
...
func testSkynetMultipartUpload(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	sk, err := r.SkykeyCreateKeyPost(t.Name(), skykey.TypePrivateID)
	if err != nil {
		t.Fatal(err)
	...
	//
	// Define test func
	testSmallFunc := func(files []uplotest.TestFile, fileName, skykeyName string) {
	...
	rootFile := uplotest.TestFile{Name: "file1", Data: []byte("File1Contents")}
	nestedFile := uplotest.TestFile{Name: "nested/file2.html", Data: []byte("File2Contents")}
	files := []uplotest.TestFile{rootFile, nestedFile}
	...
	largeTestFunc := func(files []uplotest.TestFile, fileName, skykeyName string) {
		// Upload the skyfile
		...
		if !bytes.Equal(largeFetchedData, allData) {
			t.Fatal("upload and download data does not match for large uplofiles", len(largeFetchedData), len(allData))
		}

		// Check the metadata of the uplofile, see that the metadata of the uplofile
		// has the skylink referenced.
		largeSkyfilePath, err := modules.SkynetFolder.Join(sup.UploPath.String())
		if err != nil {
			t.Fatal(err)
		}
		largeRenterFile, err := r.RenterFileRootGet(largeSkyfilePath)
		...
		if !bytes.Equal(smallFetchedData, rootFile.Data) {
			t.Fatal("upload and download data does not match for large uplofiles with subfiles", len(smallFetchedData), len(rootFile.Data))
		}
		...
		if !bytes.Equal(largeFetchedData, nestedFile.Data) {
			t.Fatal("upload and download data does not match for large uplofiles with subfiles", len(largeFetchedData), len(nestedFile.Data))
		}
	}

	// Add a small file at root level and a large nested file
	rootFile = uplotest.TestFile{Name: "smallFile1.txt", Data: []byte("File1Contents")}
	largeData := fastrand.Bytes(2 * int(modules.SectorSize))
	nestedFile = uplotest.TestFile{Name: "nested/largefile2.txt", Data: largeData}
	files = []uplotest.TestFile{rootFile, nestedFile}
...
func testSkynetStats(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	...
	var sps []modules.UploPath
	for name, size := range files {
		_, sup, _, err := r.UploadNewSkyfileBlocking(name, size, false)
		if err != nil {
			t.Fatal(err)
		}
		sp, err := sup.UploPath.Rebase(modules.RootUploPath(), modules.SkynetFolder)
		if err != nil {
			t.Fatal(err)
		}
		sps = append(sps, sp)
	...
	// Create a uplofile and convert it
	size := 100
	...
	_, err = r.SkynetConvertUplofileToSkyfilePost(sup, rf.UploPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	// conversion as well as the file size of the uplofile.
	uploadedFilesSize += modules.SectorSize
	...
	for _, sp := range sps {
		...
		extSP, err := modules.NewUploPath(sp.String() + modules.ExtendedSuffix)
		if err != nil {
			t.Fatal(err)
		}
	...
	convertSP, err := rf.UploPath().Rebase(modules.RootUploPath(), modules.SkynetFolder)
	if err != nil {
		t.Fatal(err)
	}
...
func testSkynetInvalidFilename(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	...
	for _, filename := range filenames {
		uploadUploPath, err := modules.NewUploPath("testInvalidFilename" + persist.RandomSuffix())
		if err != nil {
			t.Fatal(err)
		}
		...
		// Call the upload skyfile client call.
		uploadUploPath, err = modules.NewUploPath("testInvalidFilenameMultipart" + persist.RandomSuffix())
		if err != nil {
			t.Fatal(err)
		}
	...
	uploadUploPath, err := modules.NewUploPath("testInvalidFilename")
	if err != nil {
		t.Fatal(err)
	}
	sup := modules.SkyfileUploadParameters{
	...
	subfiles := make(modules.SkyfileSubfiles)
	subfiles[subfile.Filename] = subfile
	uploadUploPath, err = modules.NewUploPath("testInvalidFilenameMultipart")
	if err != nil {
		t.Fatal(err)
	}
	mup := modules.SkyfileMultipartUploadParameters{
...
func testSkynetDownloadFormats(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	...
	uploadUploPath, err := modules.NewUploPath("testSkynetDownloadFormats")
	if err != nil {
		t.Fatal(err)
	}
...
// other than 0.
func testSkynetDownloadRangeEncrypted(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
...
// baseSector
func testSkynetDownloadBaseSectorEncrypted(t *testing.T, tg *uplotest.TestGroup) {
	testSkynetDownloadBaseSector(t, tg, "basesectorkey")
}

// testSkynetDownloadBaseSectorNoEncryption tests downloading a skylink's
// baseSector
func testSkynetDownloadBaseSectorNoEncryption(t *testing.T, tg *uplotest.TestGroup) {
	testSkynetDownloadBaseSector(t, tg, "")
}

// testSkynetDownloadBaseSector tests downloading a skylink's baseSector
func testSkynetDownloadBaseSector(t *testing.T, tg *uplotest.TestGroup, skykeyName string) {
	r := tg.Renters()[0]
	...
	if !reflect.DeepEqual(expected, metadata) {
		uplotest.PrintJSON(expected)
		uplotest.PrintJSON(metadata)
...
func testSkynetDownloadByRootEncrypted(t *testing.T, tg *uplotest.TestGroup) {
	testSkynetDownloadByRoot(t, tg, "rootkey")
}

// testSkynetDownloadByRootNoEncryption tests downloading by root
func testSkynetDownloadByRootNoEncryption(t *testing.T, tg *uplotest.TestGroup) {
	testSkynetDownloadByRoot(t, tg, "")
}

// testSkynetDownloadByRoot tests downloading by root
func testSkynetDownloadByRoot(t *testing.T, tg *uplotest.TestGroup, skykeyName string) {
	r := tg.Renters()[0]
...
func testSkynetFanoutRegression(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
...
func testSkynetSubDirDownload(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	...
	uploadUploPath, err := modules.NewUploPath(name)
	if err != nil {
		t.Fatal(err)
	}
...
func testSkynetDisableForce(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
...
func testSkynetBlocklistHash(t *testing.T, tg *uplotest.TestGroup) {
	testSkynetBlocklist(t, tg, true)
}

// testSkynetBlocklistSkylink tests the skynet blocklist module when submitting
// skylinks
func testSkynetBlocklistSkylink(t *testing.T, tg *uplotest.TestGroup) {
	testSkynetBlocklist(t, tg, false)
}

// testSkynetBlocklist tests the skynet blocklist module
func testSkynetBlocklist(t *testing.T, tg *uplotest.TestGroup, isHash bool) {
	r := tg.Renters()[0]
	...
	size := modules.SectorSize + uint64(100+uplotest.Fuzz())
	skylink, sup, sshp, err := r.UploadNewSkyfileBlocking(t.Name(), size, false)
	if err != nil {
		t.Fatal(err)
	}

	// Remember the uploPaths of the blocked files
	var blockedUploPaths []modules.UploPath

	// Confirm that the skyfile and its extended info are registered with the
	// renter
	sp, err := modules.SkynetFolder.Join(sup.UploPath.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	spExtended, err := modules.NewUploPath(sp.String() + modules.ExtendedSuffix)
	if err != nil {
		t.Fatal(err)
	}
	...
	blockedUploPaths = append(blockedUploPaths, sp, spExtended)
	...
	// Verify that the UploPath and Extended UploPath were removed from the renter
	...
	// Upload a normal uplofile with 1-of-N redundancy
	...
	convertSSHP, err := r.SkynetConvertUplofileToSkyfilePost(convertUP, rf.UploPath())
	if err != nil {
		t.Fatal(err)
	}
	convertSkylink := convertSSHP.Skylink

	// Confirm there is a uplofile and a skyfile
	_, err = r.RenterFileGet(rf.UploPath())
	if err != nil {
		t.Fatal(err)
	}
	skyfilePath, err := modules.SkynetFolder.Join(rf.UploPath().String())
	if err != nil {
		t.Fatal(err)
	}
	...
	// Make sure all blockedUploPaths are root paths
	sp, err = modules.UserFolder.Join(rf.UploPath().String())
	if err != nil {
		t.Fatal(err)
	}
	blockedUploPaths = append(blockedUploPaths, sp, skyfilePath)
	...
	_, err = r.SkynetConvertUplofileToSkyfilePost(convertUP, rf.UploPath())
	if err == nil || !strings.Contains(err.Error(), renter.ErrSkylinkBlocked.Error()) {
		t.Fatalf("Expected error %v but got %v", renter.ErrSkylinkBlocked, err)
	}

	// This should delete the skyfile but not the uplofile
	_, err = r.RenterFileGet(rf.UploPath())
	if err != nil {
		t.Fatal(err)
	}
	...
	// in the renter.
	for _, uploPath := range blockedUploPaths {
		_, err = r.RenterFileRootGet(uploPath)
		if err != nil {
			t.Error(err)
		}
	}
	...
	if err := build.Retry(15, time.Second, func() error {
		for _, uploPath := range blockedUploPaths {
			_, err = r.RenterFileRootGet(uploPath)
			if err == nil || !strings.Contains(err.Error(), filesystem.ErrNotExist.Error()) {
				return fmt.Errorf("File %v, not deleted; error: %v", uploPath, err)
			}
		}
		return nil
	}); err != nil {
...
// skylinks
func testSkynetBlocklistUpgrade(t *testing.T, tg *uplotest.TestGroup) {
	...
	blacklistCompatFile, err := os.Open("../../compatibility/skynetblacklistv143_uplotest")
	if err != nil {
		t.Fatal(err)
	}
	...
	skylinkFile, err := os.Open("../../compatibility/skylinkv143_uplotest")
	if err != nil {
		t.Fatal(err)
	}
...
func testSkynetPortals(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]

	portal1 := modules.SkynetPortal{
		Address: modules.NetAddress("uplosky.net:8480"),
		Public:  true,
	}
	// loopback address
	...
	portal3 := modules.SkynetPortal{
		Address: modules.NetAddress("uplosky.net"),
		Public:  true,
	}
...
func testSkynetHeadRequest(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
...
func testSkynetNoMetadata(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
...
func testSkynetIncludeLayout(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
...
func testSkynetNoWorkers(t *testing.T, tg *uplotest.TestGroup) {
...
func testSkynetDryRunUpload(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	...
	verifyDryRun := func(sup modules.SkyfileUploadParameters, dataSize int) {
		...
		skyfilePath, err := modules.SkynetFolder.Join(sup.UploPath.String())
		if err != nil {
			t.Fatal(err)
		}
		_, err = r.RenterFileRootGet(skyfilePath)
	...
	uploadUploPath, err := modules.NewUploPath(fmt.Sprintf("%s%s", t.Name(), "S"))
	if err != nil {
		t.Fatal(err)
	}
	...
	uploadUploPath, err = modules.NewUploPath(fmt.Sprintf("%s%s", t.Name(), "L"))
	if err != nil {
		t.Fatal(err)
	}
	verifyDryRun(modules.SkyfileUploadParameters{
...
func testSkynetRequestTimeout(t *testing.T, tg *uplotest.TestGroup) {
	...
	pinUploPath, err := modules.NewUploPath(t.Name())
	if err != nil {
		t.Fatal(err)
	}
	pinLUP := modules.SkyfilePinParameters{
...
// resumed.
func testRegressionTimeoutPanic(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
...
func testSkynetLargeMetadata(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	...
	filedata := fastrand.Bytes(int(100 + uplotest.Fuzz()))
	files := []uplotest.TestFile{{Name: filename, Data: filedata}}
...
// testRenameUploPath verifies that the uplopath to the skyfile can be renamed.
func testRenameUploPath(t *testing.T, tg *uplotest.TestGroup) {
	// Grab Renter
	r := tg.Renters()[0]
	...
	err = r.RenterRenamePost(uploPath, modules.RandomUploPath(), false)
	if err == nil {
	...
	uploPath, err = modules.SkynetFolder.Join(uploPath.String())
	if err != nil {
		t.Fatal(err)
	}
	newUploPath, err := modules.SkynetFolder.Join(persist.RandomSuffix())
	if err != nil {
		t.Fatal(err)
	}
	err = r.RenterRenamePost(uploPath, newUploPath, true)
	if err != nil {
		t.Fatal(err)
	}
...
// correctly
func testSkynetDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	// Specify subtests to run
	subTests := []uplotest.SubTest{
...
func testHasIndexNoDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
	fc2 := "File2Contents"
...
func testHasIndexDisabledDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
	fc2 := "File2Contents"
...
func testHasIndexDifferentDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
	fc2 := "File2Contents"
...
func testHasIndexInvalidDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
	fc2 := "File2Contents"
...
// It should return the content of about.html.
func testNoIndexDifferentDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
	fc2 := "File2Contents"
	aboutHtml := "about.html"
...
// INVALID default path.
// This should fail on upload with "invalid default path provided".
func testNoIndexInvalidDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
	fc2 := "File2Contents"
	invalidPath := "invalid.js"
...
// It should not return an error and download the file as zip
func testNoIndexNoDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
	fc2 := "File2Contents"
	files := []uplotest.TestFile{
...
// It should not return an error and download the file as zip.
func testNoIndexSingleFileDisabledDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
...
func testNoIndexSingleFileNoDefaultPath(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
	fc1 := "File1Contents"
	files := []uplotest.TestFile{
		{Name: "index.js", Data: []byte(fc1)},
...
// to default path.
func testSkynetDefaultPath_TableTest(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]

	fc1 := []byte("File1Contents")
	fc2 := []byte("File2Contents. This one is longer.")

	singleFile := []uplotest.TestFile{
		{Name: "about.html", Data: fc1},
	}
	singleDir := []uplotest.TestFile{
		{Name: "dir/about.html", Data: fc1},
	}
	multiHasIndex := []uplotest.TestFile{
		{Name: "index.html", Data: fc1},
		{Name: "about.html", Data: fc2},
	}
	multiHasIndexIndexJs := []uplotest.TestFile{
		{Name: "index.html", Data: fc1},
		{Name: "index.js", Data: fc1},
		{Name: "about.html", Data: fc2},
	}
	multiNoIndex := []uplotest.TestFile{
...
func testSkynetSingleFileNoSubfiles(t *testing.T, tg *uplotest.TestGroup) {
	r := tg.Renters()[0]
...
func TestFormContractBadScore(t *testing.T) {
	...
	// should be formed.
	time.Sleep(time.Second * 5)
	err = uplotest.CheckExpectedNumberOfContracts(r, 1, 0, 0, 0, 0, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenewContractBadScore(t *testing.T) {
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		return uplotest.CheckExpectedNumberOfContracts(r, 2, 0, 0, 0, 0, 0)
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		return uplotest.CheckExpectedNumberOfContracts(r, 0, 0, 0, 0, 2, 0)
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSkynetCleanupOnError(t *testing.T) {
	...
	rt := node.RenterTemplate
	rt.Allowance = uplotest.DefaultAllowance
	rt.Allowance.PaymentContractInitialFunding = uplotest.DefaultPaymentContractInitialFunding
	rt.RenterDeps = deps
	nodes, err := tg.AddNodes(rt)
	if err != nil {
		t.Fatal(err)
	...
	// Create a helper function that returns true if the uplopath does not exist.
	skyfileDeleted := func(path modules.UploPath) bool {
	...
	smallPath, err := modules.SkynetFolder.Join(small.UploPath.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	largePath, err := modules.SkynetFolder.Join(large.UploPath.String())
	if err != nil {
		t.Fatal(err)
	}
	...
	largePathExtended, err := modules.NewUploPath(largePath.String() + modules.ExtendedSuffix)
	if err != nil {
		t.Fatal(err)
	}
