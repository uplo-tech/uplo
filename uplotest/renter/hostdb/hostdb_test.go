...
// TestUplomuxRequired checks that the hostdb will count a host as offline if the
// host is not running uplomux.
func TestUplomuxRequired(t *testing.T) {
	if testing.Short() {
		t.SkipNow()
	}
	t.Parallel()
	...
	// dependency.
	deps := &dependencies.DependencyDisableHostUplomux{}
	...
	tg, err := uplotest.NewGroup(testDir, renterTemplate, hostTemplate, node.Miner(filepath.Join(testDir, "miner")))
	if err != nil {
...
func TestInitialScanComplete(t *testing.T) {
	...
	tg, err := uplotest.NewGroup(testDir, renterTemplate, node.Host(filepath.Join(testDir, "host")),
...
func testFilterMode(tg *uplotest.TestGroup, renter *uplotest.TestNode, fm modules.FilterMode) error {
	// Grab all host pks
	...
	var filteredHosts []types.UploPublicKey
	numHosts := 0
	...
	var nullList []types.UploPublicKey
