...
func TestGatewayBlocklist(t *testing.T) {
	...
	// Create Gateway
	testDir := gatewayTestDir(t.Name())
	gateway, err := uplotest.NewCleanNode(node.Gateway(testDir))
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestGatewayOfflineAlert(t *testing.T) {
	...
	testNode, err := uplotest.NewCleanNode(params)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	testNode2, err := uplotest.NewCleanNodeAsync(node.Gateway(testDir + "2"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestGatewayBandwidth(t *testing.T) {
	...
	// Create two Gateways
	testDir := gatewayTestDir(t.Name())
	gateway1, err := uplotest.NewCleanNode(node.Gateway(testDir + "/gateway1"))
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	gateway2, err := uplotest.NewCleanNode(node.Gateway(testDir + "/gateway1"))
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
