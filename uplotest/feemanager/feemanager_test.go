...
func TestFeeManager(t *testing.T) {
	...
	fm, err := uplotest.NewCleanNode(node.FeeManager(testDir))
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestFeeManagerProcessFee(t *testing.T) {
	...
	tg, err := uplotest.NewGroup(testDir, feeManagerParams, minerParams, renterParams, renterParams2)
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	nebulousStartingBal := nwg.ConfirmedUplocoinBalance
	rwg, err := r.WalletGet()
	if err != nil {
		t.Fatal(err)
	}
	renterStartingBal := rwg.ConfirmedUplocoinBalance
	fmwg, err := fm.WalletGet()
	if err != nil {
		t.Fatal(err)
	}
	fmStartingBal := fmwg.ConfirmedUplocoinBalance
	...
	amount := types.UplocoinPrecision.Mul64(10)
	appUID := modules.AppUID("testapp")
	recurring := fastrand.Intn(2) == 0
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		if nwg.ConfirmedUplocoinBalance.Cmp(nebExpected) != 0 {
			return fmt.Errorf("Incorrect Nebulous wallet balance; expected %v, got %v", nebExpected.HumanString(), nwg.ConfirmedUplocoinBalance.HumanString())
		}
		...
		if rwg.ConfirmedUplocoinBalance.Cmp(devExpected) != 0 {
			return fmt.Errorf("Incorrect Renter wallet balance; expected %v, got %v", devExpected.HumanString(), rwg.ConfirmedUplocoinBalance.HumanString())
		}
		...
		if fmwg.ConfirmedUplocoinBalance.Cmp(fmExpected) >= 0 {
			return fmt.Errorf("Incorrect FeeManager wallet balance; expected less than %v, got %v", fmExpected.HumanString(), fmwg.ConfirmedUplocoinBalance.HumanString())
		}
		return nil
	})
	if err != nil {
