...
func TestApiHeight(t *testing.T) {
	...
	// Create a new server
	testNode, err := uplotest.NewNode(node.AllModules(testDir))
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
...
func TestConsensusBlocksIDGet(t *testing.T) {
	...
	tg, err := uplotest.NewGroupFromTemplate(consensusTestDir(t.Name()), groupParams)
	if err != nil {
		...
		for _, tx := range cbhg.Transactions {
			...
			for i, fc := range tx.FileContracts {
				...
				for _, vp := range fc.ValidProofOutputs {
					txn.FileContracts[i].ValidProofOutputs = append(txn.FileContracts[i].ValidProofOutputs, types.UplocoinOutput{
						Value:      vp.Value,
						UnlockHash: vp.UnlockHash,
					})
				}
				for _, mp := range fc.MissedProofOutputs {
					txn.FileContracts[i].MissedProofOutputs = append(txn.FileContracts[i].MissedProofOutputs, types.UplocoinOutput{
						Value:      mp.Value,
						UnlockHash: mp.UnlockHash,
					})
				}
			}
			for _, sfo := range tx.UplofundOutputs {
				txn.UplofundOutputs = append(txn.UplofundOutputs, types.UplofundOutput{
					Value:      sfo.Value,
			...
			// Verify UplocoinOutput IDs
			for i, sco := range tx.UplocoinOutputs {
				if sco.ID != txn.UplocoinOutputID(uint64(i)) {
					t.Fatalf("UplocoinOutputID not as expected, got %v expected %v", sco.ID, txn.UplocoinOutputID(uint64(i)))
				}
			}
			...
			// Verify UplofundOutput IDs
			for i, sfo := range tx.UplofundOutputs {
				// Failing, switch back to !=
				if sfo.ID != txn.UplofundOutputID(uint64(i)) {
					t.Fatalf("UplofundOutputID not as expected, got %v expected %v", sfo.ID, txn.UplofundOutputID(uint64(i)))
				}
			}
		}
	}
...
func TestFoundationHardfork(t *testing.T) {
	...
	// Have the renter upload some files to Uplo prior to the foundation fork
	...
	localFile, remoteFile, err := r.UploadNewFileBlocking(100+uplotest.Fuzz(), 1, 1, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	if balance.Cmp(types.UplocoinPrecision.Mul64(100)) < 0 {
		t.Log(balance)
	...
	primaryWSP, err := w.WalletUplocoinsPost(types.UplocoinPrecision, foundationPrimaryAddress, false)
	if err != nil {
		t.Fatal(err)
	}
	failsafeWSP, err := w.WalletUplocoinsPost(types.UplocoinPrecision, foundationFailsafeAddress, false)
	if err != nil {
		t.Fatal(err)
	}
	failsafeWSP2, err := w.WalletUplocoinsPost(types.UplocoinPrecision, foundationFailsafeAddress, false)
	if err != nil {
		t.Fatal(err)
	}
	//
	// Grab the output ids for the Uplocoin outputs that were sent to the
	// foundation addresses. These will be needed to create the transaction that
	// updates the subsidy ownership.
	var primaryOutputID types.UplocoinOutputID
	var failsafeOutputID types.UplocoinOutputID
	var failsafeOutputID2 types.UplocoinOutputID
	found := false
	for _, txn := range primaryWSP.Transactions {
		for i, output := range txn.UplocoinOutputs {
			if output.UnlockHash == foundationPrimaryAddress {
				found = true
				primaryOutputID = txn.UplocoinOutputID(uint64(i))
			}
		}
	}
	if !found {
	...
	for _, txn := range failsafeWSP.Transactions {
		for i, output := range txn.UplocoinOutputs {
			if output.UnlockHash == foundationFailsafeAddress {
				found = true
				failsafeOutputID = txn.UplocoinOutputID(uint64(i))
			}
		}
	}
	if !found {
	...
	for _, txn := range failsafeWSP2.Transactions {
		for i, output := range txn.UplocoinOutputs {
			if output.UnlockHash == foundationFailsafeAddress {
				found = true
				failsafeOutputID2 = txn.UplocoinOutputID(uint64(i))
			}
		}
	}
	if !found {
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		for _, txn := range tptg.Transactions {
			for _, output := range txn.UplocoinOutputs {
				if output.UnlockHash == foundationPrimaryAddress {
					primaryFound = true
					continue
				}
	...
	txn := types.Transaction{
		...
		UplocoinOutputs: []types.UplocoinOutput{{
			Value:      types.InitialFoundationSubsidy.Sub(types.UplocoinPrecision),
			UnlockHash: addr,
		}},
		MinerFees: []types.Currency{
			types.UplocoinPrecision,
		},
		TransactionSignatures: make([]types.TransactionSignature, foundationPrimaryUnlockConditions.SignaturesRequired),
	}
	...
	_, err = w.WalletUplocoinsPost(types.InitialFoundationSubsidy, wag.Address, false)
	if err != nil {
		t.Fatal(err)
	}
	waitForTxns(2)
	...
	txn = types.Transaction{
		...
		UplocoinOutputs: []types.UplocoinOutput{{
			Value:      subsidyCoins.Sub(types.UplocoinPrecision),
			UnlockHash: addr,
		}},
		MinerFees: []types.Currency{
			types.UplocoinPrecision,
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		expectedBalance := originalBalance.Add(subsidyCoins).Sub(types.UplocoinPrecision)
	...
	_, err = w.WalletUplocoinsPost(balance.Sub(types.UplocoinPrecision), wag.Address, false)
	if err != nil {
		t.Fatal(err)
	}
	waitForTxns(2)
	...
	err = build.Retry(100, 100*time.Millisecond, func() error {
		...
		if balance.Cmp(types.UplocoinPrecision) >= 0 {
	...
	if newBalance.Cmp(balance.Add(subsidyCoins).Sub(types.UplocoinPrecision)) != 0 {
