...
\title{Uplo: Simple Decentralized Storage}
...
\subsection*{Abstract}
The authors introduce Uplo, a platform for decentralized storage.
Uplo enables the formation of storage contracts between peers.
...
In this respect, Uplo can be viewed as a Bitcoin derivative that includes support for such contracts.
Uplo will initially be implemented as an altcoin, and later financially connected to Bitcoin via a two-way peg.

\section{Introduction}
Uplo is a decentralized cloud storage platform that intends to compete with existing storage solutions, at both the P2P and enterprise level.
Instead of renting storage from a centralized provider, peers on Uplo rent storage from each other.
Uplo itself stores only the storage contracts formed between parties, defining the terms of their arrangement.
...
Uplo will initially be implemented as a blockchain-based altcoin.
...
The Uplo protocol largely resembles Bitcoin except for the changes noted below.

\section{General Structure}
Uplo's primary departure from Bitcoin lies in its transactions.
...
Uplo opts instead to use an $M$--of--$N$ multi-signature scheme for all transactions, eschewing the scripting system entirely.
This reduces complexity and attack surface.

Uplo also extends transactions to enable the creation and enforcement of storage contracts.
...
Uplo relies on an ecosystem that facilitates decentralized storage.
...
However, we reiterate that uptime incentives are not part of the Uplo protocol; they are entirely dependent on client behavior.
...
\section{Uplofunds}
Uplo is a product of Nebulous Incorporated.
Nebulous is a for-profit company, and Uplo is intended to become a primary source of income for the company.
...
Instead, Nebulous intends to generate revenue from Uplo in a manner proportional to the value added by Uplo, as determined by the value of the contracts set up between clients and hosts.
This is accomplished by imposing a fee on all contracts.
When a contract is created, 3.9\% of the contract fund is removed and distributed to the holders of \textit{uplofunds}.
Nebulous Inc. will initially hold approx. 88\% of the uplofunds, and the early crowd-fund backers of Uplo will hold the rest.

Uplofunds can be sent to other addresses, in the same way that Uplocoins can be sent to other addresses.
They cannot, however, be used to fund contracts or miner fees.
When uplofunds are transferred to a new address, an additional unspent output is created, containing all of the Uplocoins that have been earned by the uplofunds since their previous transfer.
These Uplocoins are sent to the same address as the uplofunds.

\section{Economics of Uplo}
The primary currency of Uplo is the Uplocoin.
The supply of Uplocoins will increase permanently, and all fresh supply will be given to miners as a block subsidy.
...
There are inefficiencies within the Uplo incentive scheme.
The primary goal of Uplo is to provide a blockchain that enforces storage contracts.
The mining reward, however, is only indirectly linked to the total value of contracts being created.

The Uplocoin, especially initially, is likely to have high volatility.
...
Additionally, hosts can advertise their prices in a more stable currency (like USD) and convert to Uplocoin immediately before finalizing a contract.
...
% The primary foundation of Uplo has been established above.
% Other considerations, such as mining algorithms, block time, etc., can be assumed to mirror those found in Bitcoin.

% Giving careful attention to ``A Treatise on Altcoins'' \cite{alts}, we are considering the following changes to Uplo for the overall improvement of the cryptocurrency.
...
% \subsection{Uplofund the Miner fees}
% Have some portion of uplofunds contribute to the miner fees, which ensures that miners have compensation so long as Uplo is being used for its core purpose - storage.
...
% Uplo could enforce something like a single leading 1, followed by a bunch of leading 0s.
% This creates the property that no hash is ever valid for both Bitcoin and Uplo.
...
\section{Conclusion}
Uplo is a variant on the Bitcoin protocol that enables decentralized file storage via cryptographic contracts.
...
Uplofunds are used as a mechanism of generating revenue for Nebulous Inc., the company responsible for the release and maintenance of Uplo.
By using Uplofunds instead of premining, Nebulous more directly correlates revenue to actual use of the network, and is largely unaffected by market games that malicious entities may play with the network currency.
Miners may also derive a part of their block subsidy from uplofunds, with similar benefits.
...
We believe Uplo will provide a fertile platform for decentralized cloud storage in trustless environments.
...
\bibitem{cpr}
	...
	Proc. of Auplocrypt 2008, vol. 5350, Dec 2008, pp. 90-107.
